# -*- coding: utf-8 -*-
"""D-372: extend complete-branch inner replay through XGPIO3.

Scratch only.  At D-367's selected U3/R58 layout, reserve XGPIO6/XGPIO7,
replace XGPIO5 on In3, XGPIO4 on In2, and XGPIO2 on In3 using the mechanisms
proven by D-369 through D-371, then replace the complete XGPIO3 branch with
the same qualified native-face/inner-haul mechanism.  In2 and In3 are screened
independently for XGPIO3.  Remaining accepted U3 branches are restored exactly
and attached only after all four replacements close.  The authoritative PCB
is never edited.
"""
...
OUT = os.path.join(SP, "u3_xgpio3_inner_replay_074.json")
SCRATCH = os.path.join(SP, "w", "U3_XGPIO3_INNER_REPLAY_074")
X3_LAYERS = ("I2", "I3")


def restore_except_replaced(pcb, allowed):
    wanted = collections.Counter({s: n for s, n in allowed.items()
                                  if s[1] not in ("/XGPIO2", "/XGPIO3", "/XGPIO4", "/XGPIO5")})
...
def main():
    ...
    baseline, _ = RU.drc(IR.AUTH, "u3x3inner_base", SCRATCH)
    boundaries = {net: sum(n for s, n in allowed.items() if s[1] == net)
                  for net in ("/XGPIO2", "/XGPIO3", "/XGPIO4", "/XGPIO5")}
    attempts = []
    for x3_inner in X3_LAYERS:
        pcb, removed = D367.prepare("x3_" + x3_inner.lower(), allowed, (0.0, -0.5))
        ...
        x3 = (route_inner(qb, "XGPIO3_INNER", x3_inner) if x2["ok"]
        ...
        if x3["ok"]:
            ...
            qb = QR.QBoard(pcb); IR.inject_existing_via_obstacles(qb)
            for net in D364.SCHEDULE[4:]:
                attachments.append(D364.attach_terminal(qb, net, branches[net]["u3_pad"],
                                                         branches[net]["width"]))
                if not attachments[-1]["ok"]: break
            if len(attachments) == len(D364.SCHEDULE)-4 and all(x["ok"] for x in attachments):
        ...
        drc, details = RU.drc(pcb, "u3x3inner_"+x3_inner.lower(), SCRATCH)
        ...
        closed = (removed == allowed and x5["ok"] and x4["ok"] and x2["ok"] and x3["ok"]
                  and restored == wanted and len(attachments) == len(D364.SCHEDULE)-3
        ...
        row = {"xgpio3_inner_layer": x3_inner, "xgpio4_inner_layer": "I2",
               "xgpio5_inner_layer": "I3", "pair_routes": pair,
               "xgpio5_route": x5, "xgpio4_route": x4,
               "xgpio2_route": x2, "xgpio3_route": x3,
        ...
        attempts.append(row)
        print(x3_inner, "x5", x5["ok"], "x4", x4["ok"], "x2", x2["ok"], "x3", x3["ok"],
    ...
    ev = {"schema_version":1, "decision":"D-372", "source_decision":"D-371",
          "authoritative_board_sha256":auth_sha,
          "authoritative_unchanged":hashlib.sha256(open(IR.AUTH,"rb").read()).hexdigest()==auth_sha,
          "method":"complete_XGPIO5_In3_XGPIO4_In2_XGPIO2_In3_then_complete_XGPIO3_inner_branch_replacement",
          ...
          "xgpio3_mechanism_wins":sum(1 for x in attempts if x["xgpio3_route"]["ok"]),
          "transaction_candidates":len(wins), "promotion_candidate":False,
          "conclusion":"closed_U3_transaction_candidate" if wins else "XGPIO3_inner_replacement_characterized"}
    with open(OUT,"w",encoding="utf-8") as f: json.dump(ev,f,indent=2,sort_keys=True)
    print("RESULT",ev["conclusion"],"x3 wins",ev["xgpio3_mechanism_wins"],
