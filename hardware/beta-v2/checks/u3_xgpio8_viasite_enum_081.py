# -*- coding: utf-8 -*-
"""D-379: enumerate explicit XGPIO8 via sites after the D-377 prefix.

Scratch only.  Build the six proven replacement routes, then enumerate each
XGPIO8 endpoint independently on In2/In3.  Test both endpoint orders and join
only distinct pairs which can actually be reserved.  The authoritative PCB is
never edited.
"""
...
OUT = os.path.join(SP, "u3_xgpio8_viasite_enum_081.json")
SCRATCH = os.path.join(SP, "w", "U3_XGPIO8_VIASITE_ENUM_081")
RANKS = range(4)
SEPARATION = 500000
...
def build_prefix(tag, allowed):
    ...
    qb.save(pcb)
    return pcb, removed, pair, routes, x1, x0
...
def main():
    ...
    baseline, _ = RU.drc(IR.AUTH, "u3x8enum_base", SCRATCH)
    base = pcbnew.LoadBoard(IR.AUTH)
    boundary, _ = D362.boundary(base); allowed = boundary & D362.copper(base)
    seed, removed, pair, routes, x1, x0 = build_prefix("seed", allowed)
    ...
    for inner in ("I2", "I3"):
        for endpoint, pad, other, near in (("R59.1",pa,pb,"F"),("U3.13",pb,pa,"B")):
            for rank in RANKS:
        ...
        for order in (("R59.1","U3.13"),("U3.13","R59.1")):
            for first_rank in RANKS:
                for second_rank in RANKS:
                    trial = QR.QBoard(seed); IR.inject_existing_via_obstacles(trial)
                    spec = {"R59.1":(pa,pb,"F"), "U3.13":(pb,pa,"B")}
                    p,o,n = spec[order[0]]
                    ...
                    key = (inner, reserved["R59.1"], reserved["U3.13"])
                    ...
                    attempts.append({"inner":inner,"order":list(order),
                        ...
                        "distinct_pair":bool(is_new_pair),
                        "join":join})

    prefix_drc, details = RU.drc(seed, "u3x8enum_prefix", SCRATCH)
    ...
    ev = {"schema_version":1,"decision":"D-379","source_decision":"D-378",
          "authoritative_board_sha256":auth_sha,
          "authoritative_unchanged":hashlib.sha256(open(IR.AUTH,"rb").read()).hexdigest()==auth_sha,
          "method":"D377_prefix_XGPIO8_independent_explicit_viasite_enumeration",
          ...
          "xgpio1_ranked_route":x1,"xgpio0_route":x0,"prefix_ok":prefix_ok,
          "isolated_endpoint_attempts":isolated,"endpoint_sites":endpoint_sites,
          "ordered_pair_attempt_count":len(attempts),"distinct_via_pairs":len(distinct),
          "route_wins":len(wins),"attempts":attempts,"promotion_candidate":False,
          "conclusion":"XGPIO8_distinct_viasite_route_found_needs_transaction_replay" if wins else
                       "XGPIO8_explicit_viasite_enumeration_characterized"}
