# -*- coding: utf-8 -*-
"""D-381: extend the D-380 U3 transaction through complete XGPIO9 replay.

Scratch only. Rebuild the seven selected replacement routes, then replace the
complete XGPIO9 branch independently on In2 and In3. Restore later branches
only after XGPIO9 closes. The authoritative PCB is never edited.
"""
...
OUT = os.path.join(SP, "u3_xgpio9_inner_replay_083.json")
SCRATCH = os.path.join(SP, "w", "U3_XGPIO9_INNER_REPLAY_083")
...
def route_xgpio9(qb, inner):
    group = dict(IR.GROUPS["XGPIO9"])
    group["inner_long_haul_plan"] = {
        "a":"R60.1", "b":"U3.14", "a_near":"F", "b_near":"B",
        "inner":[inner],
    }
...
def main():
    ...
    baseline, _ = RU.drc(IR.AUTH,"u3x9inner_base",SCRATCH)
    ...
    attempts = []
    for inner in ("I2", "I3"):
        pcb, removed = D367.prepare("x9_"+inner.lower(), allowed, (0.0,-0.5))
        ...
        x9 = route_xgpio9(qb,inner) if x8["ok"] else {"ok":False,"reason":"XGPIO8_FAILED"}
        qb.save(pcb)
        wanted = restored = collections.Counter(); attachments = []
        if x9["ok"]:
            wanted, restored = restore_later(pcb,allowed)
            qb = QR.QBoard(pcb); IR.inject_existing_via_obstacles(qb)
            for branch in D364.SCHEDULE[8:]:
            ...
            if len(attachments)==len(D364.SCHEDULE)-8 and all(x["ok"] for x in attachments):
        ...
        drc, details = RU.drc(pcb,"u3x9inner_"+inner.lower(),SCRATCH)
        ...
        closed = (removed==allowed and len(pair)==2 and all(x["ok"] for x in pair)
                  ...
                  and x0["ok"] and x8["ok"] and x9["ok"] and restored==wanted
                  and len(attachments)==len(D364.SCHEDULE)-7 and all(x["ok"] for x in attachments)
        ...
        row = {"xgpio9_inner_layer":inner,"pair_routes":pair,"prefix_routes":prefix,
               "xgpio1_ranked_route":x1,"xgpio0_route":x0,"xgpio8_ranked_route":x8,
               "xgpio9_route":x9,"remaining_restored_items":sum(restored.values()),
        ...
        attempts.append(row)
        print(inner,"x9",x9["ok"],"attachments",len(attachments),
    ...
    ev = {"schema_version":1,"decision":"D-381","source_decision":"D-380",
          "authoritative_board_sha256":auth_sha,
          "authoritative_unchanged":hashlib.sha256(open(IR.AUTH,"rb").read()).hexdigest()==auth_sha,
          "method":"D380_prefix_then_complete_XGPIO9_inner_branch_replacement",
          ...
          "xgpio9_mechanism_wins":sum(1 for x in attempts if x["xgpio9_route"]["ok"]),
          "transaction_candidates":len(wins),"promotion_candidate":False,
          "conclusion":"closed_U3_transaction_candidate_needs_independent_gate" if wins else "XGPIO9_inner_replacement_characterized"}
    with open(OUT,"w",encoding="utf-8") as f: json.dump(ev,f,indent=2,sort_keys=True)
    print("RESULT",ev["conclusion"],"x9 wins",ev["xgpio9_mechanism_wins"],
