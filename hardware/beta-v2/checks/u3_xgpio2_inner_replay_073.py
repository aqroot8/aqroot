# -*- coding: utf-8 -*-
"""D-371: extend complete-branch inner replay through XGPIO2.

Scratch only.  At D-367's selected U3/R58 layout, reserve XGPIO6/XGPIO7,
replace XGPIO5 on In3 and XGPIO4 on In2 using the mechanisms proven by D-369
and D-370, then replace the complete XGPIO2 branch with the same qualified
native-face/inner-haul mechanism.  In2 and In3 are screened independently for
XGPIO2.  Remaining accepted U3 branches are restored exactly and attached only
after all three replacements close.  The authoritative PCB is never edited.
"""
...
import u3_xgpio4_inner_replay_072 as D370
import u3_xgpio6_replay_065 as D363

OUT = os.path.join(SP, "u3_xgpio2_inner_replay_073.json")
SCRATCH = os.path.join(SP, "w", "U3_XGPIO2_INNER_REPLAY_073")
X2_LAYERS = ("I2", "I3")


def restore_except_replaced(pcb, allowed):
    wanted = collections.Counter({s: n for s, n in allowed.items()
                                  if s[1] not in ("/XGPIO2", "/XGPIO4", "/XGPIO5")})
...
def main():
    ...
    baseline, _ = RU.drc(IR.AUTH, "u3x2inner_base", SCRATCH)
    boundaries = {net: sum(n for s, n in allowed.items() if s[1] == net)
                  for net in ("/XGPIO2", "/XGPIO4", "/XGPIO5")}
    attempts = []
    for x2_inner in X2_LAYERS:
        pcb, removed = D367.prepare("x2_" + x2_inner.lower(), allowed, (0.0, -0.5))
        ...
        x5 = D370.route_inner(qb, "XGPIO5_INNER", "I3") if pair_ok else {"ok": False, "error": "pair_not_reserved"}
        x4 = D370.route_inner(qb, "XGPIO4_INNER", "I2") if x5["ok"] else {"ok": False, "error": "xgpio5_not_closed"}
        x2 = route_inner(qb, "XGPIO2_INNER_PILOT", x2_inner) if x4["ok"] else {"ok": False, "error": "xgpio4_not_closed"}
        qb.save(pcb)
        wanted = restored = collections.Counter(); attachments = []
        if x2["ok"]:
            wanted, restored = restore_except_replaced(pcb, allowed)
            qb = QR.QBoard(pcb); IR.inject_existing_via_obstacles(qb)
            for net in D364.SCHEDULE[3:]:
                attachments.append(D364.attach_terminal(qb, net, branches[net]["u3_pad"],
                                                         branches[net]["width"]))
                if not attachments[-1]["ok"]: break
            if len(attachments) == len(D364.SCHEDULE)-3 and all(x["ok"] for x in attachments):
        ...
        drc, details = RU.drc(pcb, "u3x2inner_"+x2_inner.lower(), SCRATCH)
        ...
        closed = (removed == allowed and x5["ok"] and x4["ok"] and x2["ok"]
                  and restored == wanted and len(attachments) == len(D364.SCHEDULE)-2
        ...
        row = {"xgpio2_inner_layer": x2_inner, "xgpio4_inner_layer": "I2",
               "xgpio5_inner_layer": "I3", "pair_routes": pair,
               "xgpio5_route": x5, "xgpio4_route": x4, "xgpio2_route": x2,
        ...
        attempts.append(row)
        print(x2_inner, "x5", x5["ok"], "x4", x4["ok"], "x2", x2["ok"],
    ...
    ev = {"schema_version":1, "decision":"D-371", "source_decision":"D-370",
          "authoritative_board_sha256":auth_sha,
          "authoritative_unchanged":hashlib.sha256(open(IR.AUTH,"rb").read()).hexdigest()==auth_sha,
          "method":"complete_XGPIO5_In3_XGPIO4_In2_then_complete_XGPIO2_inner_branch_replacement",
          ...
          "xgpio2_mechanism_wins":sum(1 for x in attempts if x["xgpio2_route"]["ok"]),
          "transaction_candidates":len(wins), "promotion_candidate":False,
          "conclusion":"closed_U3_transaction_candidate" if wins else "XGPIO2_inner_replacement_characterized"}
    with open(OUT,"w",encoding="utf-8") as f: json.dump(ev,f,indent=2,sort_keys=True)
    print("RESULT",ev["conclusion"],"x2 wins",ev["xgpio2_mechanism_wins"],
