...
## Current design state (2026-08-23, after FBV2-S1-001)

* Digital pin architecture: LOCKED to Beta Pin Map v0.2.4
* Schematic capture: **`01_POWER_TREE` CAPTURED for Full Beta v2. Sheets 02-09 are inherited
  from Beta-DM, unmodified, and still carry the Beta-DM architecture**
* ERC: **RUN.** 55 violations, all inherited from Beta-DM; **zero introduced by FBV2-S1**
  (`reports/FBV2-S1-erc.rpt`). This is not "ERC clean" — see FBV2-S2
...
| Sheet | Status | Notes |
|---|---|---|
| `01_POWER_TREE` | **CAPTURED — FBV2-S1 POWER-TREE IMPLEMENTATION** | 136 parts, all with footprints. Reverse protection (P2) + LTC4368-1, autonomous dead-cell recovery, ACC_3V3 / ACC_5V accessory power, NFC 3V3-FIT / 5V-DNP select, VBUS_PRESENT and fault telemetry, 19 test points |
| `02_MCU_CORE` | **CAPTURED — INHERITED, BETA-DM ARCHITECTURE** | ESP32-S3-WROOM-1, EN/BOOT networks, strap test points |
| `03_SPI_A_DISPLAY_SD` | **CAPTURED — INHERITED, BETA-DM ARCHITECTURE** | display placeholder + microSD, both CS pull-ups. v2 display (`ER-TFT035IPS-6` + Hirose `FH69-50S-0.5SH`) **not yet migrated** |
| `04_SPI_B_RADIOS_NFC` | **CAPTURED — INHERITED, BETA-DM ARCHITECTURE** | CC1101 / SX1262 / ST25R3916 placeholders, 3 CS pull-ups. **`U9` VDD/VDD_TX still sit on `NFC_5V_PA_PENDING`, not on `NFC_SUPPLY`** |
...
sheet carries the Full Beta v2 architecture. Only `01_POWER_TREE` does.
...
| ERC | **RUN 2026-08-23** — `reports/FBV2-S1-erc.rpt`, 55 violations, all inherited, zero introduced by FBV2-S1 |
| Footprints assigned | **271 of 272 components.** Assigned is not verified — the per-footprint pad-overlap audit against vendor drawings is **FBV2-S2** and has not run |
