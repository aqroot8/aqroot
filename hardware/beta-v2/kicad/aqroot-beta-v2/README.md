...
## Current design state (2026-08-23, after FBV2-S1-007)

* Digital pin architecture: LOCKED to Beta Pin Map v0.2.4
* Schematic capture: **`01_POWER_TREE`, `02_MCU_CORE`, `03_SPI_A_DISPLAY_SD`,
  `04_SPI_B_RADIOS_NFC`, `05_I2C_DEVICES`, `06_AUDIO` and `07_IR` carry the Full Beta v2
  architecture. Sheets 08-09 are inherited from Beta-DM, unmodified, and still carry the
  Beta-DM architecture**
* ERC: **RUN.** **2 errors and 43 warnings, 45 messages total**
  (`reports/FBV2-S1-007-erc.rpt`) against a Beta-DM baseline of 5 errors / 53 warnings.
...
| `06_AUDIO` | **CAPTURED — FBV2-S1 AUDIO MIGRATION** | A3 sheet. **`U5` and `J6` arrived DNP — the speaker output path had never been built — and are now FITTED** (D-144). **`MK1` = PUI `DMM-4026-B-I2S-R`, replacing the obsolete ICS-43434: SEVEN pads, not six, so a new symbol and footprint were built from the manufacturer drawing** (D-145); `CONFIG`→GND mandatory; **no 1.8 V rail needed**; **`R120` 100 kΩ on `I2S_MIC_DIN` is a data-sheet requirement**. **Bus rate is set by the microphone: 48 kHz × 64 = 3.072 MHz, decimate to 16 kHz in firmware** — 16 kHz on the wire is out of spec (D-146). `U5` = **`MAX98357AETE+T`**, `GAIN_SLOT` → VDD = **6 dB** because 12 dB clipped the top 6.8 dB of digital range against the 3.3 V rail (D-147). `LS1` = **PUI `AS02008MR-LW152-R`**, Ø20 × 3 mm, 8 Ω, 500–4000 Hz, AWG #32 leads crimping straight into `J6` (D-148). EMI recovery `R121`/`R122` 0 Ω FITTED + `C81`/`C82` 1 nF DNP (D-150). `LS1` remains the one part with no footprint, by design (off-board speaker) |
| `07_IR` | **CAPTURED — FBV2-S1 INFRARED MIGRATION** | A3 sheet. **Eight parts arrived DNP — the whole subsystem — and are now FITTED** (D-153), the fourth consecutive sheet with a load-bearing inherited DNP. `D1` = **Vishay `TSAL6100`** (940 nm, `Ie` 170 mW/sr, ±10°); **TSAL6200 is a proven drop-in fallback — identical package, `VF` and `IFM`, so `R24` is unchanged** (D-154). **Peak 150 mA = 75 % of the `IFM` 200 mA REPETITIVE rating**; `IFSM` 1.5 A is a single-pulse ≤ 5 µs surge and is not a carrier rating; 300 mA rejected as out of spec (D-155). **Supply `+3V3`, not `SYS`** — 118–170 mA regulated against 64–166 mA on `SYS` (D-156). `R24` 18 Ω → **12 Ω** plus `R123` DNP parallel trim, **never below 10 Ω** (D-157). `C12` 4.7 µF → **22 µF 1210** — 4.7 µF gave 218 mV of carrier ripple (D-158). **`Q1` AO3400A pinout CONFIRMED 1 = G / 2 = S / 3 = D; the "needs the official AOS land pattern" blocker is CLOSED because AOS publishes none** (D-159). `U6` = **`TSOP38438`**, a pure MPN change, and the inherited `R21`/`C11` filter is **quantified at 41 dB at 38 kHz — ~90× margin against the Fig. 7 knee, and it is what makes sharing `+3V3` safe** (D-160). `TP39`/`TP40` added. **O-5 open: Vishay marks AGC4 "No" for Sony code** — receive-only, and the `TSOP38238` symbol is retained so reverting is a `lib_id` change | TSOP38238 RX + TSAL6200 low-side NMOS TX |
