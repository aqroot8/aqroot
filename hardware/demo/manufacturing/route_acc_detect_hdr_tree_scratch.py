#!/usr/bin/env python3
"""Route and fully gate the retained Community Port Accessory Detect tree."""
...
LEGS = ("ACC_DETECT_HDR_TVS_RESISTOR", "ACC_DETECT_HDR_RESISTOR_TESTPOINT",
...
def run_case(work, index, legs, baseline):
    ...
    promotion = (len(routes) == 3 and all(r["result"].get("ok") for r in routes)
                 and row["open_edges"] == 0 and not attributable and not removed
                 and not wrong)
    return {"order": legs, "routes": routes, "drc_exit": checked.returncode,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-acc-detect-preflight-") as temporary:
        ledger_path = Path(temporary) / "ledger.json"
        subprocess.run([sys.executable, str(LEDGER), "--board", str(BOARD),
                        str(ledger_path)], check=True, stdout=subprocess.DEVNULL)
        row = next(r for r in json.loads(ledger_path.read_text())["nets"] if r["net"] == NET)
        if row["open_edges"] == 0:
            print(json.dumps({"schema": 1, "preflight_result": "REFUSED_CONNECTED_TARGET",
                              "target_open_edges": 0, "cases_tested": 0}, indent=2))
            return 0
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-acc-detect-") as temporary:
        cases = [run_case(Path(temporary), i, legs, baseline)
                 for i, legs in enumerate(itertools.permutations(LEGS), 1)]
