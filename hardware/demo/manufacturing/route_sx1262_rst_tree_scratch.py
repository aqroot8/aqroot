#!/usr/bin/env python3
"""Atomically route and gate the fitted SX1262 reset tree."""
...
NET = "/SX1262_RST_N"
LEGS = ("SX1262_RST_EXPANDER_PULLDOWN", "SX1262_RST_PULLDOWN_RADIO")
...
def coalesce_target_vias(path):
    board = pcbnew.LoadBoard(str(path)); seen = set(); removed = 0
    for item in list(board.GetTracks()):
        if item.GetClass() != "PCB_VIA" or item.GetNetname() != NET: continue
        p = item.GetPosition()
        key = (p.x, p.y, item.GetWidth(pcbnew.F_Cu), item.GetDrillValue(), item.TopLayer(), item.BottomLayer())
        if key in seen: board.Remove(item); removed += 1
        else: seen.add(key)
...
def run_case(work, order, baseline):
    ...
    coalesced = coalesce_target_vias(scratch)
    ...
    return {"order": order, "routes": routes, "coalesced_exact_vias": coalesced,
            "drc_types": dict(types), "attributable_drc_count": attributable,
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-sx1262-rst-") as td:
