#!/usr/bin/env python3
"""Screen one-complete-net withdrawals around boxed SYS land C26.2."""
...
BOARD, WALL, RADIUS = landings.BOARD, "C26.2", 5_000_000
...
def inventory(path):
    ...
    fp = board.FindFootprintByReference("C26")
    origin = next(p.GetPosition() for p in fp.Pads() if p.GetNumber() == "2")
    rows = {}
...
def main():
    ...
    if args.prepare:
        ...
        return 0
    before, nearby, cases = sha256(BOARD), inventory(BOARD), []
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-sys-c26-pocket-") as td:
