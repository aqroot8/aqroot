#!/usr/bin/env python3
"""Build and gate the two NFC transmitter-output arms as one transaction."""
...
# RFO2 owns the only launch corridor past U9.15.  Reserving it first leaves a
# legal RFO1 path; the opposite order boxes U9.15 with RFO1 copper.
NAMES = ("NFC_RFO2", "NFC_RFO1")
NETS = (
...
def main() -> int:
    ...
    before = sha256(BOARD)
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-nfc-rfo-") as temporary:
        work = Path(temporary)
        ...
        report = {
            ...
            "tactic": "atomic symmetric NFC RFO outer-layer, no-via pair",
            "routes": routes,
            "lengths_mm": lengths,
