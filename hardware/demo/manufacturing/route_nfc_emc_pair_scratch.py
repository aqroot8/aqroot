#!/usr/bin/env python3
"""Build and gate the symmetric four-pad NFC EMC nodes as one transaction."""
...
ORDERS = {
    "a-first": ("NFC_EMCA_L", "NFC_EMCA_SHUNT", "NFC_EMCA_CAP",
                "NFC_EMCB_L", "NFC_EMCB_SHUNT", "NFC_EMCB_CAP"),
    "b-first": ("NFC_EMCB_L", "NFC_EMCB_SHUNT", "NFC_EMCB_CAP",
                "NFC_EMCA_L", "NFC_EMCA_SHUNT", "NFC_EMCA_CAP"),
}
NETS = ("/04_SPI_B_RADIOS_NFC/NFC_EMCA", "/04_SPI_B_RADIOS_NFC/NFC_EMCB")
    ...
    for item in board.GetTracks():
        if item.GetClass() == "PCB_VIA":
            ...
            result[(item.GetNetname(), "VIA", item.GetWidth(pcbnew.F_Cu),
                    item.GetDrillValue(), position.x, position.y)] += 1
            continue
...
def main():
    ...
    baseline = pcbnew.LoadBoard(str(BOARD))
    baseline_copper = copper(baseline)
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-nfc-emc-") as temporary:
        work = Path(temporary)
        ...
        after = pcbnew.LoadBoard(str(scratch))
        after_copper = copper(after)
        removed = baseline_copper - after_copper
        added = after_copper - baseline_copper
        wrong_net_additions = [list(item) for item in added if item[0] not in NETS]
        ...
        target_rows = {r["net"]: r for r in ledger["nets"] if r["net"] in NETS}
        promotion = (all(r["result"].get("ok") for r in routes)
                     and all(target_rows[n]["open_edges"] == 0 for n in NETS)
                     and not attributable and not removed and not wrong_net_additions)
        ...
        report = {
            ...
            "tactic": "atomic symmetric NFC EMC B.Cu/no-via trees",
            ...
            "wrong_net_additions": wrong_net_additions,
            "target_open_edges": {n: target_rows[n]["open_edges"] for n in NETS},
