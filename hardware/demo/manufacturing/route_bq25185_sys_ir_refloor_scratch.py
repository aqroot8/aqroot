...
def timeout_tail(value):
    """Normalize TimeoutExpired output across Python subprocess variants."""
...
def replay_sw9_a(path):
    """Replay the complete accepted hard-power-switch tree exactly."""
    source = pcbnew.LoadBoard(str(BOARD)); board = pcbnew.LoadBoard(str(path))
    net = board.FindNet(SW9_A_NET); objects = 0
    for item in source.GetTracks():
        if item.GetNetname() != SW9_A_NET:
            continue
        if item.GetClass() == "PCB_VIA":
            clone = pcbnew.PCB_VIA(board)
            clone.SetPosition(item.GetPosition()); clone.SetWidth(item.GetWidth())
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-sys-ir-refloor-") as td:
        ...
        scans = {}
        for pad in ("C26.2", "C27.1", "C28.1", "L4.1", "U12.10"):
            scan_board = seed
            if pad == "L4.1":
        ...
        cases = [(a, b, c, d, e) for a in scans["C26.2"]
                 for b in scans["C27.1"] for c in scans["C28.1"]
                 for d in scans["L4.1"] for e in scans["U12.10"]]
        stop = min(len(cases), args.case_start + args.case_limit)
        for index in range(args.case_start, stop):
            (c26_candidate, c27_candidate, c28_candidate,
             l4_candidate, u12_10_candidate) = cases[index]
            ...
            try:
                ...
                rows.append({"case": index, "reservation_order": "u12_10_before_l4",
                             "route_timeout_seconds": args.case_timeout,
                             "last_stdout": timeout_tail(exc.stdout),
                             "last_stderr": timeout_tail(exc.stderr)})
                continue
            ...
            if sys_complete:
                ...
                replay.append({"leg": "SW9_A", "result": replay_sw9_a(scratch)})
                for leg in REPLAY:
                    run = subprocess.run([sys.executable, str(LOCAL), leg, "--route", str(scratch)], text=True, capture_output=True)
                    record = last_json(run.stdout); replay.append({"leg": leg, **record})
                    if run.returncode or not record["result"].get("ok"): break
            complete = sys_complete and len(replay) == len(REPLAY) + 3 and all(row["result"].get("ok") for row in replay)
            ...
            if complete:
                ...
                ledger = json.loads(ledger_path.read_text()); opens = {row["net"]: row["open_edges"] for row in ledger["nets"] if row["net"] in (sysroute.NET, IR_NET, ILIM_NET, L1_NET, NFC_ANT_B_NET, SW9_A_NET)}
            after = measured_copper(scratch); removed = baseline-after; added = after-baseline
            refloor_nets = (IR_NET, ILIM_NET, L1_NET, NFC_ANT_B_NET, SW9_A_NET)
            ...
            row = {"case": index, "reservation_order": "u12_10_before_l4", "c26_candidate": c26_candidate, "c27_candidate": c27_candidate, "c28_candidate": c28_candidate, "l4_candidate": l4_candidate, "u12_10_candidate": u12_10_candidate, "sys_complete": sys_complete, "sys_route": routed, "refloor_replay": replay, "open_edges": opens, "drc_exit": drc_exit, "drc_types": dict(types), "attributable_drc_count": len(attributable), "removed_wrong_net_items": wrong_removed, "added_wrong_net_items": wrong_added, "promotion_candidate": ok, "path": scratch}
            rows.append(row)
