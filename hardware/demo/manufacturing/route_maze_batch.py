...
DRU_CLASS = {
    ...
    "SYS_MAIN":     dict(width=500000, clr=250000, layers=None),
    "ACC_3V3":      dict(width=350000, clr=250000, layers=None),
    "ACC_5V":       dict(width=400000, clr=250000, layers=None),
    "VBUS_CHG":     dict(width=350000, clr=250000, layers=None),
    "NFC_5V_PA":    dict(width=350000, clr=250000, layers=None),
    "P3V3":         dict(width=400000, clr=200000, layers=None),
    "BAT_MAIN":     dict(width=600000, clr=300000, layers=None),
...
# --------------------------------------------------------------------------- #
def propose(path, nets, grid, via_cost_mm):
    import pcbnew
...
def gate(nets, grid, via_cost_mm, workdir, promote=False, candidate=None):
    ...
    routed = json.loads(subprocess.run(
        [sys.executable, __file__, "--propose", str(scratch),
         "--grid", str(grid), "--via-cost", str(via_cost_mm), *nets],
        check=True, text=True, capture_output=True).stdout)["results"]
    ...
    ok = (not attributable and inherited_ok and not regressed and not removed
          and not foreign and edges_after < edges_before
...
def main():
    ...
    if a.propose:
        propose(a.propose, a.nets, a.grid, a.via_cost)
        return 0
    if not a.nets:
