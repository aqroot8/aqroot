#!/usr/bin/env python3
"""Route and fully gate the five-land USB-C shield tree atomically."""
...
NET = "Net-(J3-SHIELD)"
PADS = ("J3.SH@38.680,141.625", "J3.SH@38.680,145.805",
...
def pad_key(pad):
    ...
    if ref != "J3.SH":
        return ref
    return f'{ref}@{pad["x"] / 1e6:.3f},{pad["y"] / 1e6:.3f}'
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-usb-shield-") as temporary:
        work = Path(temporary)
        ...
        pads = {pad_key(p): p for p in ir.physical_net_pads(board, NET)}
        if set(pads) != set(PADS):
            raise RuntimeError(f"unexpected fitted pads: {sorted(pads)}")
        routes = []
        for a, b, layer in BRANCHES:
            if layer == "HOP":
                route = qr.connect_hop(
                    board, NET, pads[a], pads[b], 300_000, 200_000, 200_000,
                    near="F", far="B", G=25_000, fine=25_000,
                    via_dia=600_000, via_drill=300_000)
            else:
                route = qr.connect_role(board, NET, pads[a], pads[b], layer,
                                        300_000, 200_000, 200_000, G=25_000)
            routes.append(route)
            if not route.get("ok"):
                break
        board.save(scratch)
        ...
        geometry = []
        for item, count in sorted(added.items(), key=lambda entry: repr(entry[0])):
            geometry.extend([item] * count)
        ...
        report = {
            ...
            "added_vias": sum(count for item, count in added.items() if item[1] == "VIA"),
            "added_layers": dict(Counter(item[1] for item in geometry)),
