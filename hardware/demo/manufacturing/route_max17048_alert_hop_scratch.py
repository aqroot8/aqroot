#!/usr/bin/env python3
"""Build and gate the bounded MAX17048 alert two-via inner-layer hop."""
...
def main():
    ...
    parser.add_argument("--far", choices=("I2", "I3"), default="I3")
    parser.add_argument("--candidate", type=Path)
    parser.add_argument("--promote", action="store_true")
    args = parser.parse_args()
    before_sha = sha256(BOARD)
    baseline = copper(pcbnew.LoadBoard(str(BOARD)))
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-max17048-alert-") as temporary:
        work = Path(temporary)
        ...
        result = qr.connect_hop(
            board, NET, pads[PADS[0]], pads[PADS[1]],
            200_000, 200_000, 200_000, near="B", far=args.far,
        ...
        promotion = (result.get("ok", False) and row["open_edges"] == 0
        ...
        report = {
            ...
            "tactic": "MAX17048 alert B.Cu-to-inner two-via hop",
            "far_layer": args.far, "route": result,
