#!/usr/bin/env python3
"""Route and fully gate the four-land ST25R3916 VDD_RF supply tree."""
...
LEGS = ("NFC_VDD_RF_UPPER", "NFC_VDD_RF_LOWER", "NFC_VDD_RF_CAP_JOIN")
LEG_ORDERS = tuple(itertools.permutations(LEGS))
...
def main():
    ...
    parser.add_argument("--order", type=int, choices=range(len(LEG_ORDERS)), default=0)
    args = parser.parse_args()
    before = sha256(BOARD)
    baseline = copper(BOARD)
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-nfc-vdd-rf-") as tmp:
        scratch = Path(tmp) / "candidate.kicad_pcb"
        for suffix in (".kicad_pcb", ".kicad_dru", ".kicad_pro"):
            scratch.with_suffix(suffix).write_bytes(BOARD.with_suffix(suffix).read_bytes())
        routes = []
        for leg in LEG_ORDERS[args.order]:
            try:
                run = subprocess.run(
                    [sys.executable, str(LOCAL), leg, "--route", str(scratch)],
                    text=True, capture_output=True, check=True, timeout=20,
                )
                routes.append(json.loads(run.stdout))
            except subprocess.TimeoutExpired:
                routes.append({"name": leg, "result": {
                    "ok": False, "reason": "BOUNDED_SEARCH_TIMEOUT",
                    "seconds": 20,
                }})
        ...
        report = {
            ...
            "order": args.order, "leg_order": LEG_ORDERS[args.order],
