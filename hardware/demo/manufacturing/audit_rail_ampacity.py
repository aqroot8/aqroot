...
RAILS = (
    ...
    dict(name="BAT_PROTECTED_P", net="/01_POWER_TREE/BAT_PROTECTED_P",
         ...
         basis="D-793 / R12-05 + R12-08 re-based this from 2.60 A: the "
               ...
               "retention floor is 3.20 V, where the same full-internal + "
               "both-budgets envelope is 2.4902 A path-bound.  2.50 A is the "
               "routing/thermal design current with explicit margin.  "
               "IBAT_OCP remains a FAULT threshold, not a routing current, and "
               "demo_feature_contract F6 refuses this number if the envelope "
               "ever exceeds it.",
    ...
    dict(name="P3V3_MAIN", net="+3V3",
         ...
         basis="D-793 / R12-05 re-based this from 2.12 A.  Espressif's "
               ...
               "loss terms that model did not have -- the diode's forward "
               "voltage at the inductor PEAK rather than at the average, "
               "switch transition and Coss loss at the MAXIMUM published "
               "switching frequency, gate-drive loss, and the winding's core "
               "loss and hot DCR -- taking the converter input 211.58 -> "
               "233.13 mA at an implied 75.4 % efficiency.  U20's worst "
               "programmed limiter corner is 0.8049 A at the WIDEST published "
               "SLVSGP6A accuracy ratio (D790-A12 refuses the bracketed one "
               "as an estimate), with R97 moved 1.78 -> 1.87 kOhm so that the "
...
# D-791 / D790-A02.  WHAT `TREG` ACTUALLY REGULATES, COMPUTED RATHER THAN
# ASSERTED.
#
# D-790 said the charge regime "needs no bound, by design" because SLUSF65B
# 6.3.7.6 reduces the CHARGE current at `TREG`.  Round-10 is right that this
# does not universally cap SYS-load heat: with an adapter attached the BQ25185
# dissipates in TWO places, and `TREG` only touches one of them.
#
#   * THE INPUT FET carries the whole SYSTEM current from `VIN` to `SYS`.
#     SLUSF65B publishes `RON_IN` = 330 typ / 470 mOhm MAX at `VIN` = 5 V,
#     `IIN` = 1 A, and `VSYS_REG` = 4.5 V with `VBATREG` <= 4.3 V.  `TREG` does
#     NOT reduce the system load -- there is nothing in the device that can --
#     so this term is unregulated.
#   * THE CHARGE FET carries `ICHG` from `SYS` down to `VBAT`, and THAT is the
#     term `TREG` folds back.
#
# Both are computed below at the worst published corners, the regulated and
...
CHARGE_REGIME = dict(
    ...
    consequence="a longer charge in a warm enclosure, not an exceeded "
                "junction, FOR THE CHARGE CURRENT ALONE.  D-791 / D790-A02: "
                "TREG does NOT regulate the SYSTEM load, which crosses the "
                "same package through the INPUT FET while an adapter is "
                "attached, so `charge_regime_junction` below separates the "
                "two and asks whether the UNREGULATED half alone is inside "
                "TI's operating maximum.  The BATFET discharge regime has no "
                "equivalent regulation at all -- TREG reduces CHARGE current "
                "and there is none when no adapter is attached -- so that "
...
# ==========================================================================
# D-797 / D797-02.  THE JUNCTION-SAFE POWER IS NOW CELL-CONDITIONED (about
# ILIM_min x VBAT: a reachable SUPPLEMENT is absorbing and hot), so the grid is
# 50 mV wide wherever a firmware floor can be read off it, plus 2.86 V -- the
# lowest cell at which the BATFET can be CONNECTED (just above the lowest
# falling BUVLO trip) and so the one that bounds the universal figure -- and
# the two historical special points, 3.52 V and the 4.221 V regulation top.
REGIME_VBAT_GRID_V = tuple(sorted(
    {2.85, 2.86, 3.52, apm.BAT_RAW_MAX_V}
    | {round(2.90 + 0.05 * i, 3) for i in range(27)}))
...
def regime_domain():
    ...
    lo, hi, reconnect = apm.buvlo_band_V()
    return dict(
        vbat_grid_V=list(REGIME_VBAT_GRID_V),
        vbat_lower_bound_basis=(
            "VBUVLO 3.0 V TYP less a DECLARED 5 % = 2.85 V, the LOWEST "
            "falling trip.  D-796 / D796-08: at and under it the BATFET "
            "cannot supplement, so a load above what the input carries "
            "collapses SYS; nothing is solved as SUPPLEMENT there"),
        vbat_upper_bound_basis="BAT_RAW_MAX_V 4.221 V, VBATREG at its "
                               "+0.5 % accuracy",
        buvlo_band_V=dict(falling_trip_low=lo, falling_trip_high=hi,
                          rising_reconnect_high=reconnect),
        batfet_states_by_vbat={("%.3f" % v): list(apm.batfet_states_at(v))
                               for v in REGIME_VBAT_GRID_V},
        sweeps=[round(x * apm.CHARGER_BRANCH_THRESHOLD_SWEEP, 6)
                for x in REGIME_SWEEP_POINTS],
        histories=list(REGIME_HISTORIES),
        histories_with_the_batfet_open=[None],
        ...
        treg_low_C=round(apm.BQ25185["treg_typ_C"]
                         - apm.BQ25185["treg_declared_band_K"], 6),
        model_assumptions=apm.CHARGER_MODEL_ASSUMPTIONS)


def regime_row_key(cls_key, vbat, ilim, sweep, hist, amb, batfet="connected"):
    return "%s/vbat%.3f/batfet_%s/ilim_%s/sweep%+.3f/%s/amb%.0f" % (
        cls_key, vbat, batfet, ilim, sweep, hist or "none", amb)
...
DPPM_HELD_BRANCHES = ("ILIM", "VINDPM", "DPPM")


def _regime_point(cls, vbat, bf, ilim, sw, hist, r_sys, theta, tj_max,
                  treg_low, delivered_out_W, keep_evidence):
    """The rows of ONE (source, cell, BATFET, ILIM, sweep, history) point,
    one per ambient."""
    kw = dict(vbus_V=cls["vbus_V"], path_ohm=cls["path_ohm"],
              source_key=cls["key"], previous_mode=hist, sweep=sw,
              batfet=bf)

    def _st(p, zero=False):
        return apm.charger_state(p, vbat, ilim,
                                 treg_folds_charge_to_zero=zero, **kw)
    ...
    def _reachable(p):
        ...
        with this history, from a COLD START at the full program.

        If the full-program state is SUPPLEMENT -- entered by necessity from
        a cold start, or retained from a SUPPLEMENT history -- it is
        ABSORBING: no charge flows for TREG to fold and the actual SYS cannot
        reach the exit, so it is the state, at its STATIC junction.
        Otherwise TREG may fold the charge anywhere down to zero, and the
        zero-charge state is the one TREG cannot improve on -- carried even
        where the full program collapses SYS (below VBUVLO), because a charge
    ...
    carrying = carrying_zero = None
    if bf != "connected":
        # THE INPUT-CARRYING POWER.  With the BATFET open nothing supplements,
        # so the largest load the part holds WITH ITS CHARGE LOOP ACTIVE is
        # where SYS collapses (`p_nd`: no supplement exists to end it any
        # other way).  The zero-charge figure is carried beside it: that is
        # how far the junction bound has to reach, because TREG folding the
        # charge to zero is the one state that carries more.
        carrying = p_nd
        carrying_zero = _bisect_max(lambda p: _st(p, zero=True) is not None)
    out = []
    for amb in REGIME_AMBIENTS_C:
        def _tj(st):
            return apm.charger_junction(st, amb, r_sys, theta,
                                        delivered_out_W)[0]

        def _treg0_unreachable(p):
            z = _st(p, zero=True)
            return z is not None and _tj(z) < treg_low

        def _junction_ok(p):
            reach = _reachable(p)
            return reach is not None and all(_tj(x) <= tj_max
                                             for x in reach)

        p_t0 = _bisect_max(_treg0_unreachable)
        t0_at = _st(p_t0, zero=True)
        t0_above = _st(p_t0 * (1.0 + 1e-7) + 1e-7, zero=True)
        p_tj = _bisect_max(_junction_ok)
        _r_at = _reachable(p_tj) or [None]
        _r_above = _reachable(p_tj * (1.0 + 1e-7) + 1e-7) or [None]
        tj_at = max(_r_at, key=lambda x: -1e9 if x is None else _tj(x))
        tj_above = max(_r_above, key=lambda x: -1e9 if x is None else _tj(x))
        junction_limited = True
        junction_binding = ("junction" if tj_above is not None
                            else "no_operating_point_above")
        if p_tj >= REGIME_JUNCTION_UNBOUNDED_W:
            # the junction holds up to the top of the searched range
            junction_limited = False
            junction_binding = "domain_cap"
        elif carrying_zero is not None and \
                p_tj >= carrying_zero * (1.0 - 1e-9):
            # below VBUVLO the input-carrying limit arrives first: above it
            # SYS collapses, which dissipates nothing.  Not a junction limit.
            junction_limited = False
            junction_binding = "input_carrying_below_vbuvlo"
        junction_W = p_tj if junction_limited else REGIME_JUNCTION_UNBOUNDED_W
        # D-797 / D797-01: what lies just above a junction ceiling, named.  A
        # reachable absorbing SUPPLEMENT at or above TSHUT_RISING is the
        # device's own thermal shutdown cycling VSYS -- protection, not
        # regulation, and never counted as compliance with 125 C.
        _hot = tj_above
        above_regime = None
        if _hot is not None:
            _tj_hot = _tj(_hot)
            if _hot["mode"] in apm.SUPPLEMENTING_MODES:
                ...
                above_regime = "ZERO_CHARGE_ABOVE_TJ_MAX"
        nd_W = min(p_nd, p_t0)
        if carrying is not None and p_nd <= p_t0:
            nd_binding = "input_carrying_below_vbuvlo"
        elif p_nd <= p_t0:
            nd_binding = "supplement_onset"
        else:
            nd_binding = "treg_could_fold_the_charge_to_zero"
        ceiling = min(nd_W, junction_W)
        row = dict(
            key=regime_row_key(cls["key"], vbat, ilim, sw, hist, amb, bf),
            source_class=cls["key"], qualified=bool(cls["rules"]),
            vbat_V=vbat, batfet=bf, ilim_corner=ilim, sweep=sw,
            previous_mode=hist, ambient_C=amb,
            vbus_V=cls["vbus_V"], path_ohm=cls["path_ohm"],
            ilim_A=(apm.BQ25185["ilim_max_A"] if ilim == "max"
                    else apm.BQ25185["ilim_min_A"]),
            no_discharge_electrical_W=round(p_nd, 6),
            treg_zero_unreachable_W=round(p_t0, 6),
            no_discharge_W=round(nd_W, 6),
            no_discharge_binding=nd_binding,
            input_carrying_W=(None if carrying is None
                              else round(carrying, 6)),
            input_carrying_at_zero_charge_W=(
                None if carrying_zero is None else round(carrying_zero, 6)),
            junction_W=round(junction_W, 6),
            # D-797 / D797-03: the exact scalar the junction evidence is
            # solved at -- published even where the row is not junction-
            # limited, so every *_at / *_above state binds to a number.
            junction_boundary_W=round(p_tj, 6),
            junction_limited=junction_limited,
            junction_binding=junction_binding,
            ceiling_W=round(ceiling, 6),
            binding=("no_discharge" if nd_W <= junction_W else "junction"),
            mode_at_no_discharge_ceiling=(nd_at["mode"] if nd_at else None),
            junction_at_mode=(tj_at["mode"] if tj_at else None),
            junction_above_mode=(tj_above["mode"] if tj_above else None),
            junction_above_regime=(above_regime if junction_limited
                                   else None),
            junction_above_static_C=(round(_tj(tj_above), 3)
                                     if (tj_above is not None
                                         and junction_limited) else None))
        if keep_evidence:
            row["_evidence"] = dict(
                no_discharge_at=nd_at, no_discharge_above=nd_above,
                treg_zero_at=t0_at, treg_zero_above=t0_above,
                junction_at=tj_at, junction_above=tj_above,
                thermal=dict(ambient_C=amb, r_sys_K_per_W=r_sys,
                             theta_ja_C_per_W=theta,
                             delivered_out_W=delivered_out_W,
                             tj_max_C=tj_max, treg_low_C=treg_low))
        out.append(row)
    return out
...
def charge_regime_envelope(system=None, spec=None, delivered_out_W=0.0,
                           keep_evidence=True):
    """Every (source class, VBAT, BATFET state, ILIM corner, threshold sweep,
    history, ambient) point, and the limits at each.

    NO-DISCHARGE (D-796 / D796-02).  The largest system power at which the
    part is not in SUPPLEMENT (`no_discharge_electrical_W`) AND at which TREG
    cannot fold the charge to zero -- the zero-charge junction below TREG's
    declared LOW end at THIS ambient (`treg_zero_unreachable_W`).  SLUSF65B
    6.3.7.6 says that once TREG has reduced the charge to zero "the battery
    supplies the current needed by the SYS output", and the release does not
    choose a reading of that sentence (`CHARGER_MODEL_ASSUMPTIONS`): it
    publishes no-discharge only where the sentence cannot apply.  D-795 called
    the ceiling "independent of ambient and of TREG"; it is neither.

    BELOW VBUVLO (D796-08) the BATFET cannot supplement at all, so there is no
    supplement onset: the limit is the INPUT-CARRYING power, above which SYS
    collapses (`input_carrying_W`).  That is a brown-out, not a junction
    hazard, so it never lowers the junction figure.

    JUNCTION.  The largest system power at which the state TREG CANNOT bring
    to its threshold -- the zero-charge state -- is inside TI's 125 C
    operating maximum.  Wherever TREG can act the junction is held at the
    threshold (declared high end 110 C), and where no static TREG state
    exists (D-796 / R15-01, `treg_limit_cycle` in CHARGER_MODEL_ASSUMPTIONS)
    the loop cycles about it with the same time-average.  Evidence brackets
    each ceiling so the oracle re-checks it.
    """
    ...
    for cls in apm.USB_SOURCE_CLASSES:
        for vbat in REGIME_VBAT_GRID_V:
            ...
            tj_rows = [r["junction_W"] for r in sel if r["junction_limited"]]
            tj = min(tj_rows) if tj_rows else None
            carry = [r["input_carrying_W"] for r in sel
                     if r["input_carrying_W"] is not None]
            envelope.append(dict(
                source_class=cls["key"], qualified=bool(cls["rules"]),
                vbat_V=vbat, rows=len(sel),
                batfet_states=list(apm.batfet_states_at(vbat)),
                no_discharge_raw_W=round(nd, 6),
                junction_raw_W=(None if tj is None else round(tj, 6)),
                no_discharge_published_W=round(_floor(nd), 6),
                junction_published_W=(None if tj is None
                                      else round(_floor(tj), 6)),
                junction_limited=bool(any(r["junction_limited"]
                                          for r in sel)),
                input_carrying_raw_W=(round(min(carry), 6) if carry
                                      else None),
                input_carrying_published_W=(round(_floor(min(carry)), 6)
                                            if carry else None),
                no_discharge_bindings=sorted({r["no_discharge_binding"]
                                              for r in sel})))
    # ---- D-797 / D797-02.  THE CHARGING-SAFE POWER, BY CELL. -------------
    # At each grid cell, over the QUALIFIED (named-adapter) classes and every
    # other axis: the published junction figure (reachable states, absorbing
    # supplement included) and, under the BUVLO trip, the input-carrying
    # power above which SYS collapses (a reboot, not a junction limit, but
    # equally not an operating state).  `at_or_above` is the minimum over
    # every grid cell at or above this one: a charging cell only RISES while
    # the part is charging, so a state admitted at a cell must stay inside
    # the envelope at every higher one.
    _q_env = [e for e in envelope if e["qualified"]]
    by_cell = []
    for v in REGIME_VBAT_GRID_V:
        es = [e for e in _q_env if e["vbat_V"] == v]
        lims = []
        for e in es:
            if e["junction_published_W"] is not None:
                lims.append(("junction", e["junction_published_W"],
                             e["source_class"]))
            if e["input_carrying_published_W"] is not None:
                lims.append(("input_carrying_below_vbuvlo",
                             e["input_carrying_published_W"],
                             e["source_class"]))
        lim = min(lims, key=lambda x: x[1]) if lims else None
        by_cell.append(dict(
            vbat_V=v, batfet_states=list(apm.batfet_states_at(v)),
            charging_safe_published_W=(None if lim is None
                                       else round(lim[1], 6)),
            binding=(None if lim is None else lim[0]),
            ruling_source_class=(None if lim is None else lim[2])))
    for i, row in enumerate(by_cell):
        higher = [b["charging_safe_published_W"] for b in by_cell[i:]
                  if b["charging_safe_published_W"] is not None]
        row["charging_safe_at_or_above_published_W"] = (
            round(min(higher), 6) if higher else None)
    ...
    carry_q = [r["input_carrying_W"] for r in q
               if r["input_carrying_W"] is not None]
    return dict(
        domain=dom,
        rows=rows,
        ...
        charging_safe_by_cell=by_cell,
        charging_safe_universal_published_W=round(min(
            b["charging_safe_published_W"] for b in by_cell
            if b["charging_safe_published_W"] is not None), 6),
        input_carrying_below_vbuvlo_raw_W=(round(min(carry_q), 6)
                                           if carry_q else None),
        input_carrying_below_vbuvlo_published_W=(
            round(_floor(min(carry_q)), 6) if carry_q else None),
        guardband=REGIME_GUARDBAND, grid_W=REGIME_GRID_W,
        tj_operating_max_C=tj_max,
        model_assumptions=apm.CHARGER_MODEL_ASSUMPTIONS,
        what=("the no-discharge boundary is VBAT-, source- AND ambient-"
              "CONDITIONED (D-796: TREG must be unable to fold the charge to "
              "zero) and is published as a table; the junction-safe system "
              "power is universal over the domain wherever an operating "
              "point exists; below VBUVLO the input-carrying power bounds "
              "operation and is a brown-out, not a junction limit."))
...
def charge_regime_junction(system_W, ambient_C=None, spec=None, system=None,
    ...
    corners = {}
    # D-797 / D797-01: BOTH histories wherever the BATFET can be connected,
    # unless the caller pins one.  A retained supplement is a physical state
    # the part can be in, and it is the hot one.
    hist_set = ((previous_mode,) if previous_mode is not None
                else REGIME_HISTORIES)
    for vbat, bf, prev in [(v, b, h) for v in grid
                           for b in apm.batfet_states_at(v)
                           for h in hist_set
                           if b == "connected" or h != "SUPPLEMENT"]:
        for ilim_corner in REGIME_ILIM_CORNERS:
            for cls in apm.USB_SOURCE_CLASSES:
                ckey = "vbat%.3f/%s/ilim_%s/%s__%s" % (
                    vbat, bf, ilim_corner, prev or "none", cls["key"])
                kw = dict(ilim_corner=ilim_corner, vbus_V=cls["vbus_V"],
                          path_ohm=cls["path_ohm"], source_key=cls["key"],
                          previous_mode=prev, sweep=sweep, batfet=bf)
                ...
                if st is not None and st["mode"] in apm.SUPPLEMENTING_MODES:
                    ...
                    zero = apm.charger_state(system_W, vbat,
                                             treg_folds_charge_to_zero=True,
                                             **kw)
                if st is None or zero is None:
                    corners[ckey] = dict(
                        vbat_V=vbat, source_key=cls["key"], batfet=bf,
                        previous_mode=prev,
                ...
                corners[ckey] = dict(
                    ...
                    # D-796 / R15-01: a limit cycle's hot phase is a static
                    # figure the die never settles at; the loop holds the
                    # time-average at TREG (CHARGER_MODEL_ASSUMPTIONS).
                    junction_C=(treg_high if st["thermal"]["regime"]
                                == "TREG_LIMIT_CYCLE_HOT_PHASE"
                                else st["thermal"]["junction_C"]),
                    junction_hot_phase_static_C=(
                        st["thermal"]["junction_C"] if st["thermal"]["regime"]
                        == "TREG_LIMIT_CYCLE_HOT_PHASE" else None),
                    thermal_regime=st["thermal"]["regime"],
    ...
    return dict(
        ...
        highest_vbat_without_supplement_on_every_qualified_class_V=(
            max((v for v in grid if not any(
                ...
                default=None)),
        tshut_protection_cycle_qualified_corners=sorted(
            k for k, v in corners.items() if v.get("source_rules")
            and v.get("thermal_regime") == "TSHUT_PROTECTION_CYCLE"),
        # D-796 / D796-08: below VBUVLO a load the input cannot carry has no
        # operating point -- SYS collapses.  Named, never silently dropped.
        sys_collapses_below_vbuvlo_at=sorted(
            k for k, v in corners.items() if v.get("source_rules")
            and v.get("no_operating_point") and v.get("batfet") == "uvlo_open"),
        no_operating_point_qualified_corners=sorted(
            k for k, v in corners.items() if v.get("source_rules")
            and v.get("no_operating_point")),
...
# D-791 / D790-A02 + D790-A03 REPLACED THE DECLARED REFERENCE STATE WITH A
# DERIVED CONCURRENCY TABLE, BECAUSE THE D-790 ONE WAS NOT REACHABLE.
#
# D-790 declared a single sustained reference state -- both published accessory
# budgets, the display at full brightness, BOTH radios transmitting and the
# audio amplifier at its capped level -- and computed its battery current with
# an IDEAL-SOURCE formula:
#
#     ((0.89378 + 0.400) x 3.223012 / 0.90 + 0.300 x 5.165 / 0.88) / 3.85
#
# which is 1.6608 A, rounded up to 1.70 A.  Round-10 is right on both counts.
#
#   * D790-A02: that formula is not the network the rest of this programme
#     uses.  `demo_feature_contract` F6 solves the SAME state through the live
#     copper, the BQ25185 BATFET and the SYS->U21 trunk and gets 1.9492 A on
#     the live resistances and 1.9653 A at the declared path ceilings.  Two
#     numbers for one state is one number too many.
#   * D790-A03: and neither of them closes the CELL side.  F6 starts AT
#     `BAT_PROTECTED_P`; F10 prices the pass pair separately; nothing asked
#     whether an attainable cell can hold that node at that current.  It
#     cannot: solved end to end from the cell through the PCM, the harness,
#     `F1`, the four `AO4800` channels and `R75`, this state has NO STABLE
#     OPERATING POINT at any cell voltage -- and at the ones nearest to
#     feasible the current is above the BQ25185's own `IBAT_OCP` minimum.
#
# So the state is no longer DECLARED.  `demo_feature_contract` F12 solves the
# complete cell-to-load network for each entry of `SUSTAINED_STATES` below and
# DERIVES, per state, the lowest cell open-circuit voltage at which it holds
# against every limit at once: the BQ25185's `IBAT_OCP` margin convention, the
# pass pair's own guaranteed conduction row, the BQ25185 junction maximum, the
# fitted pouch's published discharge window, and the existence of a stable
# operating point at all.  Nothing here is asserted; this table only names the
# MODES, which are observable and firmware-controllable, and itemises what each
# one draws.  The budget lines themselves are `demo_feature_contract`'s
# `P3V3_INTERNAL_BUDGET`, cited there.
#
# THE PUBLISHED ACCESSORY BUDGETS ARE UNCHANGED.  Every state below keeps
...
SUSTAINED_STATES = (
    dict(key="display_only", modes=(),
         what="display at full brightness, panel logic, expanders/IMU and the "
              "front RGB; no radio transmitting and the audio amplifier idle"),
    dict(key="display_audio", modes=("audio at the capped level",),
         what="the always-on set plus the audio amplifier at its capped level"),
    dict(key="display_subghz", modes=("sub-GHz TX",),
         what="the always-on set plus ONE sub-GHz radio transmitting"),
    dict(key="display_subghz_audio",
         modes=("sub-GHz TX", "audio at the capped level"),
         what="the always-on set plus one sub-GHz radio transmitting AND the "
              "audio amplifier at its capped level"),
    dict(key="display_wifi", modes=("Wi-Fi / BLE TX",),
         what="the always-on set plus the Wi-Fi/BLE radio transmitting"),
    dict(key="display_wifi_subghz",
         modes=("Wi-Fi / BLE TX", "sub-GHz TX"),
         what="the always-on set plus BOTH radios transmitting"),
    dict(key="d790_declared",
         modes=("Wi-Fi / BLE TX", "sub-GHz TX", "audio at the capped level"),
         what="D-790's declared reference state, retained as the NEGATIVE "
              "control that D790-A03 found unreachable"),
)
# The state D-791 PUBLISHES as the sustained reference.  F12 refuses it if the
# derivation does not support it, and refuses any state ABOVE it that the
# documents publish as supported.  It is a NAME, not a number.
SUSTAINED_REFERENCE_KEY = "display_subghz"
SUSTAINED_REFERENCE_STATE = dict(
    what="both published Community-Port budgets (400 mA on the 3.3 V rail, "
         ...
         "the panel logic, the expanders/IMU and the front RGB, and ONE "
         "sub-GHz radio transmitting -- held indefinitely",
    excluded="the NFC field, a microSD write and an IR burst are BURSTY and "
             "are carried as bounded-duty allowances rather than removed "
             "(D790-A02); a SECOND transmitting radio and the audio amplifier "
             "at its capped level are NOT in the sustained reference state "
             "and each raises the derived cell floor, which F12 prints",
    reference_key=SUSTAINED_REFERENCE_KEY,
    always_on_A=dict(SUSTAINED_ALWAYS_ON),
    optional_A=dict(SUSTAINED_OPTIONAL),
    acc_3v3_A=0.400, acc_5v_A=0.300,
    i_bat_is_derived="demo_feature_contract F12 solves it from the complete "
                     "cell-to-load network and this file asserts no value.  "
                     "D-790's 1.70 A came from an ideal-source formula that "
                     "disagreed with F6's own network by 0.25 A and did not "
                     "close the cell side at all (D790-A02 / D790-A03).")
# D-791 / D790-A02.  THE BURSTY LOADS ARE BOUNDED, NOT REMOVED.
#
# D-790 dropped the NFC field, a microSD write and an IR burst from the
# sustained state on the grounds that they are "bursty rather than continuous".
# Round-10 is right that a load is not gone because it is short: what makes it
# safe to leave out of a STEADY-STATE model is a bounded duty against a thermal
# time constant, and that bound has to be written down.  The enclosure's time
# constant is minutes; the BQ25185 junction's is seconds.  Each allowance below
...
DISCHARGE_SYSTEM = dict(
    vcell_V=3.85,
    # D-791 / D790-A02.  THIS IS A FALLBACK, NOT THE MODEL'S ANSWER.  D-790
    # ASSUMED the node sat at the firmware's dual-rail floor for every state;
    # D790-A02/A03 overturn that -- the node voltage is an OUTPUT of the
    # cell-to-load network and is 0.4 to 0.9 V below the cell, never 3.85 V
    # under a published load.  Every caller that has a network answer now
    # passes `v_node_V`; this constant is used only when one does not, where it
    # is conservative because a higher node charges the enclosure more power
    # for the same current.
    vcell_basis="FALLBACK ONLY (D-791 / D790-A02): the firmware's former "
                "dual-rail floor, used when a caller supplies no "
                "network-solved node voltage.  A HIGHER node voltage costs "
                "MORE pack power for the same current and is the conservative "
                "direction for the enclosure",
...
def upstream_loss_W(amps, pass_pair_ohm_per_channel=0.0, spec=None,
    ...
    series = upstream_series_ohm(pass_pair_ohm_per_channel, spec, channels)
    return amps * amps * series, series


def thermal_state(amps, v_node_V, delivered_out_W, upstream_series_ohm_value,
                  ambient_C=None, board_copper_W=0.0, ron_bat_ohm=None,
                  spec=None, system=None):
    """D-791 / D790-A02.  ONE THERMAL EVALUATION, FED BY THE REAL NETWORK.

    `v_node_V` is the potential the cell-to-load network produces at
    `BAT_PROTECTED_P`; `amps` is the battery current at that node;
    `delivered_out_W` is the power that LEAVES the enclosure through the
    Community Port, and `upstream_series_ohm_value` is the cell-to-node
    resistance whose loss stays INSIDE.  Pure.
    """
    spec = PACKAGE_JUNCTION if spec is None else spec
    system = SYSTEM_THERMAL if system is None else system
    ambient_C = AMBIENT_DESIGN_MAX_C if ambient_C is None else ambient_C
    ron = (RON_BAT_MAX_OHM * RON_BAT_VBAT_ALLOWANCE if ron_bat_ohm is None
           else ron_bat_ohm)
    r_sys = system_thermal_resistance_K_per_W(system)
    pack_W = v_node_V * amps
    upstream_W = amps * amps * upstream_series_ohm_value
    internal_W = pack_W - delivered_out_W + upstream_W
    air_C = ambient_C + r_sys * internal_W
    p_bat_W = amps * amps * ron
    tj_C = air_C + spec["theta_ja_C_per_W"] * (p_bat_W + board_copper_W)
    return dict(
        ambient_C=ambient_C, amps=round(amps, 6),
        v_node_V=round(v_node_V, 6),
        pack_W=round(pack_W, 6), delivered_out_W=round(delivered_out_W, 6),
        upstream_series_ohm=round(upstream_series_ohm_value, 6),
        upstream_loss_W=round(upstream_W, 6),
        internal_W=round(internal_W, 6),
        r_sys_K_per_W=round(r_sys, 4),
        enclosure_rise_K=round(r_sys * internal_W, 3),
        internal_air_C=round(air_C, 3),
        ron_bat_ohm=round(ron, 6), p_bat_W=round(p_bat_W, 6),
        tj_C=round(tj_C, 3),
        tj_limit_C=spec["tj_operating_max_C"],
        pouch_limit_C=POUCH_ADJACENT_LIMIT_C,
        junction_is_inside_the_operating_maximum=bool(
            tj_C <= spec["tj_operating_max_C"] + 1e-9),
        internal_air_is_inside_the_pouch_window=bool(
            air_C <= POUCH_ADJACENT_LIMIT_C + 1e-9),
        method="TJ = TA(external) + R_SYS x P_internal + thetaJA x PBAT, with "
               "P_internal = V_node x I - (what the Community Port hands to "
               "an accessory outside the right wall) + the cell-to-node "
               "upstream loss, which is under the same lid.  D-791 / "
               "D790-A02: V_node and I both come from the cell-to-load "
               "network rather than from an assumed node voltage.")


def internal_dissipation_W(amps, spec=None, accessory=True,
                           pass_pair_ohm_per_channel=0.0, v_node_V=None):
    spec = DISCHARGE_SYSTEM if spec is None else spec
    # D-791 / D790-A02: the node voltage is an INPUT when a caller has solved
    # the network, and the declared fallback only when it has not.
    node = spec["vcell_V"] if v_node_V is None else v_node_V
    pack = node * amps
...
def package_junction(amps, board_copper_W=0.0, ambient_C=None,
                     ...
                     accessory=True, charge_system_W=None):
    ...
    # A DECLARED fallback for the charge-regime system power when no caller has
    # solved it; `demo_feature_contract` F12 passes the reference state's own.
    if charge_system_W is None:
        charge_system_W = (DISCHARGE_SYSTEM["acc_3v3_A"]
                           * DISCHARGE_SYSTEM["acc_3v3_V"]
                           + DISCHARGE_SYSTEM["acc_5v_A"]
                           * DISCHARGE_SYSTEM["acc_5v_V"]) + 3.0
    r_sys = system_thermal_resistance_K_per_W(system)
    ...
    out.update(
        # D-791 / D790-A02 + D790-A03.  THE REFERENCE STATE IS NO LONGER A
        # NUMBER THIS FILE ASSERTS.  D-790 declared `i_bat_A = 1.70` from an
        # ideal-source formula and evaluated it here; `demo_feature_contract`
        # F12 now solves the COMPLETE cell-to-load network for every state in
        # `SUSTAINED_STATES` and derives the lowest cell voltage each one
        # holds at.  This file publishes the MODES and the current CEILING the
        # enclosure supports; F12 decides which states fit under it.
        sustained_reference_state=dict(
            SUSTAINED_REFERENCE_STATE,
            derived_by="demo_feature_contract F12 "
                       "(judge_cell_to_load / F12_cell_to_load_network)",
            bursty_allowances=[dict(b) for b in BURSTY_ALLOWANCES],
            bursty_time_averaged_A=BURSTY_TIME_AVERAGED_A),
        charge_regime_split=charge_regime_junction(
            charge_system_W,
            ambient_C=ambient_C,
            delivered_out_W=(DISCHARGE_SYSTEM["acc_3v3_A"]
                             * DISCHARGE_SYSTEM["acc_3v3_V"]
                             + DISCHARGE_SYSTEM["acc_5v_A"]
                             * DISCHARGE_SYSTEM["acc_5v_V"])),
        ...
        # D-791 / D790-A02.  THE VERDICT IS ABOUT THE ENVELOPE THIS FILE CAN
        # ANSWER FOR: the charge regime must be self-limiting in the half TREG
        # can reach, and a SUSTAINED CURRENT CEILING must exist at all.
        # Whether any given PRODUCT STATE fits under that ceiling is F12's
        # question, because only F12 closes the cell side; asking it here from
        # a declared current was exactly the D790-A02/A03 defect.
        ok=bool(out["charge_regime"]["is_self_limiting"]
                and lo > 0.0
                and sustained["tj_C"] <= spec["tj_operating_max_C"] + 1e-6
                and sustained["internal_air_C"]
                <= POUCH_ADJACENT_LIMIT_C + 1e-6),
...
# The fitted pouch's published CHARGE working range tops out at 40 C, which is
# a different and tighter limit from its 60 C discharge window -- D-791 /
# D790-A02 separates them because the charge regime is a different state.
POUCH_CHARGE_LIMIT_C = 40.0
