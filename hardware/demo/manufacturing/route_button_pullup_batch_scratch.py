#!/usr/bin/env python3
"""Route and fully gate the three equivalent U2/button pull-up branches."""
...
LEGS = ("BTN_DOWN_U2_PULLUP", "BTN_LEFT_U2_PULLUP", "BTN_A_U2_PULLUP")
NETS = (
    "/08_BUTTONS_EXPANDERS/BTN_DOWN_N",
    "/08_BUTTONS_EXPANDERS/BTN_LEFT_N",
    "/08_BUTTONS_EXPANDERS/BTN_A_N",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-button-pullups-") as tmp:
        ...
        ok = (len(routes) == len(LEGS) and all(r["result"].get("ok") for r in routes)
              and all(targets[n] == 2 for n in NETS) and not attributable
