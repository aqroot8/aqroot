#!/usr/bin/env python3
"""Atomically route and gate the fitted display backlight-return tree."""
...
LEGS = ("LED_K_DISPLAY_DRIVER", "LED_K_DRIVER_SENSE")
...
def bridge_display_lands(path):
    board = pcbnew.LoadBoard(str(path))
    net = board.FindNet(NET)
    pads = [p for fp in board.GetFootprints() if fp.GetReference() == "J1"
            for p in fp.Pads() if p.GetNumber() in ("2", "3")]
    if len(pads) != 2:
        raise RuntimeError(f"expected J1.2/J1.3, got {len(pads)}")
    a, b = sorted((p.GetPosition() for p in pads), key=lambda p: p.x)
    track = pcbnew.PCB_TRACK(board)
    track.SetNet(net); track.SetLayer(pcbnew.F_Cu); track.SetWidth(300_000)
    track.SetStart(a); track.SetEnd(b); board.Add(track)
    pcbnew.SaveBoard(str(path), board)


def enforce_current_width(path):
    """Widen every scratch LED_K segment before connectivity/DRC acceptance."""
    board = pcbnew.LoadBoard(str(path))
    for item in board.GetTracks():
        if item.GetClass() != "PCB_VIA" and item.GetNetname() == NET:
            item.SetWidth(300_000)
    pcbnew.SaveBoard(str(path), board)
...
def run_case(work, order, baseline):
    ...
    if len(routes) == len(LEGS) and all(r["result"].get("ok") for r in routes):
        bridge_display_lands(scratch)
        enforce_current_width(scratch)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aqroot-demo-led-k-tree-") as temporary:
        orders = list(itertools.permutations(LEGS))
        if args.order is not None:
            orders = [orders[args.order]]
