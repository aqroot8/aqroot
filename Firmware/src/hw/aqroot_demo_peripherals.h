...
// ---------------------------------------------------------------------------
// Backlight: moved to `aqroot_demo_backlight.h` at D-788 so the SHIPPED
// `backlightRamp()` -- its PWM duty writer and its microsecond hold, not a
// test's own fakes -- can be compiled and executed by
// `Firmware/test/test_production_timing.cpp`.  Included here so every existing
// caller of this header keeps seeing `aqroot::backlightRamp`.
// ---------------------------------------------------------------------------
