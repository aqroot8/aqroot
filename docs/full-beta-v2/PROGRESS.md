...
**FBV2-P2-003Z (2026-08-30) - D-300 GOVERNED CTO FAIL / LEVER REFUTATION + WIP RETIREMENT: the `AQROOT_LTCGATE` defer-to-congestion lever's full-authority gate COMPLETED and it is BEHAVIOURALLY IDENTICAL to D-299 (gained 0 / lost 0, same `LTC_GATE U18.10->Q3.4` terminal wall, same D-249 track_width / D-269 clearance rejections, identical final DRC) - so a pure re-order is a NULL OPERATION on this wall: the lever and its G15 WIP are REJECTED/RETIRED and the false-positive probe is RETIRED; copper is NOT promoted, readiness/progress unchanged, autonomy CONTINUES. Starting HEAD `3ce5244` (D-299; pushed; `phaseA_journal.json` restored at HEAD) carrying three uncommitted WIP files: the OFF-by-default `AQROOT_LTCGATE` defer-to-congestion lever in `checks/route_battery_block.py` (+34/-1), its G15 contract in `checks/router_regression.py` (+30), and the measured-record probe `checks/ltcgate_join_probe_003z.py` (untracked). THE LEVER (a pure re-order): `route_battery_block.py` removed the `LTC_GATE U18.10->Q3.4` branch from section `8b` (guarded on the flag AND the exact triple) and re-queued it as the LAST functional item (a `13z` stage after the closure stage and test points) at the ordinary SIG ladder; OFF -> byte-identical, join stays in 8b; pinned OFF/ON + scoped by G15. Theory (from the probe): 8b routes the join before the western margin fills, so `connect_role` takes a short central path that narrows to 0.20 mm inside the BPP 1.20 mm-trunk keep-region (D-249) and grazes a `BAT_MAIN` path at 0.2803 mm (~20 µm short of D-269); deferring to full congestion was supposed to force a clean west detour. THE GATE RAN AND COMPLETED: `AQROOT_U18BPP_JOIN=I3 AQROOT_U19CAP=1 AQROOT_LTCGATE=1 bash w/run_003t_full.sh 003z3_ltcgate w/cand_003t/t_a_r77e15n10_r79e15n10.json` -> `checks/w/phaseA_003t_full_003z3_ltcgate.json` (secs 1497.0, driver exited clean); the shared `phaseA_journal.json` was restored byte-identical to HEAD after and no process remains; judged by `python3 w/judge_003z.py w/phaseA_003t_full_003z3_ltcgate.json`. A genuine full-authority artifact (not a proxy - D-286). THE VERDICT - IDENTICAL TO D-299, THE LEVER DOES NOTHING: vs the 003Y2 baseline (D-299) connections 72=72, skipped 101=101, ratsnest 705/-76 = 705/-76, journal 75=75; the connected-set diff GAINED 0 / LOST 0. The failing rung is byte-for-byte the same two frozen owner rules - `track_width` (rule 'BAT_PROTECTED_P high-current trunk width - D-249' min 1.2000 mm; actual 0.2000 mm) and `clearance` (rule 'BAT_MAIN routed clearance - current path role - D-269' 0.3000 mm; actual 0.2803 mm); `LTC_GATE U18.10<->Q3.4` NOT in the connected set, still the terminal fail; final DRC histogram identical (`{hole_clearance:5, lib_footprint_issues:199, solder_mask_bridge:1, track_width:1, unconnected_items:499}`); no sub-0.50 non-fine via (run via diameters 0.35/0.60/0.65/0.80). Deferring the join to route last changed nothing - the driver's `connect_role` greedily re-takes the identical rule-violating central path even when the branch is queued as the very last functional item. A re-order is a null operation on this wall. THE PROBE WAS A FALSE POSITIVE (D-286 reaffirmed): `ltcgate_join_probe_003z.py` measured a legal 10.475 mm west detour (B.Cu, w=0.20, 0 new DRC, ratsnest 705->704) via a post-hoc `QR.connect_role` on the SAVED D-299 full-run board - but that does NOT reproduce the driver's real in-run state (reserved-lane keep-outs already lifted before the closure stage; a board that is not the post-hoc snapshot; the greedy path-cost model still offers and takes the shorter central rung because a re-order never physically blocks it). A post-hoc measurement on the final saved board is a proxy the full gate overrides - only a genuine full-authority Phase-A PASS promotes copper (D-286), and here it FAILs identically. RULING: REJECT/RETIRE the `AQROOT_LTCGATE` lever and its G15 WIP via an exact reverse patch scoped to the two tracked files (`git diff -- route_battery_block.py router_regression.py | git apply -R`, NOT a broad destructive reset); RETIRE the false-positive probe (untracked, never committed) so no artifact in the tree claims the lever works - the negative finding is recorded in the audit and CTO_DECISIONS instead. Copper NOT promoted; the authoritative board stays six layers / 0 tracks / 0 vias; readiness/progress UNCHANGED. NOT an owner decision (no floor relaxed, no frozen part moved, no DRU change, no D-249/D-269 relaxation); stop file ABSENT, autonomy CONTINUES. RETIREMENT PROOF: post-revert `git hash-object` = `HEAD:` blob for each file (`route_battery_block.py` `ebcafae…`, `router_regression.py` `38eb3a8…`); `git grep LTCGATE|AQROOT_LTCGATE|13z|ltcgate_join_probe` over tracked Python source (excluding gitignored `checks/w/`) NO match; probe removed. TESTS: `router_regression.py` ALL CHECKS PASS (G12/G13/G14 intact; G15 correctly gone). OPPORTUNITY & SIMPLIFICATION SCAN (mandated at this LTC_GATE / power-protection milestone): no product-capability / BOM / recoverability (D-049) / testability / manufacturing / firmware / UX / future-option opportunity justifies changing architecture - the wall is a single internal LTC4368 gate-drive control-net join (all other `LTC_GATE` segments connected), not a capability or BOM gap; no component change closes a 20 µm routing pinch; TP17 already covers testability; the bare inner In2/In3 remain spare future capacity (D-297 lesson). No irreversible cost / strategic fork. Open owner decisions: NONE. NEXT BEST TECHNICAL LEVER (grounded): the probe proved a legal ~10.5 mm west detour exists; the driver just won't take it while the shorter central lane is offered - so the next lever is path-shaping, not ordering: an explicit local waypoint / central-lane keep-out (on the proven `AQROOT_U19CAP` KO mechanism, lifted after the join) that BLOCKS the rule-violating central lane and forces `connect_role` onto the west detour clear of the BPP trunk (D-249) and the `BAT_MAIN` path (D-269, miss ~19.7 µm). Fallback: a bounded immediate-neighbor placement ECO (~20 µm nudge) re-screened with full-placement DRC (D-286). INTEGRITY: authoritative PCB byte-identical to HEAD (`sha256 2235e273…d642d7e`; six layers, 0 tracks, 0 vias, placement at home); no DRC absorbed; `phaseA_journal.json` at HEAD; no via below the D-257 ladder; D-269 (0.300 mm), ≥1.20 mm BPP (D-249), 0.60 mm BAT_MAIN ENFORCED; D-290 untouched; the accepted `AQROOT_U18BPP_JOIN` (D-297) and `AQROOT_U19CAP` (D-299, G14), `place_003l` (D-285), the D-275/D-288 bridge, D-275 and D-277..D-299 preserved; frozen `beta-full-reference-v1` untouched; gitignored 003Z evidence preserved (`checks/w/phaseA_003t_full_003z3_ltcgate.json`, `w/judge_003z.py`, `w/FULL003T_003z*_ltcgate/`, `w/TEST003Z_*/`, `w/run_003z_ltcgate.log`); `JLCPCB_READINESS` unchanged. NEXT FBV2-P2-004A: build ONE bounded, env-gated (OFF-by-default) path-shaping lever for `LTC_GATE U18.10->Q3.4` (a central-lane keep-out forcing the west detour - NOT a re-order, refuted here), keep `AQROOT_U18BPP_JOIN=I3`+`AQROOT_U19CAP=1` ON, validate vs `router_regression.py` (authoritative byte-identical), full-authority-gate judged (connected-set diff vs `w/phaseA_003t_full_003y2_u19cap.json` and `w/phaseA_003t_full_003w_u18bpp_i3.json`); the placement ECO is the fallback; no D-290 reauth, DRU/rule change, via below the D-257 ladder, D-249/D-269 relaxation, or topology/footprint/outline change; copper promoted only on a genuine full Phase-A PASS. Evidence: audit `audits/2026-08-30-p2-003z-d300-ltcgate-defer-to-congestion-lever-refuted-false-positive-probe-retired.md`. NO PROGRESS EARNED (no copper promoted): PCB routing 0 %, overall 74 %, readiness ~77 %.**

**FBV2-P2-003Y (2026-08-30) - D-299 GOVERNED CTO ACCEPT + COMMIT + overall-run FAIL + HANDOFF: the D-298 U19 CAPACITY lever's full-authority gate COMPLETED and it is a GENUINE +2 connected-set gain (NOT the D-296 swap) - so `AQROOT_U19CAP` is ACCEPTED and COMMITTED (banked env-gated/OFF-by-default, pinned by G14); but copper is NOT promoted because full Phase-A still FAILs, the terminal wall newly ADVANCING past the whole U19 field to `LTC_GATE U18.10->Q3.4` (candidate paths DRC-gate-rejected by the frozen D-249 BPP-trunk-width and D-269 BAT_MAIN-clearance rules), readiness/progress unchanged, autonomy CONTINUES. Starting HEAD `7a39430` (D-298; `phaseA_journal.json` at HEAD) carrying the uncommitted OFF-by-default `AQROOT_U19CAP` lever + G14 WIP. THE GATE RAN AND COMPLETED: the governing foreground run `AQROOT_U18BPP_JOIN=I3 AQROOT_U19CAP=1 bash w/run_003t_full.sh 003y2_u19cap w/cand_003t/t_a_r77e15n10_r79e15n10.json` -> `checks/w/phaseA_003t_full_003y2_u19cap.json` (secs 1463.2, driver exited clean); the SHARED `phaseA_journal.json` was restored byte-identical to HEAD after and no process remains; judged by `checks/w/judge_003y2.py`. THE VERDICT - A GENUINE +2, NOT A SWAP: vs the D-297 003W baseline (conn 70) connections 70->72, skipped 99->101, ratsnest 707/-74 -> 705/-76, journal 73->75; the connected-set diff GAINED exactly 2 - `N_BATDIV R89.2->U19.6` and `REC_BAT_LOW (node)->U19.7` (both SIG, F.Cu, 2 vias, board-legal 0.60/0.30) - and LOST 0. Both boxed U19 pins close SIMULTANEOUSLY for a strict +2 with nothing lost - the categorical opposite of D-296's 1-for-1 swap (`U19.7` 15.621 mm, `U19.6` 9.52 mm, both F.Cu 0.60/0.30 - the full router's board-legal choice, slightly different from the probe's In3/In2 route but the same +2). `LTC4368_FAULT_N` DETOURS CLEANLY (the D-298 §5 risk resolved): all three branches connected on B.Cu (`U18.7->R81.2` 8.478, `R81.2->R82.1` 2.719, `R82.1->Q9.1` 77.567 - the 64 mm run detoured ~13 mm longer and still cleared the per-connection `gate()`), FAULT_N not the terminal wall. Final DRC histogram IDENTICAL to 003W `{hole_clearance:5, lib_footprint_issues:199, solder_mask_bridge:1, track_width:1, unconnected_items:499}` - no new class, no increase; no sub-0.50 non-fine via (run via diameters 0.35/0.60/0.65/0.80, all on/above the D-257 ladder). THE NEW TERMINAL WALL - `LTC_GATE U18.10->Q3.4`, characterized without a new route: the +2 moved the wall two connections forward, past the whole U19 field, to the LTC4368 gate-drive net. It is NOT `NO_PATH`/`NO_LEGAL_ESCAPE` - candidate join paths ARE found but the per-connection `gate()` REJECTS every one for TWO FROZEN OWNER RULES: (1) D-249 `BAT_PROTECTED_P` high-current trunk (min 1.20 mm) - the candidate narrows to 0.20 mm where the BPP trunk-area rule applies (it enters the western-margin BPP trunk keep-region); (2) D-269 `BAT_MAIN` routed clearance (0.300 mm) - the candidate comes within 0.2803 mm, only ~19.7 µm short. `LTC_GATE`'s other segments are all connected (`U18.10->R76.1` F.Cu FINE_ESC_3 2x 0.35/0.20 vias; `Q3.2->Q3.4`/`Q2.2->Q2.4`/`Q3.2->Q2.2` B.Cu); the failing connection is the JOIN bridging the `U18.10` F.Cu escape cluster to the `Q3.4` B.Cu gate cluster. Same family as the U18.8/U19 walls - a full-run-emergent congestion/corridor pinch on a flexible low-current control net squeezed between the BPP trunk (D-249) and a BAT_MAIN path (D-269) - bounded and reducible in principle (the join has slack; the D-269 miss is ~20 µm), a routing/ordering lever within CTO scope, NOT an owner decision and NOT a rule relaxation. RULING: the U19 CAPACITY lever is a genuine +2 -> ACCEPTED and COMMITTED (the two source files retained/committed, banked env-gated/OFF-by-default, byte-identical when unset; D-298 deliberately deferred the commit to this gate); copper NOT promoted (full Phase-A still FAILs at the new LTC_GATE wall, so the board stays six layers / 0 tracks / 0 vias, readiness/progress UNCHANGED - D-286: only a full PASS promotes); NOT an owner decision (no floor relaxed, no frozen part moved, no DRU change); stop file ABSENT, autonomy CONTINUES. TESTS: `router_regression.py` ALL CHECKS PASS incl. G14 (lever OFF by default -> byte-identical; `AQROOT_U19CAP` activates; reserved-lane geometry spans U19.7/U19.6; hooks scoped to the U19 east lane + REC_BAT_LOW-before-N_BATDIV). INTEGRITY: authoritative PCB byte-identical to HEAD (`sha256 2235e273…d642d7e`; six layers, 0 tracks, 0 vias, placement at home); no DRC absorbed; `phaseA_journal.json` at HEAD; no via below the D-257 ladder; D-269 (0.300 mm), ≥1.20 mm BPP (D-249), 0.60 mm BAT_MAIN ENFORCED; D-290 untouched; the accepted `AQROOT_U18BPP_JOIN` (D-297), `place_003l` (D-285), the D-275/D-288 bridge, D-275 and D-277..D-298 preserved; frozen `beta-full-reference-v1` untouched; `JLCPCB_READINESS` unchanged. NEXT FBV2-P2-003Z: one narrowly-scoped env-gated (OFF-by-default) bounded lever that re-sites/re-orders/detours the `LTC_GATE U18.10->Q3.4` join corridor so its path stays clear of the `BAT_PROTECTED_P` 1.20 mm trunk region (preserving D-249) and opens the 0.300 mm `BAT_MAIN` clearance (preserving D-269), within D-257/D-266 mechanics, keeping `AQROOT_U18BPP_JOIN=I3`+`AQROOT_U19CAP=1` ON, full-authority-gate judged (connected-set diff vs `w/phaseA_003t_full_003y2_u19cap.json` and `w/phaseA_003t_full_003w_u18bpp_i3.json`), without any D-290 reauth, DRU/rule change, via below the D-257 ladder, D-249/D-269 relaxation, or topology/footprint/outline change; copper promoted only on a genuine full Phase-A PASS. Evidence: audit `audits/2026-08-30-p2-003y-d299-u19cap-full-gate-plus2-accepted-committed-ltc-gate-wall.md`. A GENUINE +2 BANKED IN SOURCE, BUT NO COPPER PROMOTED: PCB routing 0 %, overall 74 %, readiness ~77 %.**

**FBV2-P2-003X (2026-08-30) - D-298 GOVERNED CTO IMPLEMENT + SCREEN + HANDOFF: the bounded U19 CAPACITY lever (`AQROOT_U19CAP`) is implemented env-gated/OFF-by-default, regression-pinned (G14) and SCREENED DRC-CLEAN on the real 003W full-run board - both boxed U19 pins escape SIMULTANEOUSLY onto bare In3/In2 with the only board-legal 0.65/0.40 via (a capacity ADD, categorically distinct from the refuted D-296 swap); but copper is NOT promoted (the ~22-min full gate has not run) so source is left UNCOMMITTED and handed off, autonomy CONTINUES. Starting HEAD `46b582f` (D-297; pushed; `phaseA_journal.json` at HEAD). ROOT CAUSE (measured on `w/FULL003T_003w_u18bpp_i3/…kicad_pcb`): U19.6/U19.7 are a BOTTOM SOT-23-8, pad-boxed N/S by neighbour pads (placement-fixed); their only non-pad directions E/W are walled by control tracks - the EAST lane for BOTH by the same `LTC4368_FAULT_N R82.1->Q9.1` 64 mm B.Cu run (its direct path grazes the U19 east column; endpoint (4.85,28.95) sits 30 µm off U19.7's east ray). POFV (via-in-pad) is DRU-BARRED: the fine 0.35/0.20 via (annular 0.075) is DRU-legal only for specifically-named escapes, and U19.6 (N_BATDIV)/U19.7 (REC_BAT_LOW) are the ONLY U19 pins WITHOUT a D-257 exception (U19.2/U19.3/U19.5 have one); the only globally-legal via is 0.65/0.40, which fits only if the tighter pin escapes first. THE LEVER: (a) reserve the U19.7/U19.6 shared east escape lane with a keep-out so `LTC4368_FAULT_N` (low-current, ample slack) DETOURS on B.Cu, lifted before the closure stage; (b) close `REC_BAT_LOW U19.7` BEFORE `N_BATDIV U19.6` (U19.6-first re-boxes U19.7 - an intra-pair swap). Both then escape onto bare In3/In2. SCREEN (`checks/w/u19cap_probe_003x.py`, real board + real KiCad DRC): A both boxed; B FAULT_N reroutes around the reserved lane; C `U19.7`->In3 13.2 mm and `U19.6`->In2 9.6 mm COEXIST; D real KiCad DRC (zone-refilled) adds ZERO new violation involving REC_BAT_LOW/N_BATDIV - the U19 escapes are DRC-CLEAN. (The probe's crude whole-net FAULT_N reroute grazes BAT_MAIN - an artifact the driver's per-connection `gate()` resolves.) WHAT THE FULL GATE MUST JUDGE (D-286): (1) does FAULT_N detour CLEANLY under full congestion; (2) net +2 or a swap - only the ~22-min connected-set diff decides; no proxy promotes copper. RULING: IMPLEMENT + BANK OFF-by-default + HAND OFF; copper NOT promoted (full gate exceeds the ACP 10-min cap, may not be backgrounded). Source (`route_battery_block.py`, `router_regression.py`) left UNCOMMITTED per the 003X discipline; NOT an owner decision (no floor relaxed, no frozen part moved, no DRU change, direction-2 not exhausted); stop file ABSENT, autonomy CONTINUES. GOVERNING RUN (CTO, persistent terminal, ~22 min): `cd hardware/beta-v2/checks && cp phaseA_journal.json /tmp/phaseA_journal.HEAD.json && AQROOT_U18BPP_JOIN=I3 AQROOT_U19CAP=1 bash w/run_003t_full.sh 003x_u19cap w/cand_003t/t_a_r77e15n10_r79e15n10.json && cp /tmp/phaseA_journal.HEAD.json phaseA_journal.json` - keep `AQROOT_U18BPP_JOIN=I3` ON (D-297), do NOT combine with `AQROOT_U19_RESV` (D-296); judge by the connected-set diff vs `phaseA_003o_b1_r75rot_cto.json` and `w/phaseA_003t_full_003w_u18bpp_i3.json`. NEXT LEVER IF FAULT_N CANNOT DETOUR CLEAN: a DRU fine-via escape exception for REC_BAT_LOW U19.7/N_BATDIV U19.6 mirroring the existing D-257 exceptions for U19.2/U19.3/U19.5 - a DRU change / OWNER re-authorization, not a routing lever. INTEGRITY: authoritative PCB byte-identical to HEAD (`sha256 2235e273…d642d7e`; six layers, 0 tracks, 0 vias, placement at home); no DRC absorbed; `phaseA_journal.json` at HEAD (force-restored from git after the DEADCELL smoke run); no via below the D-257 ladder; D-269 (0.300 mm), ≥1.20 mm BPP, 0.60 mm BAT_MAIN ENFORCED; D-290 untouched; the accepted `AQROOT_U18BPP_JOIN` (D-297), `place_003l` (D-285), the D-275/D-288 bridge, D-275 and D-277..D-297 preserved; frozen `beta-full-reference-v1` untouched; `JLCPCB_READINESS` unchanged. `router_regression.py` ALL CHECKS PASS incl. new G14; `w/u19cap_probe_003x.py` ALL CHECKS PASS. Evidence: audit `audits/2026-08-30-p2-003x-d298-u19-capacity-east-lane-reservation-lever-screened-clean-handoff.md`. NO PROGRESS EARNED: PCB routing 0 %, overall 74 %, readiness ~77 %.**

**FBV2-P2-003W (2026-08-30) - D-297 GOVERNED ACCEPT (SECONDARY lever) + overall-run FAIL: the SECONDARY U18.8 I2-join lever (the D-295/D-296 handoff) completes `BAT_PROTECTED_P U18.8→R75.2` on In3 for a GENUINE +1 connected-set gain — a PURE JOIN with NO casualty, NO new via, NO new DRC class, and the lone `via_dangling` cleared — so it is ACCEPTED and retained env-gated / OFF-by-default in tracked source; but copper is NOT promoted (the full run still FAILs on the saturated U19 field), so readiness/progress DO NOT move. Starting HEAD `27f9790` (pushed; `phaseA_journal.json` at HEAD) carrying the lever WIP `checks/route_battery_block.py` (+25/−1 lines), a G13 regression contract, and the measured-record probe `checks/u18_i3_join_probe_003w.py`. THE WALL (D-294/295): at the D-293 direction-2 placement `t_a_r77e15n10_r79e15n10` the two 0.35/0.20 THROUGH reserve vias land at `R75.2`(2.800,66.800)/`U18.8`(7.200,66.500) on In2 and their In2 JOIN is `NO_PATH` (a `BAT_RAW` 0.600 mm current-path wall on In2 at x≈6.4→6.65 severs the west→east lane). THE LEVER: the reserve vias are THROUGH vias so the join is electrically identical on In2/In3; **In3.Cu is a routable six-layer signal layer** (`ROUTABLE[6]=('F','B','I2','I3')`) EMPTY across the whole corridor (2 In3 tracks board-wide, none here; only In1/In4 GND pours), so `AQROOT_U18BPP_JOIN=I3` completes the ONE branch on In3 with NO new via / NO DRU change / NO topology change; unset → join on `va[2]` (In2), byte-identical. PROBE (on the actual full-run routed board, throwaway copy): In2 `NO_PATH`, In3 **ok 4.410 mm**, real KiCad DRC **ZERO new classes**, `via_dangling` **1→0**. FULL GATE (`w/phaseA_003t_full_003w_u18bpp_i3.json`, secs 1272.5) vs the D-294 baseline `w/phaseA_003t_full_e15n10cto.json`: connections **69→70** (+1), skipped-already-connected **98→99** (+1, a downstream `BAT_PROTECTED_P` pad now found already-joined — a positive sign), ratsnest **708/−73 → 707/−74**, journal **72→73** (+1 `JOIN U18.8→R75.2` I3 4.410 mm 0 vias), DRC `via_dangling` **1→0** with **no new class**, terminal fatal wall **UNCHANGED** (`REC_BAT_LOW U19.7→(node) NO_LEGAL_ESCAPE`, `N_BATDIV U19.6` next). THE DECISIVE DIFF IS A STRICT PURE GAIN: the entire journal delta is EXACTLY one added JOIN entry with NOTHING lost — the opposite of D-296's 1-for-1 swap; the In3 join takes capacity from no other net (In3 unused). RULING: ACCEPTED and banked env-gated/OFF-by-default in source (pinned by G13 + the probe), but copper NOT promoted (Phase-A copper promotes only on a full PASS — D-286 — and the run still FAILs on the U19 field), so the board stays six layers / 0 tracks / 0 vias; a governed CTO ACCEPT + overall-run FAIL, NOT an owner decision; stop file ABSENT, autonomy CONTINUES. TESTS: `router_regression.py` ALL PASS incl. new G13 (In3 routable; OFF by default → byte-identical; `=I3` activates; non-I2/I3 never activates; override scoped to exactly `BAT_PROTECTED_P U18.8→R75.2`); `u18_i3_join_probe_003w.py` ALL PASS. INTEGRITY: authoritative PCB byte-identical to HEAD (`sha256 2235e273…d642d7e`; six layers, 0 tracks, 0 vias, placement at home); no DRC absorbed; `phaseA_journal.json` at HEAD (backed up/restored around the run); no via below the D-257 ladder; D-269 (0.300 mm), ≥1.20 mm BPP, 0.60 mm BAT_MAIN ENFORCED; D-290 untouched; `place_003l` (D-285) and D-275/D-288 bridge preserved; D-275 and D-277..D-296 preserved; frozen `beta-full-reference-v1` untouched; `JLCPCB_READINESS` unchanged; gitignored evidence preserved. NEXT FBV2-P2-003X: a bounded, env-gated (OFF-by-default) U19 CAPACITY lever that offloads a saturating U19-neighbour escape (the direction-2-induced `VREC_VCC U19.8` F.Cu pad-escape and/or a boxed U19 escape) onto a bare inner layer (In2/In3, proven free by D-297) to close `REC_BAT_LOW U19.7` AND `N_BATDIV U19.6` SIMULTANEOUSLY within D-257/D-266 mechanics — no via below the D-257 ladder, no D-290 reauth, no DRU/rule/topology/footprint/outline change — full-authority connected-set diff judged (a 1-for-1 swap is NOT a gain, D-296), copper promoted only on a genuine full Phase-A PASS. Evidence: audit `audits/2026-08-30-p2-003w-d297-secondary-u18bpp-i3-join-lever-net-gain-accepted.md`. A GENUINE NET GAIN BANKED IN SOURCE, BUT NO COPPER PROMOTED: PCB routing 0 %, overall 74 %, readiness ~77 %.**

**FBV2-P2-003V (2026-08-30) - D-296 GOVERNED FAIL / PRIMARY-FAMILY REFUTATION: the PRIMARY U19.7 escape-reservation lever (the D-295 handoff) FIRES and CLOSES U19.7 with a board-legal 0.60/0.30 via, but it is a bounded ORDERING TRADE with NO connected-set progress — it merely chooses which pin of the saturated U19 field is the casualty — so it is REJECTED for production and the uncommitted `AQROOT_U19_RESV` source WIP is RETIRED. Starting HEAD `a2e27fc` (pushed; `phaseA_journal.json` at HEAD) carrying one uncommitted WIP file `checks/route_battery_block.py` (+65 lines: the env-gated, OFF-by-default lever D-295 designed). Two full authority runs were completed under CTO authority and analysed here; NO new long run in this closeout (both artifacts pre-existed). RESV (0.35/0.20 rung): the corridor-less reservation cannot legalise a sub-minimum via (rejected on `via_diameter` board-min 0.50 mm + `annular_width` 0.075<0.125 mm), the reservation is dropped non-fatally, and the run falls through unchanged — `w/phaseA_003t_full_003v_u19resv.json` conn 69, skip 98, ratsnest 708/−73, journal 72 (NO RESERVE step), same DRC, terminal fail still `REC_BAT_LOW U19.7→(node) NO_LEGAL_ESCAPE`; **connected-set diff vs the D-294 baseline `w/phaseA_003t_full_e15n10cto.json` is EMPTY both ways** (the promised "unset reproduces the run" property). RESV2 (0.60/0.30 board-legal rung): the rung self-corrects to the ordinary Default GENERAL_SIGNAL 0.60/0.30 (REC_BAT_LOW's own SIG via), the reservation is laid (journal 73: +1 RESERVE step `REC_BAT_LOW U19.7→Q7.1`, 1 via) and **U19.7 ROUTES** — but the saturated U19 field is only re-ordered: `w/phaseA_003t_full_003v_u19resv2.json` conn **69** (unchanged), skip **98** (unchanged), ratsnest **708/−73** (unchanged), **identical DRC histogram** `{hole_clearance:5, lib_footprint_issues:199, solder_mask_bridge:1, track_width:1, unconnected_items:499, via_dangling:1}`, terminal wall MOVES to `N_BATDIV U19.6→(node) NO_LEGAL_ESCAPE` (U19.7 ×22, U19.5 ×20, track ×6, C60.2 ×5). THE DECISIVE CONNECTED-SET DIFF (D-294→RESV2) is a STRICT 1-FOR-1 SWAP: GAINED `REC_BAT_LOW U19.7→Q7.1 (SIG)`, LOST `REF_POL TP24.1→U19.2 (SIG)`, requested-connected count 68→68 — no connection genuinely gained or lost; the field is capacity-saturated, not priority-limited for one pin. POSITIVE FINDING (recorded, not promoted): the reservation MECHANISM is REAL and the D-295 diagnosis is CONFIRMED — with a board-legal via the lever fires as predicted, U19.7 is closable in principle, and the run stays fully board-legal (DRC unchanged, no rule relaxed, no via below the D-257 ladder) — but closing one pin at the cost of another in a saturated field is NOT a net gain (D-286: no proxy, not even a real full-authority run, promotes copper without net progress). RULING: REJECTED for production; the `AQROOT_U19_RESV` WIP is RETIRED via an EXACT REVERSE PATCH (`git diff -- checks/route_battery_block.py \| git apply -R`, scoped to the WIP hunks, not a destructive reset). RETIREMENT PROOF: `git diff --stat checks/route_battery_block.py` empty; worktree blob `bba62d35…` = `HEAD:checks/route_battery_block.py` blob (byte-identical); `git grep U19_RESV` over tracked source (excluding gitignored `checks/w/`) returns NO match; working tree clean apart from these docs; gitignored evidence preserved (`w/phaseA_003t_full_003v_u19resv.json`, `…_u19resv2.json`, `w/FULL003T_003v_u19resv*/`, `w/TEST003V_U19RESV/`). A governed FAIL, NOT an owner decision (the SECONDARY U18.8/U18.9 I2 reserve-via lever remains open); stop file ABSENT, autonomy CONTINUES. INTEGRITY: authoritative PCB byte-identical to HEAD (`sha256 2235e273…d642d7e`; six layers, 0 tracks, 0 vias, placement at home); no DRC absorbed; `phaseA_journal.json` at HEAD; no via below the D-257 ladder; D-269 (0.300 mm), ≥1.20 mm BPP, 0.60 mm BAT_MAIN ENFORCED; D-290 untouched; `place_003l` (D-285) and D-275/D-288 bridge preserved; D-275 and D-277..D-295 preserved; frozen `beta-full-reference-v1` untouched; `JLCPCB_READINESS` unchanged. HANDOFF FBV2-P2-003W: execute the SECONDARY D-295 lever — a bounded U18.8/U18.9 I2 reserve-via siting/ordering study for a ≥0.200 mm `U18.8→R75.2` join lane within existing D-257/D-266 corridor mechanics, full-authority-gate judged (connected-set diff vs `phaseA_003o_b1_r75rot_cto.json` and `w/phaseA_003t_full_e15n10cto.json`), WITHOUT any D-290 reauth, DRU/rule change, via below the D-257 ladder, or topology/footprint/outline change. Evidence: audit `audits/2026-08-30-p2-003v-d296-primary-reservation-lever-ordering-trade-no-progress-retired.md`. NO PROGRESS EARNED: PCB routing 0 %, overall 74 %, readiness ~77 %.**

**FBV2-P2-003U (2026-08-30) - D-295 GOVERNED CHARACTERIZATION / NO-PROGRESS + HANDOFF: the two D-294 walls are FULL-RUN-EMERGENT ordering/congestion casualties and NO cheap vehicle judges either at the D-293 direction-2 placement; the PRIMARY (`REC_BAT_LOW U19.7`) is diagnosed EXACTLY and shown REDUCIBLE-in-principle, but the governing ~22-min full gate cannot run foreground under the ACP 10-min cap, so 003U delivers a precise CTO handoff. Starting HEAD `36662db` (clean; `phaseA_journal.json` at HEAD). D-294 defined 003U as a bounded reservation/ordering study of the two 003T walls (PRIMARY reserve U19.7's escape before its neighbors; SECONDARY U18.8/U18.9 I2 reserve-via siting) judged only by the full authority gate. THE BINDING CONSTRAINT: full runs take ~1300-1780 s, the ACP wrapper caps a call at 10 min, and 003U bars backgrounding/detached jobs (foreground-only) - so the gate cannot run inside this task and the discipline requires a handoff. DECISIVE CHEAP EVIDENCE (connected-set diff 003O->003T, no new long run): in 003O ALL SEVEN U19 pins escaped, incl. `REC_BAT_LOW U19.7->Q7.1` (F.Cu 14.907 mm); direction-2 moves nothing near U19 so the escape GEOMETRY is identical - the +2-connection congestion SWAPPED `VREC_VCC`'s two segments' layers (`U19.8->C60.1` B.Cu(0via)->F.Cu(2via), `C60.1->R84.2` F->B), so U19.8's pad-escape now occupies the F lane south of U19.7 that carried U19.7 in 003O (`U19.8` x26 dominant blocker, U19.6 x13, U19.5 x7, track x6). U19.7 is a greedy-tightest-first casualty (the driver already re-measures order_tight per-pin, PR-32, so NOT a stale-table bug - U19.7 is loose early, sorts late, is boxed by its turn), and as a `(node)` join it is ineligible for the D-278 inner hop (guarded `and not node`). A REDUCIBLE ordering-class wall, NOT a D-289/290/292 placement mutual-exclusion. VACUITY PROVEN: `LOCAL=DEADCELL` (~2min) omits the divider field that causes the box and is placement-independent; `LOCAL=U19` reproduces it but lays the whole prefix (~15-20min, over cap); `R80`/`D256` never route U19; and the 003T `R80` focused screen reported the exact failing candidate as conn 20 `fail=None` U18.8 JOIN ok (vacuous). SECONDARY: at U18 n+1.25 both I2 reserve vias place and U18.9 JOINs, but U18.8->R75.2 JOIN NO_PATH ("no I2 corridor at 0.200 mm between the two reserved vias") -> NO_VIA_SITE - a full-congestion I2 pinch (reserve-via centres ~1.97 mm apart, so a corridor pinch, not touching vias); reserve-via siting/ordering lever, full-gate-only. VERDICT: both walls BOUNDED full-context routing/ordering casualties (no floor relaxed, no frozen part moved, D-290 untouched, D-269 enforced 0.300 mm); reducible/siting levers exist but only the full gate judges them (D-286: no proxy promotes copper) -> NO PROMOTION, a governed CTO characterization/NO-PROGRESS + HANDOFF, NOT an owner decision (direction-2 remains authorized and is not exhausted); stop file ABSENT, autonomy CONTINUES. HANDOFF FBV2-P2-003V: implement ONE narrowly-scoped env-gated (OFF by default) bounded lever, validate vs `router_regression.py` (authoritative byte-identical), then run `bash w/run_003t_full.sh 003v_u19resv w/cand_003t/t_a_r77e15n10_r79e15n10.json` (lever env set; back up/restore the shared `phaseA_journal.json`), judged by the full-run connected-set diff vs `phaseA_003o_b1_r75rot_cto.json` and `w/phaseA_003t_full_e15n10cto.json`. INTEGRITY: no source/copper/placement/rule change (git diff --stat HEAD empty for driver source); authoritative PCB byte-identical (`sha256 2235e273…d642d7e`; six layers, 0 tracks, 0 vias); no DRC absorbed; no promotion; `place_003l` (D-285) and D-275/D-288 bridge preserved; D-275 and D-277..D-294 preserved; frozen `beta-full-reference-v1` untouched; `phaseA_journal.json` at HEAD; readiness unchanged. Evidence: audit `audits/2026-08-30-p2-003u-d295-two-walls-full-run-emergent-ordering-cheap-vacuous-handoff.md`. NO PROGRESS EARNED: PCB routing 0 %, overall 74 %, readiness ~77 %.**

**FBV2-P2-003T (2026-08-30) - D-294 direction 2 (D-293) EXECUTED: a focused minimum candidate genuinely EXISTS, but the governing full authority gate FAILs, so NO candidate is promotable; direction-2 is PRODUCTIVE (+2 connections vs the committed 003O baseline, 67→69; the old REF_POL R87.2 terminal wall is now PAST) but INCOMPLETE (U18.8 BAT_PROTECTED_P still open on the full-context I2 join corridor; a NEW governing wall surfaces at REC_BAT_LOW U19.7 NO_LEGAL_ESCAPE). Starting HEAD `9c708f3` (clean; `phaseA_journal.json` at HEAD). D-293 (Alpha) authorized direction 2 - relocate the minimum escape targets so BAT_RAW (U18.1 east) and BAT_PROTECTED_P (U18.8 west) leave U18 through independent corridors, floors preserved, D-290 closed. 003T screened a bounded candidate grid on a cheap focused vehicle then ran the single survivor under FULL CTO AUTHORITY (the CTO completed the decisive run directly after ACP-wrapper failures). THE FOCUSED MINIMUM EXISTS: east-only (`e10`/`e20`) gate-rejects the east LTC_OV U18.3->R77.2 current pin (clearance 6 - narrow window), north-only (`n125`) breaches LTC_UV U18.2->R79.2 (clearance 1), R80/R81-north (`t_b`) leaves LTC_UV NO_LEGAL_ESCAPE; EXACTLY ONE combined move - `t_a_r77e15n10_r79e15n10` (R77/R79 +1.5 mm east +1.0 mm north, U18 north +1.25 mm) - clears every east AND west U18 pin with ZERO added DRC (focused conn 20, `fail=None`, U18.8 JOIN ok and U18.9 JOIN ok; `checks/w/phaseA_003t_t_a_r77e15n10_r79e15n10.json`). THE GOVERNING FULL GATE FAILs (`checks/w/phaseA_003t_full_e15n10cto.json`, secs 1313.8): PHASE A FAIL at REC_BAT_LOW U19.7->(node) NO_LEGAL_ESCAPE (blocked by U19.8 x26, U19.6 x13, U19.5 x7, track x6 - U19.7's own package neighbors escaped first and consumed its corridor); conn 69, skipped 98, ratsnest 781->708 (-73). U18.9 JOINED (In2 Kelvin) but U18.8 stayed OPEN - R75.2 join NO_PATH (no I2 corridor at 0.200 mm BETWEEN the two reserved vias - the U18.9-Kelvin and U18.8-BAT_PROTECTED_P reserve vias pinch it under full congestion), then fallback NO_VIA_SITE (no 0.65 mm B via site). Final DRC {hole_clearance:5, lib_footprint_issues:199, solder_mask_bridge:1, track_width:1, unconnected_items:499, via_dangling:1} - the residual scratch dangling is SURFACED, not absorbed. D-293 VERDICT: the focused closure is VACUOUS vs the congested full run (the governing gate); a candidate that closes only in the focused vehicle is NOT promotable (D-286: no proxy promotes copper), so NO 003T candidate passes the full authority gate -> NO PROMOTION. A governed CTO FAIL, NOT an owner decision (direction-2 was authorized, remains valid, and is NOT exhausted; the two walls are bounded full-context routing/ordering walls with no floor relaxed and no frozen part moved); `/home/aqroot8/.aqroot-autopilot-stop` stays ABSENT and autonomy CONTINUES. No source/copper/placement/rule change; authoritative PCB byte-identical to HEAD (sha256 2235e273...d642d7e; six layers, 0 tracks, 0 vias, placement at home); no DRC absorbed; `place_003l` (D-285) and the D-275/D-288 bridge preserved; D-275 and D-277..D-293 preserved; frozen `beta-full-reference-v1` untouched; `phaseA_journal.json` at HEAD; `JLCPCB_READINESS` unchanged. Scratch evidence gitignored under `checks/w/` (`phaseA_003t_full_e15n10cto.json`, `FULL003T_e15n10cto/`, `phaseA_003t_t_*.json`, `log_003t_*.txt`, `Q003T_*/`, `cand_003t/`, `mkcands_003t.py`, `batch_003t.sh`). NO PROGRESS EARNED: PCB routing 0 %, overall 74 %, readiness ~77 %. Full analysis: [`audits/2026-08-30-p2-003t-d294-direction2-full-gate-fail-u18-corridor-u19-escape.md`](audits/2026-08-30-p2-003t-d294-direction2-full-gate-fail-u18-corridor-u19-escape.md). Next FBV2-P2-003U: a bounded full-context reservation-and-ordering corridor study of the two named walls at the D-293 placement - (1, PRIMARY) reserve REC_BAT_LOW U19.7's escape BEFORE its neighbor pins U19.8/U19.5 and/or re-route the six blocking tracks (escape-ordering, no DRU change); (2, SECONDARY) study the U18.8/U18.9 I2 reserve-via siting/ordering to open a >=0.200 mm join corridor within D-257/D-266 mechanics - without re-authorizing D-290, dropping any via below the D-257 ladder, weakening D-269, or changing topology/footprint/outline.**

**FBV2-P2-003R (2026-08-29) - D-290 the LAST bounded routing-only U18 co-closure lever (off-layer vacate of U18.7) is REFUTED on cheap non-vacuous evidence (−1 regression, conn 34 vs baseline 35); the refutation is GEOMETRIC and EXACT — the U18.7/U18.8/U18.9 3-into-one-corner contention is an irreducible placement-geometry mutual-exclusion at the 0.5 mm pad pitch vs the 0.300 mm current-path clearance floor (D-269): the vacate moves U18.7's escape transition from a 0.15 mm B.Cu neck 0.250 mm from U18.8's reserve via to a 0.35 mm through via 0.150 mm from it (CLOSER, not farther), so In2 AND In3 both revert, and U18.9 is an independent casualty (dominant blocker U18.10 ×25) no via-size/layer/direction/ordering lever reopens without a DRU change or a frozen-part move. NO concrete legal routing-only site remains, so the standing policy trigger (D-289, CURRENT_STATE §5/§8) fires: a bounded LTC4368/R75 placement micro-ECO is now a genuine OWNER DECISION. The D-290 WIP is retired; `/home/aqroot8/.aqroot-autopilot-stop` is now PRESENT (OWNER_DECISION); autonomy HALTED. No source/copper/placement/rule change; routing 0 %, overall 74 %, readiness ~77 % unchanged. Baseline `phaseA_003r_baseline.json` (conn 35, U18.7✓ B.Cu 9.728 mm / U18.9✓ In2 north hop / U18.8 open NO_VIA_SITE) and lever `phaseA_003r_lever.json` (conn 34, U18.8✓ but U18.7 GATE_REJECTED 0.250<0.300 + U18.9 NO_LEGAL_ESCAPE) committed as evidence of record; the vacate genuinely fired (VACATE key matches `battery_route_plan.py:95`, In2/In3 both reverted) and In1/In4 are the only plane layers (In2/In3 free signal), so the failure is the pad-exit zone, not a plane or a rule. NEXT: OWNER DECISION — (A, CTO-recommended) authorize a bounded LTC4368/R75 placement micro-ECO to open a second independent U18.8 escape; (B) accept U18.8 open (not fabricable); (C) direction-2 widening. See [`audits/2026-08-29-p2-003r-d290-off-layer-vacate-refuted-owner-decision.md`](audits/2026-08-29-p2-003r-d290-off-layer-vacate-refuted-owner-decision.md).**

**FBV2-P2-003Q (2026-08-29) - D-289 the rejected 003P WIP is retired with NO PROGRESS (byte-aggregate-equivalent to 003O, a lateral U18.7↔U18.8 swap); the bounded U18.7/U18.8 co-closure reservation-ordering lever is REFUTED (reserve-U18.8-first = conn 34 vs baseline 35, U18.8 closes but U18.7+U18.9 open — a 3-into-one-corner placement-geometry mutual-exclusion, U18.8→R75.2 escapes NORTH into U18.7's single B lane with no alternative B via-site); the terminal REF_POL R87.2 wall is characterized as F.Cu routing capacity (N_POL 6.36 mm F.Cu saturates the R87.2→node corridor), narrowest lawful lever = D-279-class N_POL F.Cu inner offload (no DRU exception, no closure claimed). No source/copper/placement/rule change; routing 0 %, overall 74 %, readiness ~77 % unchanged. Next FBV2-P2-003R: off-layer vacate U18.7 + reserve U18.8 first + extend the D-279 offload to N_POL's F.Cu run. See [`audits/2026-08-29-p2-003q-d289-003p-rejection-and-u18-co-closure-refuted.md`](audits/2026-08-29-p2-003q-d289-003p-rejection-and-u18-co-closure-refuted.md).**

**FBV2-P2-003O (2026-08-29) - D-288 the D-275 south-bridge ENTRY-array two-layer tie is FIXED (rotation-aware in-pad POFV scan + a symmetric B.Cu tie-stub make the entry vias `via_dangling`-clean), proven by a non-vacuous regression (legacy off-pad array +4 dangling / fixed 0 dangling / no absorption) AND a natural-completion CTO full run (DRIVER_EXIT=0, no dangling cascade) - but the overall Phase-A run still FAILs on genuinely NEW downstream blockers (U18.8 BAT_PROTECTED_P escape NO_VIA_SITE + a terminal REF_POL/R87 F-corridor NO_PATH + BAT_RAW divider width vs BAT_MAIN); the bridge-code fix + regression are ACCEPTED/COMMITTED, no routing/overall progress, no readiness change, no promotion, D-275 and D-277..D-287 preserved.** A successful bridge-implementation fix is distinct from an overall Phase-A pass. **The fix (both parts in the bridge code):** (1) `bridge_route_003c.scan_entry_sites` windowed on R75.2's UNROTATED `hx/hy` and sorted "south-first", so for the -90 deg-rotated R75 pad it scanned a tall-narrow box and picked the NORTHERNMOST sites - ~0.5-1.15 mm NORTH of the real B.Cu pad copper (the D-287 dangle); new `_in_pad()` transforms each candidate into the pad's OWN rotated frame and requires the centre inside the pad rectangle inset by `IN_PAD_MARGIN=0.20 mm`, scanning the rotation-aware AABB and sorting CENTRE-OUT so every barrel overlaps the pad; (2) `stage_bridge`/`bridge_early_003i.apply_early` lay an explicit B.Cu tie-stub from each entry via to R75.2's pad centre (mirror of the exit `_lay_landing` stub) - two-layer tie. **The D-288 regression is NON-VACUOUS (`screen_003n.py --bridge --validate`, exit 0):** NEGATIVE control reproduces the D-287 off-pad F.Cu-only array and MUST dangle - measured **`via_dangling +4`**; POSITIVE control (the real fixed bridge) is CONNECTED, **`via_dangling == 0`, entry 4, exit 4, ywest 82.4, traverse 1.30 mm**; NO-ABSORPTION - zero new hard-class DRC vs the identical no-bridge board. **The natural CTO full run (`b1_r75rot`, `DRIVER_EXIT=0`, secs 1776.5; evidence `checks/phaseA_003o_b1_r75rot_cto.json`, log `w/log_003o_b1_r75rot_cto.txt` gitignored):** PHASE A **FAIL**; connections **67**, skipped-already-connected **99**, ratsnest **781 -> 708 (-73)**. The early south bridge now passes BOTH geometrically AND electrically - `land C36.1`, traverse **70.925 mm @ 1.30 mm**, **entry 4 @ y=67.95 GENUINELY INSIDE the rotated R75.2 pad**, exit 4, disjoint `ywest 82.40` - with **NO `via_dangling` cascade** anywhere (the D-287 20-gate poisoning is GONE; the fix holds at full-run scale); the `BAT_PROTECTED_P` island CLOSES **R75.2 through C36/C25/C58/D9/U11/U14/TP15**. **The NEW blockers (attributed, NOT absorbed):** (1) **`U18.8` OPEN** - reservation `GATE_REJECTED` on `clearance +1` (BAT_MAIN routed clearance), then main pass **`NO_VIA_SITE`** (no 0.65 mm / 0.35 mm via site reachable on B) - so the trunk is one pad short of full closure; (2) terminal fail **`REF_POL R87.2->(node)` NO_PATH - no F corridor at 0.150 mm** (also `R88.1->R87.2`); (3) **`BAT_RAW R89.1->(node)` NO_PATH at 0.600 mm** and `R86.2` `GATE_REJECTED` on **`track_width +4`** (four 0.20 mm BAT_RAW divider taps breach the BAT_MAIN 0.60 mm rule - a rule-conformance reject; the final board carries no such delta). **Final DRC (re-verified with `kicad-cli`):** `{hole_clearance:5, lib_footprint_issues:199, solder_mask_bridge:1, track_width:1, unconnected_items:499}` - reproduces the JSON exactly; **the single `track_width` item, truthfully:** a `BAT_PROTECTED_P` track on B.Cu, length 2.4749 mm @ (65.5, 76.05), actual 0.2000 mm, violating rule `BAT_PROTECTED_P high-current trunk width - D-249` (min 1.2000 mm) - one thin 0.20 mm SENSE/Kelvin sub-branch of the high-current trunk net (a bounded sense tap), a genuine-but-benign copper item SURFACED not absorbed, NOT a fabrication blocker. **Not an owner decision:** the new blockers are BOUNDED technical work, not a placement wall and not un-fixable without relaxing a floor or moving a frozen part; direction-2 is NOT the sole option; `/home/aqroot8/.aqroot-autopilot-stop` stays ABSENT. Source changes only in the bridge code / regression (`bridge_route_003c.py`, `bridge_early_003i.py`, `screen_003n.py`); no driver/router/DRU/footprint/netclass/rule mutated; no rule/floor relaxed (0.200 mm clearance + 0.25 mm hole-to-hole + >=1.20 mm BPP trunk + 0.60 mm BAT_MAIN ENFORCED); no DRC absorbed; `c3_00` not promoted; `place_003l` (D-285) preserved; frozen `beta-full-reference-v1` untouched; `JLCPCB_READINESS` unchanged; `checks/phaseA_003o_b1_r75rot_cto.json` pinned as evidence of record. Authoritative PCB UNCHANGED (six layers, 0 tracks, 0 vias, placement at home); `phaseA_journal.json` restored to HEAD. Regressions GREEN: `screen_003n.py --bridge --validate`; router regression G1-G12 (prior work). **NO PROGRESS EARNED: PCB routing stays 0 %, overall stays 74 %, readiness ~77 %.** Full analysis: [`audits/2026-08-29-p2-003o-d288-entry-tie-fix-and-full-run.md`](audits/2026-08-29-p2-003o-d288-entry-tie-fix-and-full-run.md). **Next: FBV2-P2-003P - (1) close U18.8's BAT_PROTECTED_P escape (a legal B-layer via site + reservation past the BAT_MAIN routed-clearance reject) to fully close the trunk; (2) investigate/close the terminal REF_POL R87.2 / R88.1->R87.2 F-corridor NO_PATH at 0.150 mm; (3) attribute the BAT_RAW R89.1/R86.2 divider-tap width vs the BAT_MAIN 0.60 mm rule - all with NO floor/rule relaxation and NO frozen-part move.**

**FBV2-P2-003N (2026-08-29) - D-287 the full bounded direction-1 placement screen is EXHAUSTED (27/27); only three candidates pass the D-286 zero-copper hard gates, and a cheap CTO-authorized bridge-connectivity probe REFUTES all three at the SAME genuine electrical fault - the D-275 south-bridge ENTRY array (R75.2 POFV) is bussed on F.Cu with NO symmetric B.Cu tie-stub, so its vias dangle on one layer regardless of placement; a BRIDGE-IMPLEMENTATION defect independent of placement (a narrower CTO lever), NOT a placement wall; direction-2 is NOT yet the sole option, so a CTO ENGINEERING FAIL, not an OWNER decision; no promotion, D-275 and D-277..D-286 preserved.** The deliverable `checks/screen_003n.py` screens the complete direction-1 space - `b1`(6), `c3`(4), `cand_00..11`(12), `c2`(5) - on REAL scratch boards at the exact D-286 pre-copper boundary (002F ECO + `place_003l` + candidate -> connectivity -> fill -> save -> `kicad-cli drc --severity-all`) plus the real `qb.escape` solver, rejecting any different-net pad short / sub-0.200 mm clearance / hole breach / courtyard overlap / un-escapable U18 pin vs the `place_003l`-only CLEAN reference. **24 REJECT at bare placement; only `b1_r75rot`, `b1_r75rotN`, `b1_q3rot` pass every hard gate + all U18 escapes.** `b1_r75rot` was then taken into a parent-supervised full run: the early south bridge reported a GEOMETRIC success (`land=C36.1 traverse=72.786 mm w=1.40 entry=4 exit=4 ywest=82.40`, disjoint) but every subsequent gate rejected on brand-new `via_dangling +4` across 20 unrelated post-bridge gates until the CTO stopped the cascade (`DRIVER_EXIT=143`). A cheap **bridge-connectivity probe** (added to `screen_003n.py --bridge`) reproduces the exact early bridge in ISOLATION on each survivor's placed board and gates KiCad `via_dangling` as the authoritative >=2-layer test; validated against the b1_r75rot control (+4). Result: `b1_r75rot` dangling **4**, `b1_r75rotN` dangling **4**, `b1_q3rot` dangling **2** AND not-disjoint (`ywest 61.04 < 75`) - **0/3 truly connected**; the probe evaluated both remaining survivors decisively so no expensive full run was warranted. **Root cause:** every dangling via is the ENTRY array on R75.2 - R75.2's B.Cu pad spans y in [67.35, 68.58] but the entry vias land at y~66.19-66.81 (~0.5-1.15 mm NORTH of the pad, over bare substrate), tied only by the F.Cu bus -> one-layer -> dangling; the EXIT array does NOT dangle because `_lay_landing` lays an explicit B.Cu tie-stub to its landing pad, while the ENTRY array has NO such symmetric B.Cu tie-stub. The defect is structural to `bridge_early_003i.apply_early` / `bridge_route_003c.scan_entry_sites`, present in every placement; the earlier "the D-275 bridge PASSED" (003K/L/M) were GEOMETRIC passes never electrically gated (masked by the c3_00 short). **Not an owner decision:** the placement space is exhausted but the survivors fail on a bridge-connectivity IMPLEMENTATION defect independent of placement - the charter's "narrower CTO technical lever" - so direction-2 (broad LTC4368 refloorplan, OWNER/mechanical) is NOT yet the sole option; `/home/aqroot8/.aqroot-autopilot-stop` stays ABSENT. Regressions GREEN: `screen_003n.py --validate` (c3_00 REJECT +3, reference clean), `screen_003n.py --bridge --validate` (b1_r75rot FAIL, via_dangling +4). Only new file `checks/screen_003n.py`; no driver/router/DRU/footprint/netclass/rule source mutated; no DRC absorbed (a `via_dangling` item IS the FAIL reason); `c3_00` not promoted; `place_003l` (D-285) preserved and clean; optional `BAT_SENSE TP20.1` separate; `JLCPCB_READINESS` unchanged. Authoritative PCB UNCHANGED (six layers, 0 tracks, 0 vias, placement at home); `phaseA_journal.json` restored to HEAD. **NO PROGRESS EARNED: PCB routing stays 0 %, overall stays 74 %.** Full analysis: [`audits/2026-08-29-p2-003n-d287-bridge-entry-array-dangling.md`](audits/2026-08-29-p2-003n-d287-bridge-entry-array-dangling.md). **Next: FBV2-P2-003O - fix the D-275 south-bridge ENTRY-array two-layer tie (add an explicit B.Cu tie-stub from each entry via to R75.2's pad centre and/or constrain `scan_entry_sites` to in-pad sites) so the entry vias are `via_dangling`-clean, symmetric to the proven exit array, no rule/floor/topology/footprint change, no absorption; verify with `screen_003n.py --bridge` on `b1_r75rot` (via_dangling 0, entry >=3, exit >=3, disjoint ywest > 75, >=1.20 mm) then take it to a parent-supervised full Phase-A run and close `BAT_PROTECTED_P`; direction-2 becomes an OWNER DECISION only if the entry tie proves un-fixable without relaxing a floor or moving a frozen part.**

**FBV2-P2-003M (2026-08-29) - D-286 the gate DRC/ratsnest baseline was measured BEFORE the candidate placement (latent since 002L), so `c3_00`'s own placement DRC poisoned every gate; the harness now measures the baseline on the ACTUAL complete pre-copper placement, and with that correction the definitive full run SURFACES that candidate `c3_00` is electrically invalid; harness fix + G12 regression COMMITTED, `c3_00` recipe MEASURED FAIL, no authoritative promotion, D-275 and D-277..D-285 preserved.** The first parent-supervised 003M attempt was invalidated by a `GATE_REJECTED` cascade (`DRIVER_EXIT=143`, CTO-stopped) carrying a FIXED placement DRC delta across unrelated nets. **Root cause:** `route_battery_block.py` computed the gate baseline right after the 002F ECO (+`AQROOT_ECO_EXTRA`) but BEFORE `AQROOT_PLACE_JSON` moved the candidate footprints, so the candidate placement's own DRC (`courtyards_overlap`/`solder_mask_bridge`/`shorting_items`/`clearance`) was never in the comparison baseline and every gate `after − base` read it as new copper. **Fix:** relocate the baseline to AFTER full placement application + connectivity rebuild + zone fill + save + `DRU.write` + fingerprint assertion, before any QBoard copper; the no-candidate path is byte-identical, so default behaviour is preserved. **Attribution (do not blindly absorb):** on independent zero-copper boards, 002F-ECO-only and `place_003l`-alone are CLEAN (D-285 vindicated); `c3_00` alone is the SOLE cause - it moves U18 (LTC4368)→(4.0,72.9,90°) onto R83/R80, producing THREE genuine different-net pad shorts (`R83.1[BAT_PROT_SHDN_CTL]` on `U18.3[LTC_OV]`/`U18.2[LTC_UV]`, `R80.1[BAT_RAW]` on `U18.10[LTC_GATE]`) + GND 0.100 mm / Default 0.0088 mm sub-floor gaps, all with ZERO copper (no route can remove a pad-on-pad short); the router's escape solver names the identical culprits. **The definitive full run (corrected harness, exact D-285 recipe, `DRIVER_EXIT=0`):** PHASE A FAIL (`BAT_PROT_SHDN_CTL R83.1→(node) NO_NODE`), connections 64, ratsnest 713 (−68); the early south bridge PASSED at full width (`land C36.1`, traverse 72.994 mm @ 1.50 mm, entry 4/exit 4, disjoint ywest 82.4) - the run fails on the c3_00-collided LTC control cluster, not the bridge. Gate discipline held: only `track_width +1` (a 0.2 mm BPP bounded-stub) increased vs baseline; the placement shorts sit identically in baseline AND final (surfaced, not absorbed). **G12** regression pins the corrected order (fails the old ordering, proves a post-baseline copper violation is still surfaced). Preflight all PASS: `router_regression` G1-G12, `bridge_probe_003c/003d/003i/003j/003k/003l`, `u19_escape_probe_003e/003f/003g/003h`. Authoritative PCB UNCHANGED (six layers, 0 tracks, 0 vias, C36/U18 home); `phaseA_journal.json` restored; `c3_00` not promoted; optional `BAT_SENSE TP20.1` separate; `JLCPCB_READINESS` unchanged. **Ordinary engineering FAIL (CTO scope), NOT an OWNER decision** - other LTC-block candidates (`c3_01/02/03`, `cand_00..11`, c2 family) are unscreened; direction-2 is not yet the sole option. **NO PROGRESS EARNED: PCB routing stays 0 %, overall stays 74 %.** Full analysis: [`audits/2026-08-29-p2-003m-d286-baseline-order-and-c3_00-placement-shorts.md`](audits/2026-08-29-p2-003m-d286-baseline-order-and-c3_00-placement-shorts.md). **Next: FBV2-P2-003N - re-screen the LTC-block placement candidates with the corrected D-286 post-placement baseline DRC, then integrate the first genuinely short-free, routable candidate with `place_003l` + the proven south bridge on a parent-supervised full run.**

**FBV2-P2-003L (2026-08-28) - D-284 (OWNER) + D-285 the minimal landing-opening PLACEMENT ECO OPENS a legal southern `BAT_PROTECTED_P` landing - PASS CANDIDATE for supervised Phase-A integration; no authoritative promotion; D-275 and D-277..D-283 preserved.** D-284 is the OWNER decision (Alpha, ratifying the CTO call): open a legal southern landing by a **bounded placement spread of the `C36`/`C25`/`U11`/`BQ25185_SYS` neighbourhood** (landing-opening direction 1), not corridor widening, not a broad refloorplan. D-285 (003L) delivers and PROVES it. **The two 003K blockers, and why the fix is forced:** the forced-south bridge lands on `C36.1` and hit TWO independent violations - (1) `GND` 0.0726 mm to C36's OWN GND pad (1.55 mm east, same footprint), INVARIANT under any C36 translation so fixable ONLY by a C36 ROTATION; (2) `BAT_MAIN` 0.0864 mm to `R68`'s fixed F.Cu `BQ25185_SYS` pad (R68 is `0R DNP` but KiCad-connected, 16-pad net). A vertical C36 is 3.05 mm tall, so it cannot sit south enough to clear R68 without colliding `C5`'s courtyard. **The minimal ECO (`place_003l`, two footprints):** `C36` -> (63.75, 75.10, 270 deg) - the rotation moves C36's GND pad 1.55 mm SOUTH of the north-poking exit array (blocker 1: 0.0726 -> 0.4750 mm) and the ~1.35 mm south shift clears R68 by distance (blocker 2: 0.0864 -> 0.2941 mm); `C5` (100 nF +3V3/GND decoupler, the SOLE courtyard obstruction to the vertical C36) -> (61.95, 75.15, 90 deg). R68 is deliberately NOT moved (real 16-pad net, no nearby legal home); C5 was chosen for plane-net routing latitude; both moves are STRICTLY NECESSARY and inside the approved neighbourhood (C5 is 1.9 mm from C36), recorded not taken silently. **The measurement (`bridge_probe_003l`, PASS):** on the reconstructed sparse 002F board + the D-275 south bridge - LANE lays (land `C36.1`, traverse **1.40 mm** F.Cu, entry 4, exit 4); DISJOINT (ywest 82.40 mm > 74.7); the LANDING OPENS with **ZERO** east-landing clearance violations, the two named blockers now clear (C36.2 GND 0.4750, R68.1 BAT_MAIN 0.2941 mm); the DRC delta vs the 003K board is EXACTLY the two landing clearances removed (clearance 4 -> 2, the survivors are the pre-existing WESTERN LTC-block issues - `R83`/`U18` GND 0.100, `R80`/`U18` LTC_GATE 0.0088; `courtyards_overlap` stays 3; every other class identical) - NO new genuine DRC class or item, and NONE absorbed. Governing achieved landing clearance **0.2941 mm**, 47% over the 0.200 mm floor; the bridge holds 1.40 mm (>= the 1.20 mm floor). **Why a CANDIDATE, not a promotion:** the proof is a REAL board + REAL kicad-cli DRC (not an analytical proxy) but proves the LANDING, not full-board connectivity - that needs the parent-supervised full Phase-A run, which task discipline forbids starting unsupervised; the ECO is integrated DEFAULT-INERT via the existing 002J `AQROOT_ECO_EXTRA` override (`place_003l.json`), no driver edit, and the authoritative PCB is left untouched (0 tracks / 0 vias, C36 at home pose). **SUITES ALL PASS:** `bridge_probe_003l` (NEW, A lane / B disjoint / C landing opens + named blockers clear / D no new DRC item / E invariant + no promotion), `bridge_probe_003c`/`003d`/`003i`/`003j`/`003k`, `router_regression` G1-G11, `u19_escape_probe_003e/003f/003g/003h`. Three NEW additive files (`place_003l.py`, `place_003l.json`, `bridge_probe_003l.py`); no existing code modified, so every default path is byte-identical. **Nothing frozen moved and nothing relaxed:** only `C36`/`C5` moved (D9/U18/R75-R83/Q3/FETs/C58/U19/D10/`R68`/U11 held); no topology/net/footprint/polarity/value/layer-stack/GND/netclass/width/clearance/hole-floor change; no safety weakening; `c3_00` NOT promoted; the optional `BAT_SENSE TP20.1` (TEST) point SEPARATE; `JLCPCB_READINESS` not touched; D-275/D-277..D-283 held byte-fixed; no authoritative promotion. Phase A NOT completed; Phase B NOT run. **NO PROGRESS EARNED: PCB routing stays 0 %; overall stays 74 %** - the blocker is resolved at candidate level, awaiting the supervised gate. **Next FBV2-P2-003M:** the parent-supervised full Phase-A run with the 003L placement + the early south bridge (pinned D-271 recipe + `AQROOT_BRIDGE_EARLY=1 AQROOT_BRIDGE_SOUTH=1` + `AQROOT_ECO_EXTRA=hardware/beta-v2/checks/place_003l.json`), to route all nets around the reserved bridge without reintroducing the 003I corridor failures and close `BAT_PROTECTED_P` with no new DRC; authoritative promotion and a routing-% award only if the full Phase-A gate passes with full connectivity; hold D-275/D-277..D-285 fixed, no netclass/width/clearance/hole-to-hole relaxation, no long full run without CTO supervision. Full analysis: `audits/2026-08-28-p2-003l-d285-landing-opening-placement.md`.
