...
| field | value |
|---|---|
| FAP-01 git SHA (`git rev-parse HEAD`, clean tree) | the D-802 content commit, recorded in `hardware/demo/manufacturing/evidence/d802-review-target.json` (`fap01_image.git_sha`) |
...
| `T` | one ISO14443A REQA while `N` is on — **report only** (receiver on power-up defaults).  Three verdicts: `VALID ANSWER: a tag answered, ATQA xx xx` only when the field and `sup3V` read back, the IRQ registers were cleared and Clear FIFO was PROVED (FIFO status reads empty) before the REQA, the REQA raised `I_txe` and `I_rxe`, exactly two whole bytes arrived with no FIFO over/underflow and no parity / framing / CRC error, the FIFO drained to empty on the two-byte read, and the ATQA passes ISO/IEC 14443-3 (RFU bits clear — `FF FF` fails); `no tag answered` only with the same proof, `I_txe`, no `I_rxe` and an empty FIFO; otherwise `NO VALID EVIDENCE: <reason>` (an all-ones or all-zero bus, an ignored command, a stale FIFO, a failed transfer) — neither an answer nor its absence | one frame | refused without a field, and refused while a `U9` chip-select hold-off (`J`/`K`/`Y`) is armed or pending; a transaction the hold-off interrupts is `NO VALID EVIDENCE` | — | `C-NFC-TUNE-01` tag read (RECORD) |
...
* **`T` (REQA) is report only.**  The ST25R3916 receiver runs on its power-up
  ...
  evidence listed in §3 (D-802): D-801 printed "a tag answered" for any FIFO
  count of two or more, so an all-ones bus or a stale FIFO read as a tag.
...
* **H6** — `Firmware/test/test_fap01_image.cpp` on the FAP-01 build (every
  stimulus above reachable, gated, bounded, quiesced; 33 destructive controls,
  each caught by the claim it names — 13 of them D-802's: no `sup3V` before the
...
* **H10** — FAP-01 isolation: only `[env:aqroot-demo-fap01]` compiles
  ...
  (D-802); eleven destructive controls.
