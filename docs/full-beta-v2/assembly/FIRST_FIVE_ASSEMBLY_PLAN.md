...
is not a hand-placement candidate at any skill level without a stencil and reflow. The D-800 live sweep
reads **zero** at JLC against a need of five (the "six in stock" this line carried was an FBV2-S2 snapshot); **buy the exact MPN plus spares from a franchised distributor and consign.**
...
> IMAGE: PENDING.**  The RELEASE image (`[env:aqroot-demo]`) is a bring-up image and
...
> live.  The steps that need those states are therefore run with a separate
> FIRST-ARTICLE TEST IMAGE, which must exist, be host-tested and be recorded
> (git SHA) before FA01, FA05, FA06, FA07 and FA08 begin: CC1101 continuous
> transmit and SX1262 CW at 22 dBm keyed ONLY through `SpiBusB::beginTransmit`, an
> NFC field keyed ONLY through `DemoBringupApp::beginNfcFieldSession`, a Wi-Fi/BLE
> TX burst, a 38 kHz IR transmit burst from `D1`, a chip-select hold-off injection for
> `U7` and `U9` (the pin held high while the driver believes it selected the part), a held audio drive at the capped level with the rails the rail-edge
> table admits, a held backlight PWM duty after the D-784 prime, and a 10 ms
> `VCELL` poll — every one under the SAME accessory permission table and
> quiesce/liveness rules as the release image.  It is never shipped.  It is a
> first-article deliverable, not a PCB/PCBA order item: nothing on the board
...
| **`C-BAT-GATE-01`** | `ΔVGATE` (GATE − `BAT_PROTECTED_P`) and pass-pair drop (`BAT_RAW` − `BAT_SENSE`) at **2.60 A** at `BAT_RAW` = 4.15 / 3.60 / 3.05 V — six numbers, on the fitted **`AO4800`**.  Draw the test current with an electronic load from `BAT_PROTECTED_P` (`TP15`) to `GND`, NOT through the system: the load then never passes `U11`'s BATFET, whose BATOCP (3.13 A TYP, ±18 % at one condition) could otherwise trip near 2.60 A, and `U11`'s own battery cut-off at 3.05 V is irrelevant to the pass pair.  The pass-pair current is also the drop across `R75` (10 mΩ, `TP20` to `TP15`).  This is what converts the one thing NO candidate publishes — hot `RDS(on)` at `VGS` = 2.5 V — from an extrapolation into a measured bound.  **D-791 / `D790-A01` ADDS TWO MORE**: the TURN-ON TIME from `SHDN` release to full enhancement, because ADI specifies `IGATE(UP)` and `tD(ON)` only at `VIN` = 12 V and neither is a 1S condition so the datasheet does not bound it here; and the current at which `VGS(Q2)` falls to the `AO4800`'s 2.5 V row, against the DERIVED ceiling of **2.2845 A**  **OUTCOME:** PASS if the current at which `VGS(Q2)` falls to the AO4800's 2.5 V row is at or above the DERIVED **2.2845 A** ceiling and the pass-pair drop at every point is at or below the model's (a `VGS(Q2)` under the 2.5 V row at 2.60 A is the model's own worst-case prediction, **2.4124 V** in `SOURCING_LEDGER.md` §4, and is recorded, not failed — D-800 corrected a criterion that demanded more than the design guarantees); otherwise RECORD + ESCALATE and re-run F10, F12 and F14 with the measured gate drive and drop before acceptance. | [`SOURCING_LEDGER.md`](SOURCING_LEDGER.md) | D-789 / `R8-N01`, part fixed at D-790 / `D789-A01` |
