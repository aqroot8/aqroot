...
> 2026-09-20/21 records.  Re-sweep with `--refresh` immediately before the order.
...
> `DemoBringupApp::beginNfcFieldSession` (`T` one REQA, report only); `V` + `W` a
...
> off, nothing else keyed); `I` a 38 kHz NEC burst from `D1`; `H` / `J` a
