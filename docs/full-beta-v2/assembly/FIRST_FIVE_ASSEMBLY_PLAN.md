...
| generic 0402 / 0603 / 0805 R and C | 110 R, 76 C fitted (D-800 count from the released BOM) | Basic library for most; the D-800 sweep marks 31 R/C lines EXTENDED (`evidence/d800-sourcing-sweep.json` `library`) | abundant |

The 186 fitted passives are the bulk of the placement count; most are Basic, and every Extended one is an exact MPN on the released BOM (D-800 corrected the FBV2-S2-001 figures here, 127 R / 80 C "entirely Basic", which no longer describe this board). **They were
...
| `TPD4E1B06DRLR` | `D2`, `D4`, `D5` — there is **no `D3`** in this schematic (D-790 / D789-A07 corrected this row: it read `D2`–`D5` and over-counted the need by five pieces) | `C1972953` | 2,176 | 15 |
| `PMEG2010AEH,115` | `D9` | `C110921` | 32,772 | 5 |
| `MHPA3528RGBCT` | `D13` | `C409779` | 70,369 | 5 |
| `BSS138LT1G` | `Q4`, `Q6`–`Q9` | `C82045` | 762,522 | 25 |
| `GRM188R61E106KA73D` | `C24` (0603) | `C344022` | 675,039 | **5** |
| `GRM31CR71E106KA12L` | `C26`, `C27` (1206; D-800 corrected this table, which had put `C26` on the 0603 line and left `C27` out) | `C77093` | 211,062 (D-800 sweep) | **10** |
...
| **`BAT54WS-7-F`** Diodes Incorporated | `D10`–`D12`, **`D14`** (D-800: `D14` became a `BAT54WS-7-F` at D-789 and this row still said 15) | **`C124205`** | **46,819** | **20** |
...
> complete sweep at **§7c**.

> **D-801 FRESH SWEEP (`D801-08`).**  `evidence/d801-sourcing-sweep.json` re-queried all
> 123 lines with `--refresh` on **2026-09-25** (every row dated).  **`TPD4E1B06DRLR`
> (`D2`,`D4`,`D5`, `C1972953`) is now SHORT — 2 against 15 — and is a consignment line
> too**; `SQ2364EES-T1_BE3` reads 40 and stays a consignment line (stock moves over
> days); `LTC4368IMS-1#TRPBF` reads 0.  D-800's "live" sweep was a replay of
> 2026-09-20/21 records.  **D-802:** `evidence/d802-sourcing-sweep.json` (`--refresh`,
> **2026-09-26**, every row dated) reads the same nine short lines at the same counts.
> Re-sweep with `--refresh` immediately before the order.
...
is not a hand-placement candidate at any skill level without a stencil and reflow. The D-801 fresh sweep
(`--refresh`, 2026-09-25) reads **zero** at JLC against a need of five (the "six in stock" this line carried was an FBV2-S2 snapshot); **buy the exact MPN plus spares from a franchised distributor and consign.**
...
*(D-800: `≈3.05 V` is inside the BQ25185 battery cut-off band the model carries,
2.85–3.15 V.  If a unit's BATFET disconnects there, or during a step, RECORD the
pack voltage at which it did — that is the unit's own `VBUVLO`, not a failure of
this step — and take the boost-operation points just above it.)*
...
- **radio step**: Wi-Fi TX bursts with `U7`/`U8` idle, and the worse sub-GHz
  radio transmitting; for each sub-GHz radio keyed at its maximum configured
  power, ALSO scope `+3V3` at that module's own `VCC` land — `U8.9` (SX1262
  module at 22 dBm) and `U7.9` (CC1101 module) — with the ground spring on the
  module's adjacent `GND` pad (`U8.10` / `U7.11`), and judge its trough against
  the same **3.000 V** rule (Ebyte's E22-M manual: full output power at ≥ 3 V).
  *(D-800 / Opus `R19-06`: neither module has a board-local ceramic at its
  `VCC` land — each land reaches the `F.Cu` and `In3.Cu` `+3V3` planes through a
  1.4 mm, 0.4 mm stub and one via, and the nearest bulk ceramic is about 20 mm
  away across planes; that is analytically adequate for a PA whose current ramps
  over tens of microseconds, so it is NOT a pre-order defect.  This measurement
  is the first-article check of that argument, and a local 100 nF + 1 µF at each
  land is a Rev-B improvement.)*
...
status registers — `radios quiesced after MCU reset` must read `[PASS]` before
...
> **D-800 (Round-19 full review) — PREREQUISITE `FAP-01`, THE FIRST-ARTICLE TEST
> IMAGE — IMPLEMENTED AT D-801 (`D801-01`); BUILD AND BENCH EXECUTION PENDING
> HARDWARE.**  The RELEASE image (`[env:aqroot-demo]`) is a bring-up image and
> deliberately keys nothing: it has no console key that starts a CC1101 or SX1262
> transmission, turns the ST25R3916 field on, bursts Wi-Fi/BLE, holds the audio
> amplifier driving beyond its 400 ms tone, holds a backlight duty, or polls
> `VCELL` at 10 ms — and its bring-up tests are refused while an accessory rail is
> live.  The steps that need those states run on the separate FIRST-ARTICLE TEST
> IMAGE, PlatformIO environment **`aqroot-demo-fap01`** (`Firmware/src/fap01/`,
> define `AQROOT_FAP01_DIAGNOSTIC`), built and flashed ONLY BY NAME
> (`pio run -e aqroot-demo-fap01`, `pio run -e aqroot-demo-fap01 -t upload`) and
> recorded (git SHA and `firmware.bin` sha256) before FA01, FA05, FA06, FA07 and
> FA08 begin — procedure, console and identity record:
> **`docs/full-beta-v2/assembly/FAP01_FIRST_ARTICLE_IMAGE.md`**.  Its upper-case
> console keys: `C` CC1101 continuous transmit and `L` SX1262 CW at 22 dBm, keyed
> ONLY through `SpiBusB::beginTransmit`; `N` an NFC field keyed ONLY through
> `DemoBringupApp::beginNfcFieldSession`, after the 3.3 V supply mode (`sup3V`)
> is written and read back (D-802) (`T` one REQA, report only, an answer only
> from validated receive evidence, D-802); `V` + `W` a
> Wi-Fi TX burst (the release table's Wi-Fi row is asked and only its no-rail
> charging refusal is waived, after the operator declares no charger, both rails
> off, nothing else keyed — and, from D-802, an EXCLUSIVE session: every other
> key is refused until it stops); `I` a 38 kHz NEC burst from `D1`; `H` / `J` a
> chip-select hold-off for `U7` / `U9` (the pin held high while the driver
> believes it selected the part; it is carried into the NEXT FAP-01 boot of ANY
> kind — `EN` pulse, power cycle or reflash — so press `Q` and see `NVS
> confirmed clear` before power-off or reflash unless testing persistence
> (D-803)), `K` / `Y` a `U9`
> hold-off landing 700 ms / 2000 ms after the key; `A` a held audio drive at the
> capped level; `B` a held backlight PWM duty after the D-784 prime; `G` a 10 ms
> `VCELL` poll for 2 s; `Q` stops everything — every one under the SAME
> accessory permission table and quiesce/liveness rules as the release image,
> and every one BOUNDED (30 s radio, 60 s field and hold-off, 10 s Wi-Fi, 30 min
> holds).  It is never shipped and never a default
> (`checks/firmware_hw_map_contract.py` H9 / H10; its host tests run in H6).  It
> is a first-article deliverable, not a PCB/PCBA order item: nothing on the board
> changes.  Where a step below says "from the console" for one of those states it
> means this image's console.  Steps that need nothing from it (`C-GAUGE-EPOCH-01`
> keys `p`/`3`, `C-FW-ABORT-01`, `C-WARM-IMAGE-01`, the accessory steps) run on the
> release image.

| item | what it accepts | where the procedure is | raised by |
|---|---|---|---|
| **`Q11-TEMP-01`** | backlight at 0 / 25 / 40 °C after soak, and the D-784 full-duty prime (§7a; a HELD PWM duty needs the first-article test image `FAP-01`)  **OUTCOME:** PASS if, at 0, 25 and 40 °C, the backlight disconnect stays OFF with the gate held low, the full-duty prime completes, there is no visible dropout or flicker at full-on or at a held PWM duty, no TPS61169 open-LED latch, and ON→OFF→ON restarts are repeatable (§7a's acceptance, in full); otherwise RECORD + ESCALATE and re-run F5 with the measured Q11 conduction before acceptance. | §7a above | D-780 |
| **`C-PWR-TRANSIENT-01`** | `+3V3` peak < 3.300 V at the panel pins and trough > 3.000 V at `U1.2` and, in the radio step, at the transmitting sub-GHz module's `VCC` land (`U8.9` / `U7.9`), DC-coupled or synchronised DC+AC, uncertainty charged against the margin  **OUTCOME:** PASS if the peak stays below 3.300 V at the panel pins and the trough above 3.000 V at `U1.2` and at each transmitting sub-GHz module's `VCC` land with the uncertainty charged against the margin; otherwise RECORD + ESCALATE and re-run F6 and F11 on the measured excursion before acceptance. | §7b below | D-788 / R7-N02, rewritten at D788-18 |
| **`C-THERM-01`** | thermography at the 40 °C top of the declared ambient envelope, held at the HEAVIEST STATE THE PRODUCTION IMAGE ADMITS: `display_audio` — the display at full brightness, the audio amplifier driving at its capped level, and both accessory rails at the **DECLARED SIMULTANEOUS PAIR** (220 mA + 170 mA) — and MEASURED against the F12 model: battery current **1.5289 A**, BQ25185 junction **86.43 °C**, enclosure internal air **55.14 °C**, and the enclosure's own `R_SYS` = **3.2493 K/W**, which is a DECLARED allowance and is what this test exists to measure.  **D-795 RE-BASED THIS STEP AGAIN, AND FOR THE SAME REASON AS D-794: it named a state the technician cannot reach.**  D-794's `display_subghz` with both rails is refused by the production rail-edge table — no accessory rail may be live while a radio transmits — so it is kept only as the SUSTAINED SIZING ENVELOPE the conductors are sized against and is **not** a test state.  The state is HELD from the first-article test image (`FAP-01`): the release image's bring-up tests refuse to run while a rail is live and its tone is a 400 ms burst.  **ALSO record the CHARGE regime** with the named adapter attached, at `display_audio` + `acc_3v3_only` (**3.818 W**) — D-797: this state is inside the charging-safe envelope ONLY with the cell at or above a **4.10 V** cell, the envelope's row for it (below that cell the load exceeds what the input carries at SYS = VBAT and the charger can hold an ABSORBING supplement, which on the high source corner reaches TSHUT).  Take the charge record under the SUPERVISED CHARGING RULE (below), with `OCV_lb` at or above 4.10 V: RECORD the input FET, the charge FET and any BATFET supplement current, the package-top temperature and, separately, the junction interval derived from it, and the ambient at which the internal air reaches the pouch's own 40 °C charge window — DERIVED at **28.7 °C** for that state.  Any heavier charging observation, or this state with `OCV_lb` below 4.10 V, is OUTSIDE the admissible charging envelope: a bounded DIAGNOSTIC only, never an acceptance point.  Also record the `U11.2` narrow run.  **OUTCOME:** PASS if the measured battery current, internal air and `R_SYS` are at or below the modelled figures and the WHOLE BQ25185 junction interval derived from the package-top reading (package − 2 K … package + 5 × ΨJT (2.0 °C/W) × the package loss + 2 K; the junction is never read off the thermogram) is at or below the modelled junction — an interval that straddles it is INDETERMINATE / RECORD, neither a PASS nor a model contradiction; otherwise RECORD + ESCALATE and re-run F6, F12 and F14 with the measured `R_SYS` before acceptance.  RECORD ONLY, per observation (D-798 / `R17-06`): (r1) the charge record — RECORD ONLY unless its junction interval lies WHOLLY above the model's junction for that state (an overlap is INDETERMINATE / RECORD), or its internal air or battery current is worse than the model for that state, which is RECORD + ESCALATE and re-run F12 and F14 before any charging acceptance; (r2) the `U11.2` narrow run — RECORD ONLY unless its temperature rise is worse than the rail-ampacity model's, which is RECORD + ESCALATE and re-run F12 and F14 before acceptance; (r3) any diagnostic above the admissible domain — RECORD ONLY: no published assumption covers a state outside the envelope. | `audit_rail_ampacity.py`, `checks/demo_feature_contract.py` F12, `AQROOT_DEMO_FAB_HANDOFF.md` | D-788, re-based at D-790, D-791, D-794, D-795 and D-796 |
| **`C-BAT-GATE-01`** | `ΔVGATE` (GATE − `BAT_PROTECTED_P`), `VGS(Q2)` and the pass-pair drop (`BAT_RAW` − `BAT_SENSE`) on the fitted **`AO4800`**, every figure GENERATED FROM `demo_feature_contract` F10 (D-801 / `R20-05`).  **BENCH CONDITION (declared):** the assembled board OUT of the enclosure, horizontal, in still room air, at **25.0 °C** air; each current held until the Q2 package-top temperature changes by less than 0.5 K per minute (at least 5 min); `BAT_RAW` = 4.15 / 3.60 / 3.05 V from a bench supply in place of the pack (the LTC4368's guaranteed gate drive is the same **3.0 V** row across that range).  Draw the test current with an electronic load from `BAT_PROTECTED_P` (`TP15`) to `GND`, NOT through the system: the load then never passes `U11`'s BATFET, whose BATOCP could otherwise trip, and `U11`'s own battery cut-off near 3.05 V is irrelevant to the pass pair.  The pass-pair current is also the drop across `R75` (10 mΩ, `TP20` to `TP15`).  Read `VGS(Q2)` directly at `Q2`'s own leads (gate pin 2 or 4 to source pin 1 or 3).  At each current below record `ΔVGATE`, `VGS(Q2)`, the drop and the `Q2` package-top temperature, then step on in 0.05 A increments to **2.7000 A** and record the current at which `VGS(Q2)` reaches the AO4800's **2.5 V** row, if it does.  This is what converts the one thing NO candidate publishes — hot `RDS(on)` at `VGS` = 2.5 V — from an extrapolation into a measured bound.  The model's WORST-CASE prediction at this condition (minimum gate drive, the AO4800's MAX `RDS(on)` at its 2.5 V row, the DECLARED 25 → 125 °C ratio **1.60**): at **1.7745 A** (sustained thermal envelope) `VGS(Q2)` ≥ **2.6855 V**, drop ≤ **413.4 mV**; at **2.0929 A** (enclosure conduction ceiling) `VGS(Q2)` ≥ **2.6175 V**, drop ≤ **502.9 mV**; at **2.5783 A** (bench crossing) `VGS(Q2)` ≥ **2.5000 V**, drop ≤ **658.0 mV**; at **2.7000 A** (peak electrical envelope) `VGS(Q2)` ≥ **2.4671 V**, drop ≤ **701.4 mV**; the bench crossing is **2.5783 A**.  Also record the TURN-ON TIME from `SHDN` release to full enhancement: ADI specifies `IGATE(UP)` and `tD(ON)` only at `VIN` = 12 V.  The enclosure conduction ceiling (**2.0929 A** at **80.83 °C** internal air) is NOT a bench quantity: F10 re-derives it from the measured drive and resistance  **OUTCOME:** PASS if at every point the measured `VGS(Q2)` is at or above, and the measured drop at or below, the model's worst-case prediction for that point, and `VGS(Q2)` does not reach 2.5 V below the bench crossing of **2.5783 A**; otherwise RECORD + ESCALATE and re-run F10, F12 and F14 with the measured gate drive and drop before acceptance. | [`SOURCING_LEDGER.md`](SOURCING_LEDGER.md) | D-789 / `R8-N01`, part fixed at D-790 / `D789-A01`, re-derived from F10 at D-801 / `R20-05` |
| **`C-DISP-01`** | the display module's own `+3V3` draw, split into **panel logic** (`J1` `VCI` + `IOVCC` + the FT6236 touch controller) and **backlight converter input** (`U17` `VIN`), at full brightness and at the rail's heavy-load minimum.  The backlight half is DERIVED — `demo_feature_contract` `F6` solves it at **233.13 mA** from published maxima plus the four DECLARED loss allowances `D790-A04` requires (diode `Vf` at the inductor PEAK, switch transition and `Coss` at the MAXIMUM switching frequency, gate drive, and core loss + hot DCR) — and the panel half is a **DECLARED 50 mA allowance**, because ILI Technology publishes Sleep-in and Deep-Standby currents and **no active-mode supply current at all**.  The EastRising ER-TFT035IPS-6 module specification IS archived in the repository and is used for the backlight (110 mA TYP / 120 mA MAX); it publishes no panel-logic active current either, so the logic half stays DECLARED.  This test is what replaces that declaration with a measurement.  **HOW, ON A BOARD WITH NO BREAK IN `+3V3` (D-800):** `U12` feeds `+3V3` directly and there is no link to lift, so the panel-logic half is read through a 50-pin 0.5 mm FPC extension inserted between `J1` and the panel with a series shunt in `VCI` and in `IOVCC`, and the backlight INPUT by difference — the battery-path current through `R75` (`TP20` to `TP15`) at a fixed pack voltage with `U17` enabled and disabled, converted through `U12`'s efficiency measured on the same unit at that load — cross-checked against the LED current from the drop across `R69`  **OUTCOME:** PASS if the measured panel-logic draw is at or below the DECLARED 50 mA and the backlight input at or below the DERIVED 233.13 mA; otherwise RECORD + ESCALATE and re-run F6, F12 and F14 with the measured draw before acceptance. | `checks/demo_feature_contract.py` F6 `p3v3_internal_budget`, `AQROOT_DEMO_FAB_HANDOFF.md` | D-790 / `D789-A11` |
...
| **`C-ACC-01`** | delivered Community-Port potential at the J5 mating interface, each 3.3 V contact **alone** and with the header fully mated.  **SIZING / DIAGNOSTIC readings, NOT operating modes (D-803):** (a) `ACC_3V3_SW` at its single-rail 400 mA with `ACC_5V_SW` unloaded; (b) the shared-ground SIZING case, 400 mA with `ACC_5V_SW` also at 300 mA, held only for the reading — the full 400 mA + 300 mA pair is NOT a supported simultaneous mode; (c) the SUPPORTED simultaneous pair, **220 mA + 170 mA**, the only both-rails load the contract admits.  Normal operating authority is unchanged  **OUTCOME:** PASS if each 3.3 V contact alone and the mated header deliver at or above the published Community-Port minimum at 400 mA; otherwise RECORD + ESCALATE and re-run F6 and F12 with the measured potential before acceptance. | [`ACC_3V3_REINFORCEMENT.json`](ACC_3V3_REINFORCEMENT.json) | D-789 / D788-02 |
...
| **J4 fit / pull / thermal** | battery pigtail hole fit, retention and thermal acceptance | [`BATTERY_HARNESS.json`](BATTERY_HARNESS.json) `acceptance` and `strain_relief` (D-800: this read "§6"; the record has no numbered sections) | D-781 / D-782 |
