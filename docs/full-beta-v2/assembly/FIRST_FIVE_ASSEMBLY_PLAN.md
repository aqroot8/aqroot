...
| **`C-THERM-01`** | thermography at the 40 °C top of the declared ambient envelope, held at the **SUSTAINED REFERENCE STATE** — the display at full brightness, **ONE sub-GHz radio transmitting**, and the accessory rails at the **DECLARED SIMULTANEOUS PAIR** — and MEASURED against the F12 model: battery current **1.5591 A**, BQ25185 junction **88.18 °C**, enclosure internal air **55.64 °C**, and the enclosure's own `R_SYS` = **3.2493 K/W**, which is a DECLARED allowance and is what this test exists to measure.  **D-794 / `R13-06` RE-BASED THIS STEP AND THE REASON IS THAT HALF OF IT WAS UNEXECUTABLE.**  It used to say *both published accessory budgets* with this mode set, and F12 says that combination has **no operating point at any attainable cell voltage** — a technician could not have reached the state the acceptance was written against, and the three figures beside it were three derivations old.  The published PER-RAIL budgets are unchanged and each is still deliverable alone; what two rails may draw AT ONCE in this mode set is the declared pair, and that is the state this step holds.  **ALSO record the CHARGE regime**: with an adapter attached, the input FET, the charge FET and any BATFET supplement current, and the ambient at which the internal air reaches the pouch's own 40 °C charge window — DERIVED at **24.47 °C** for the heaviest permitted charging state.  Also record the `U11.2` narrow run.  *(D-789 framed this around a JEDEC θJA referenced to the EXTERNAL ambient; `D789-A02` replaced the frame.  D-790 then declared a reference state with BOTH radios transmitting that `D790-A02`/`D790-A03` show has no stable operating point at all; D-791 replaces the state.)* | `audit_rail_ampacity.py`, `checks/demo_feature_contract.py` F12, `AQROOT_DEMO_FAB_HANDOFF.md` | D-788, re-based at D788-03, at D-790 / D789-A02, and AGAIN at D-791 / D790-A02 |
...
| **`C-CHG-01`** | the USB SOURCE CONTRACT, at the pin it is defined at.  With the charger drawing its programmed **1.1 A** input limit, measure the potential at **`U11` pin 10 (`VIN`)** referenced to `U11`'s GND pad: it must be at or above **4.2581 V** with the supplied adapter and cable, and the programmed input limit is **1.1 A**.  Repeat with a deliberately poor cable (2 m, 28 AWG) and RECORD the value — it is expected to fail the contract, and that is the point: the charge-time ceiling is derived at the QUALIFIED source path and the published operator rule is *24 AWG or heavier, no longer than 2 m* | `hardware/demo/manufacturing/aqroot_power_model.py` `usb_source_contract()`, `checks/demo_feature_contract.py` F12 | D-793 / `R12-02` |
...
| **`C-PWR-CHARGE-01`** | the charge regime at its published ceiling: with an adapter attached and the system held at **3.600 W**, record `VIN` at `U11` pin 10, `VSYS`, the input current, the charge current and any BATFET supplement current, and the package temperature.  The corrected D-794 / `R13-02` solver puts the junction at **104.726 °C** there, with the ruling source class in the `ILIM` branch and NO supplement.  *(D-793 said "every ruling source class in DPPM"; the rebuilt branch set separates SYS regulation, the input current limit, VINDPM and DPPM into the four distinct physical loops the part has, and the ruling corner at this load is the INPUT CURRENT LIMIT.)*  **A SUPPLEMENT AT OR BELOW THE PUBLISHED CEILING IS A FAILURE OF THIS TEST**, because the whole ceiling exists to stay below that discontinuity.  **AND RECORD THE COMPLETION QUESTION SEPARATELY**: the charge-REGIME ceiling above guarantees only that the battery does not discharge and that `U11` stays inside its operating maximum.  The **charge-COMPLETION** ceiling — the system load below which a cycle delivers the rated capacity and terminates inside the BQ25185's 360 min `tMAXCHG` on every qualified cable — is **1.150 W**, and at the regime ceiling NO qualified source class completes.  Run the completion measurement at that load | `hardware/demo/manufacturing/aqroot_power_model.py` `charger_state()`, `checks/demo_feature_contract.py` F12 | D-792 / `R11-03`, re-based at D-793 / `R12-02` |
