...
## 2026-09-19 - AQROOT Demo: D-775 — normal 400/300 mA accessory concurrency derived from the live board and enforced by firmware

`authority 8c548ece, PCB UNCHANGED`. Independent CTO recheck correctly found that D-771–D-774 still treated D-098's simultaneous normal 400 mA + 300 mA load as an ideal-source calculation while firmware used one 3.50 V accessory floor. The first ~3.85 V correction estimate was conservative from the wrong reference point: MAX17048 `U14.2/U14.3` measures `/01_POWER_TREE/BAT_PROTECTED_P`, the same node as BQ25185 `U11.2`, so upstream pack/Q2/Q3/R75 loss must not be counted again. D-775 does not replace it with another hand-written floor: F6 independently measures four live PCB paths, applies hot-copper scaling, includes TI's BQ25185 140 mΩ max BATFET with a declared 1.40× low-VBAT/high-current allowance, includes TPS22950-Q1 RON and the D-772/D-773 load terms, then **solves** the VCELL needed for 10% headroom to BQ25185 OCP minimum on both live resistance and declared path ceilings. The worse dual result is **3.7622 V**, rounded upward to **3.80 V**; single-rail requirement is 3.1232 V, so the existing 3.50 V single floor remains. Firmware now enforces **3.50 V single / 3.80 V dual**, fails closed on unreadable VCELL, sheds 5 V first below the dual floor and all accessory power below 3.50 V. On the live board, simultaneous published loads at 3.80 V model **2.2499 A, 12.20% below** OCP minimum; individual 3V3/5V cases at 3.50 V retain 30.69% / 25.55%. F6 parses the actual firmware constants, proves gauge/BAT topology, remeasures all four paths and refuses floors or paths outside the derived envelope. The policy host test passes **23/23** and three C++ destructive controls are caught. Fresh DRC remains 199 library warnings / zero other classes, parity 246 warnings / zero errors; routing 173/174 with only owner-approved U11.3; F1–F8, FAB1–FAB15, ampacity and all 19 standing contracts PASS/non-vacuous; all four PlatformIO environments SUCCESS; `hardware/beta-v2` untouched. No PCB/schematic/BOM/CPL/Gerber/drill change.

## 2026-09-18 - AQROOT Demo: D-770 — DEMO_READY_FOR_FAB is re-declared

`authority 5849b658`. External review round 2 withdrew D-764's readiness for two items; **D-765 closed the first** (the unsupported `TPS22950C` `ILIM` setting, closed from `SLVSFJ2B` §5 with no resistor, copper or envelope number moved) and **D-766 closed the second** (firmware fault / warm-reset: safe latches before the 3 s USB-CDC wait, I²C recovery before `Wire`, and loss of the `U3` fault read forcing both disconnects where it previously issued **zero** shutdown writes). Closing them exposed a pattern — *the repository stating a requirement that nothing enforced* — which D-766…D-769 followed to **five more defects**: a **30 V `Q11` on the 39 V node this board publishes**, a GND barrel **0.200 mm from `L4`'s centre** where Würth prints "No vias and traces underneath", a `MAX17048` read **16× low** that would have refused accessory power forever, a power audit that **answered differently between runs** (`id(m)`, a memory address, as tie-break), the new `LAND8` itself **blind to rotated instances**, a **released BOM naming the wrong display**, and a **superseded antenna** on `U8`. All seven are fixed and **all seven classes are gated**. **The declaration is made because the searches came back clean, not because they stopped**: all ten sheets swept for imperative notes (27 found, nine measurable, **all nine holding**), every inductor tested (only DNP `L2` carries strip copper, and `LAND8` fails if it is ever populated), every semiconductor on the 39 V node rated, every archived vendor drawing swept. Connectivity **173/174** with 0 unapproved opens; DRC 199 `lib_footprint_issues` all warnings with zero other classes; parity 0 errors; protected copper identical; `D-186`/`D-269` dru contracts live and true; **F1–F7**, **LAND1–LAND8**, **MK1–MK10**, **FAB1–FAB15** PASS; 19 contracts none failing; four builds SUCCESS; `hardware/beta-v2` untouched. **No open owner decision and no unresolved fabrication blocker.** Residual risks are named in `CTO_DECISIONS.md` D-770 §5. Independent CTO review follows.

## 2026-09-18 - AQROOT Demo: D-769 — the other superseded part name, and the clause that now refuses the whole class

`authority 5849b658, UNCHANGED` — no PCB change and no copper of any kind. D-768 §7 deliberately left this one out rather than bundling it, and it is the same defect: **`U8`'s `Package` field, the sheet module note and `ARCHITECTURE.md`'s 915 MHz row all named the internal `Taoglas FXP890.07.0100C` flex** that **D-198 superseded**, and `DEVICE_SPEC` had carried an explicit *"This is STALE"* note about those exact fields as an open item. **The consequence if believed is concrete**: an `FXP890` is an internal flex with a u.FL connector, and buying it instead of the SMA whip and `CBA-UFLSMA20IP` pigtail leaves the 915 MHz radio with **no antenna path this product supports** — the enclosure has a Ø6.5 mm top-panel bulkhead for a screw-on whip and no provision for a flex. All three now name the locked **external `TI.92.2113`** dipole. **And `F7` was generalised**, because D-768 built it for one reference and that is the mistake D-767 named — a fix that closes one instance and leaves the class open is half a fix. `F7` is now a registry (`J1` → retires `CH280QV10`/`ILI9341`/`2.8in`/`240x320`; `U8` → retires `FXP890`), **every entry a supersession this repository already recorded**, asserting the locked identity in the placed symbol, its library definition **and the released BOM row**, with text explicitly about the retirement exempt. **Four live controls, all refused.** The BOM-row leg was proved load-bearing by walking into it at D-768: after the schematic was fully corrected `F7` still failed until the package was regenerated. **F1–F7** PASS, DRC/parity sets element-for-element identical, 19 contracts none failing, FAB1–FAB15 PASS, four builds SUCCESS. **`DEVICE_SPEC` §16 item 6 is CLOSED**, and with it the last board-adjacent documentation item in that section. Full evidence: `CTO_DECISIONS.md` D-769.

## 2026-09-18 - AQROOT Demo: D-768 — the released BOM named the wrong display, on the one connector whose pin table had already been dead on arrival for exactly that reason

`authority 5849b658, UNCHANGED` — no PCB change and no copper of any kind. **`aqroot-Demo-BOM-assembly.csv` and `-full.csv` carried, on the `J1` row, `"CH280QV10-CT Rev.D 2.8in 240x320 IPS TFT + CTP"`** — a **2.8-inch 240×320 ILI9341** panel this project retired, on the board whose locked display is the **3.5-inch 320×480 ILI9488 `ER-TFT035IPS-6`** (D-074). This is not cosmetic: **D-112 already found what believing that string costs** — it replaced the display symbol because the `CH280QV10-CT` pin table is **dead on arrival here in two independent ways** (panel pin 1 is `LEDA` with `LEDK` on 2/3, so the backlight would have been **reverse-biased**; pins 36/37 are `WRX(SCL)`/`D-CX` and were **reversed**, so the panel would **never have received a valid command**) — and recorded that *"neither fault is visible from a pin count, a connector MPN or an ERC run."* D-112 fixed the pins and the `Description` and **left the retired panel's name in the `MPN` and in the provenance sentence of the same symbol**, whose `Package` field credited *"SPEC-CH280QV10-CT_Rev.D pages 6-7. TFT driver ILI9341V"* one field below a `Description` that said ILI9488. **The board was and is right** — `J1.1`=`LED_A`, `J1.2/3`=`LED_K_PANEL`, `J1.36`=`SPI_A_SCK`, `J1.37`=`DISP_DC`, checked pin by pin before anything was changed. Every place that names the panel now says `ER-TFT035IPS-6`, **tersely** — the first attempt wrote the decision history into that field and **`F7` refused it**, correctly: a released BOM column is not the place to quote the wrong panel's name even to disown it. The two dead display symbols left in the library are annotated **`RETIRED -- DO NOT INSTANTIATE`** with D-112's faults spelled out, kept rather than deleted. **`F7`** asserts the locked identity in the placed symbol, its library definition **and the released BOM row** — and checking the schematic alone would have passed a board whose shipped BOM was wrong, which is exactly what happened: `F7` still failed after the schematic was fixed, until the package was regenerated. Three live controls, all refused. **F1–F7** PASS, DRC/parity sets element-for-element identical, 19 contracts none failing, FAB1–FAB15 PASS, four builds SUCCESS. **`DEVICE_SPEC` §16 item 5 is CLOSED.** Full evidence: `CTO_DECISIONS.md` D-768.

## 2026-09-18 - AQROOT Demo: D-767 — the same two defect classes, asked of the whole board: a second inductor nobody checked, a second semiconductor on the 39 V node, and an arithmetic error in D-766 itself

`authority 5849b658, UNCHANGED` — no PCB change; the released package is UNCHANGED IN CONTENT: every Gerber, drill, CPL and BOM file is identical once the embedded `CreationDate` is normalised, and the ONLY semantic change in `MANIFEST.json` is the recorded sha256 of `03_spi_a_display_sd.kicad_sch`, the sheet that carries the corrected `Q11` note. **`FAB1` refused the package until that manifest entry was regenerated**, which is the provenance clause working exactly as intended. D-766 fixed four specific defects, and each was an INSTANCE of something general. **`LAND8` tested `L4` by name**, and the board carries **six** inductors — one of which, **`L2`, is the same Würth `74438357010`**, with **two `/NFC_5V_EN` segments running the full length of its restricted strip**. `L2` is **DNP**, so no fabricated board has a part over that copper and it is not a blocker; rerouting through a 1.39 mm channel bracketed by `L2`'s own live pads, and re-certifying the release, to cure a defect on an unpopulated part was **declined**. The gate carries it instead: `LAND8` now enumerates **every instance** of an explicit table of restricted footprints, **fails** on a fitted one and **reports** a DNP one with coordinates, and states its own coverage limit. **And D-766's `LAND8` had a bug**: it took the strip from `pad.GetSize()`, which is in the pad's own frame, so a **rotated** instance was measured across the wrong axis and returned a negative-height rectangle that collided with nothing and passed — `L1`, the `TPS63020` inductor and the highest-current one on the board, sits at **90°** and returned `0.980 × −1.030 mm`. Measured from pad bounding boxes its strip is `3.400 × 1.390 mm` and clean, and a **second live control** derives it every run. **The 39 V node has two semiconductors**: `D8` stands off the same ceiling in reverse. It is **retained and is not a defect** — `NSR0240`, `VRRM` 40 V, and the part **TI's own SNVSA40B §7.2.2.2 names for this converter** — but nothing could say so, the margin is **1.0 V on an absolute maximum**, and `FIRST_FIVE_ASSEMBLY_PLAN` already records a live substitution trap on that exact reference. `F5` now reads `D8` against a published-`VRRM` table and **reports the 1.0 V every run**; it runs **ten** live controls. **And D-766's own arithmetic was wrong**: it claimed *"roughly 10 V of spare headroom"*, but `LED_BOOST` runs at **3.884–4.484 V** in normal operation because the backlight is six LEDs **in parallel**. The conclusion survives with better reasoning — `LED_BOOST` is a **regulated** node whose range runs to the 36 V OVP, and `RDS(on)` is now bounded by scaling the **published** 200 mΩ @ `VGS` 2.5 V to our 0.396 V of overdrive: ≈ 253 mΩ, **27.6 mV and 3.0 mW**, with the design provably insensitive even at a pessimistic 10 Ω. The correction is **marked**, not silently rewritten. Recorded once more: `Q11`'s **D-750** note already said *"ANY FUTURE REVISION THAT SEPARATES THE TWO CONTROLS … MUST RE-RATE `Q11` TO AT LEAST 40 V"* — D-752 separated them and nobody re-rated it. **A note is not a gate.** Full evidence: `CTO_DECISIONS.md` D-767.

## 2026-09-18 - AQROOT Demo: D-766 — a 30 V part on a 39 V node this board publishes, a via where the manufacturer prints "no vias", a warm reset that outlived its own safe state, a trim that met its limit exactly, and a power audit that answered differently every other time it was asked

`authority 9e4728ae -> 5849b658`. **D-752 wrote "THE `AO3400A` IS A 30 V PART" in bold, in three files, and shipped the 30 V part** — while this board's own `.kicad_dru` says *"an open-LED fault puts up to **39 V** on `LED_BOOST`"* and the whole `LED_BOOST` clearance regime exists because of it. `Q11`'s drain is the panel cathode. D-752's `D14`/`C85`/`R132` ordering is sound, but **any one of those three failing re-creates the state it removed**, and a protection element must survive the fault it exists to prevent. `Q11` → **`AO3422`** (LCSC `C37130`, `BVDSS` **55 V** min, **41 % margin**, same SOT-23, same 1=G/2=S/3=D, so no land pattern moved). Conduction is guaranteed by the threshold spec itself — `VGS(th)` is specified at 250 mA and this circuit needs 109 mA — and `RDS(on)` cannot move the LED current because `FB` senses `Q11`'s **source**. **The one number that gets worse is named**: `VGS(th)` max 1.45 → 2.00 V cuts the ordering margin **4.6× → 2.06×**, accepted because the 55 V part turns a violation from avalanche into a **recoverable** `U17` open-LED latch. `F5` gained the clause it never had — it **parses the 39 V ceiling out of the `.kicad_dru`**, refuses a FET with no published rating, and checks `VDS`, held-gate enhancement and decay-to-threshold against the **fitted** part — with four more live controls, one of which is the board D-752 shipped, refused by the rating clause alone. **Würth 74438357010 prints "No vias and traces underneath the inductor" under both recommended land patterns**, and the board had a Ø0.8 mm GND barrel 0.200 mm from `L4`'s centre with 0.6 mm of track feeding it; all three objects removed, `LAND8` added with the restricted strip **derived from `L4`'s own pads** and a control that refuses the previous board by coordinate. The relocation was **measured and declined**: eight legal sites exist, the best one comes back `via_dangling`, and `U21.4`/`C65.2`/`C66.2` and three barrels are already **one connected 161.735 mm² B.Cu GND island**, so what was deleted was a ≈ 2.6 nH stub in parallel with a wider pour. **Firmware closes external review round-2 item 2**: safe latches now precede the 3 s USB-CDC wait, I²C bus recovery precedes `Wire`, losing the `U3` fault read forces both disconnects and blocks re-enable — and an off-by-sixteen `MAX17048` `VCELL` read in that very work, which would have refused accessory power forever, was caught before it shipped. `J4`'s trim goes 0.80 → **0.50 mm** with a new **`J4-T3`** ≤ 0.10 mm polyimide requirement, retiring a zero-margin declaration on raw battery positive. **The parallel `F6` rewrite is refused and D-765's kept**: it deleted D-753's two fault-envelope clauses and accepts the D-750 values D-753 measured as tripping the pack. Promotion **16/16**, 0 added / 3 removed (all GND, all claimed); routing 173/174 with 0 unapproved opens; DRC, parity and unconnected sets **element-for-element identical** to D-765; protected copper identical; ampacity `all_ok` with every verdict and worst-rise unchanged — **and running it twice exposed a fifth defect**: `audit_rail_ampacity` broke maximin frontier ties on `id(m)`, a **memory address**, so the reported path length and resistance of `USB_VBUS_RAW` and `BAT_PROTECTED_P` changed between runs on an unchanged board (23.876 / 19.876 / 23.876 mm). The bottleneck and every verdict were always right; the tie-break is now a deterministic node index and four consecutive runs are byte-identical. **F1–F6**, **FAB1–FAB15**, **LAND1–LAND8** PASS; 19 contracts run, none failing; four PlatformIO builds SUCCESS. **Readiness is NOT re-declared in this entry** — see `CURRENT_STATE.md` §1. Full evidence: `CTO_DECISIONS.md` D-766.

## 2026-09-18 - AQROOT Demo: D-765 — the accessory envelope was right and the silicon could not legally hold it

`authority 1a06b058 -> 9e4728ae` (copper byte-identical). External review round 2 withdrew D-764's readiness for two items; this closes the first. D-753's reasoning stands — on a board with no accessory current measurement only the load switch's own limit bounds an external accessory — and its 2.7 kΩ / **0.407 A** setting is retained unchanged. **The part could not legally be set that low:** TI `SLVSFJ2B` **section 5**, the Device Comparison Table of the datasheet D-753 itself cited, gives the fitted `TPS22950C` an `ILIM` range of **0.5–3.5 A**; the 0.05 A floor and the "certified from 66 mA" UL note belong to the base `TPS22950`, **which TI sells only in a WCSP package this board cannot use**. Three places in this repository already held the right number — `POWER_FAULT_STATE_TABLE.md` says *"treat 500 mA as the floor, not the target"* and D-753 then set 407 mA — and `F6` had **no clause that could express the act at all**, because it never read the limiter's part number. `U20`/`U22` are now the **`TPS22950-Q1`** (`SLVSGP6A`, orderable `TPS22950CQDDCRQ1`, LCSC `C17349276`, Active/Production, 4 050 in stock): same `DDC0006A` land pattern, same pinout, same `ILIM` equation and EC rows, same auto-retry, same always-on true RCB, same `FLT` semantics, same 170 °C TSD, **`ILIM` specified 0.05–3.5 A**, AEC-Q100 grade 1, ≈ US$0.03 per device. **No resistor, no copper and no envelope number changes** — `F6` reproduces D-753's four modes to the digit and the `.kicad_dru` **parsed** class table is proven byte-identical. `F6` gained three clauses and eight controls; `f6e` is the board D-753 shipped and is refused by the new range clause alone. Walking into a library rename also exposed that **`kicad-cli sch export bom` exits 0 and silently omits an entire sheet**, which had made `routing_ledger` report 114 nets instead of 174; the ledger now reconciles its population set against the board. Promotion **16/16** with 0 objects added or removed, routing **173/174** with zero unapproved opens, DRC 199 `lib_footprint_issues` all warnings, parity 246 warnings / 0 errors, protected copper identical, **F1–F6** and **FAB1–FAB15** PASS, 19 contracts run, four PlatformIO builds SUCCESS. **Readiness is NOT re-declared:** the review's firmware fault / warm-reset item remains open. Full evidence: `CTO_DECISIONS.md` D-765.

## 2026-09-18 - AQROOT Demo: D-742 .. D-745 — the owner decision spent, an inverted charger decode corrected, a charger that could not complete a charge fixed, and the first absolute power / feature gates built

`authority 23ee647e -> c7f5c618`. The owner approved leaving `/BQ25185_STAT2`'s `U11.3` unconnected and **D-742** spent it, publishing `unapproved_open_edges: 0` beside an unchanged `retained_open_edges: 1` with the declaration re-proved against the board every run. Reading the datasheet to spend it found the board's **charger status decode was INVERTED on `STAT1`** — `STAT1` LOW is a FAULT, not "charging" — corrected in both schematic sheets and four documents. **D-743** then found the real defect: SLUSF65B rev B halved `tMAXCHG` to 360 min, so `R37`'s 1 kΩ / 300 mA could deliver only 1800 mAh against a 2500–3000 mAh cell and the board **could not complete a charge**; `R37` → 390 Ω (769 mA) and `R36` → 13 kΩ (ILIM1100, `VBATREG` unchanged at 4.2 V) fix it with 21–34 % timer margin, and `audit_rail_ampacity.py` — the first ABSOLUTE power audit this board has had — measured the charger-input path at 250.8 mm / 440 mΩ, proved no shorter or wider path exists on any layer, and recorded the one named ampacity exception with a length budget and five controls. **D-744** found `min_connection` at 0.000 mm, so KiCad's `connection_width` test had never run either; probed, classified (74 of 81 pairs are benign acute throats) and proved **no net depends on a sub-0.15 mm contact** — plus zero orphan pour islands, NFC decoupling measured, and `M-09` closed as bounded. **D-745** built the sixteenth contract, the only one that knows what the product is: 37 Demo-scope features, 59 references and 103 nets asserted absolutely, with five controls. Real KiCad DRC unchanged in every class, 16/16 contracts PASS, `FAB1..FAB11` PASS. Full evidence: `CTO_DECISIONS.md` D-742 … D-745.
