...
| **3.3 V accessory rail** | `+3V3` -> **`TPS22950-Q1`** -> `ACC_3V3_SW`. Default OFF, external 100 k pull-down mandatory, RCB, auto-retry, TSD, `FLT`. `R_ILIM` **1.78 k -> 0.636 A typ, 0.428 A GUARANTEED / 0.849 A worst case (D-771; was 2.7 k / 0.277 A at D-753)**; the limiter IS the envelope because the board has no accessory current measurement, and D-771 set it so the rail GUARANTEES the **400 mA TOTAL** D-098 publishes for it -- 2.7 k guaranteed only 0.277 A. Silicon corrected to the `-Q1` by **D-765**: the `C` variant is only specified from 0.5 A. **D-787 ADDED THE DELIVERY HALF.**  Guaranteeing a current is not guaranteeing a voltage: at the published 400 mA the routed `U20`->`J5` copper alone could not hold the Community Port's published **3.135 V** minimum.  Each duplicated 3.3 V contact now also receives an independent manual **28-AWG** first-five reinforcement lead from **`TP12.1`, downstream of `U20`** — so the limiter and its OFF disconnect stay in series — accepted at **<=30 mOhm** finished resistance each, which with the bounded hot `U20`->`TP12` copper and `U20`'s 68 mOhm maximum RON delivers **3.146366 V minimum at 400 mA**.  The traveler is `assembly/ACC_3V3_REINFORCEMENT.json`. |
