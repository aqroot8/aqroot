...
|---|---|
| **TFT / touch** | **LOCKED (D-074…D-078).** **EastRising `ER-TFT035IPS-6`** 3.5″ IPS 320×480, **ILI9488** (COG), with **`ER-TPC035-6`** capacitive touch, **FocalTech `FT6236` @ I²C 0x38**. Assembled outline **56.54 × 84.96 × 3.95 ± 0.25 mm**, active 48.96 × 73.44 mm. **One 50-pin FPC — 0.50 mm pitch, bottom contact, 0.30 ± 0.03 mm thick, 25.5 ± 0.15 mm wide** — carries display *and* touch (touch on pins 44–47). **`J1` = Hirose `FH69-50S-0.5SH`** (top *and* bottom 2-point contact, accepts 0.30 ± 0.05 mm FPC), laid out on the **FH12/FH52E standard land pattern** **~~so `FH52E-50S-0.5SH` (LCSC C7465440) is a drop-in second source~~ - THAT CLAIM IS FALSE AND IS STRUCK (FBV2-S2-001, D-194).** Both Hirose land patterns were read: **FH69 layout depth 7.38 mm** with a 0.30 x 1.23 signal land, hold-downs 0.36 x 4.25 at 28.73 c/c, and **top-and-bottom two-point contact**; **FH52E is 4.6 mm deep, BOTTOM CONTACT ONLY**, front-flip actuator, and the FH52 catalogue states its pattern is interchangeable with the **FH12**. **They cannot share pads. `J1` keeps its dedicated FH69 footprint and is MANUAL ASSEMBLY for the first five.** ST7796S was preferred and does not exist in a documented CTP module (D-078); the cost is **+50 % SPI-A traffic**, 46 ms full frame at 80 MHz. Backlight: **6 LEDs in parallel, one anode**, 2.9–3.2 V, 120 mA max / 90 mA life point — `U17` TPS61169 retained from `+3V3`, **`R69` = 1.87 R**, **`R70`–`R73` = 4 × 33 R in parallel** (D-079). |
...
| **915 MHz LoRa** | `U8` Ebyte E22-900M22S (SX1262) on SPI-B | Module IPEX → `CBA-UFLSMA20IP` pigtail → top-panel SMA(F) bulkhead → **EXTERNAL Taoglas `TI.92.2113` SMA(M) dipole** (D-198). *Corrected at D-769; this row said "Taoglas FXP890 flex", the internal option D-198 superseded.* |
| **NFC** | **RESOLVED AND FITTED.** `U9` **`ST25R3916-AQET`** (LCSC C5267441, D-126) on SPI-B, `Y1` 27.12 MHz crystal, full differential matching network, **`NFC_SUPPLY` = `+3V3` on the first build** with the DNP 5 V boost branch preserved behind two mutually exclusive 0 Ohm links (D-055/D-056). **P-04 CLOSED as stale at FBV2-S2-001.** **`U9` AND ITS TWELVE MANDATORY DECOUPLING CAPACITORS HAD STILL BEEN MARKED `DNP` AND WERE FITTED AT FBV2-S2-001 (D-192)** - the board would otherwise have carried a finished 13.56 MHz front end with no chip on it. Six pins stay unconnected by design: `CSO`, `EXT_LM`, `AAT_A`, `AAT_B`, `CSI`, `MCU_CLK`. **~~B-54 OPEN~~ CLOSED 2026-08-23 (FBV2-S2-002, D-205): DS12484 Rev 3 Table 121 gives `I_AL-AM` = 26 mA MAX for the IC with all blocks active, and D-134's actual first-build network draws ≈ 60 mA at the driver, so the `+3V3` allocation with the field on is **100 mA** — not the 350 mA / 500 mA ABSOLUTE MAXIMA, which were deliberately not used. TPS63020 sits at 63–71 % of 2 A. BINDING GUARD RAIL: a `C_s` move to 270 pF would draw ≈ 257 mA and requires the rail budget to be re-run first.** The EMC inductors `L5`/`L6` are now **Murata `LQW18AN39NG80D`** (B-70 closed, D-204) and `Y1` is **locked**, not a candidate (D-202). | **Taoglas `FXC.46.52.0075X.B.dg`** flex + ferrite, **B variant locked** (D-131), via `J7` |
                              ...
                              SW9 hard switch ─> TPS63020 EN
                                    ...
                                    MCU, radios, display logic, I2C, audio, NFC VDD_IO
                                        ...
                                        (NFC PA rail) TPS61023 boost ─> NFC_5V_PA  [DNP on build 1]
                                                      |
                              BQ25185_SYS ─> TPS61023 boost 5.0V ─> TPS22950-Q1 ─> ACC_5V_SW
                              +3V3        ─> TPS22950-Q1 ───────────────────> ACC_3V3_SW
```
...
| 3V3 rail | `U12` TPS63020DSJR | Buck-boost. **D-787 RE-DERIVED THE SETPOINT AND MOVED THE DIVIDER.** FB was 1M / 180k 1 % and every accessory clause ran on a typed 3.3 V; it is now **1M / 178k, both Viking `ARG03B` 0.1 % / 25 ppm** (`ARG03BTC1004` / `ARG03BTC1783`) and `F6` derives the rail from TI `SLVS916I`'s 495/500/505 mV feedback band, the fitted tolerance, the selected-part TCR over -40..+85 °C, 0.5 % line and 0.5 % load regulation, and the +5 % power-save high side: **3.261337 / 3.308989 / 3.357013 V** raw PWM, **3.228806 V** heavy-load minimum, **3.542487 V** power-save high against a 3.600 V tightest internal-consumer maximum. EN driven by the physical switch, **never firmware** — the MCU cannot restore its own disabled rail. |
| Fuel gauge | `U14` MAX17048G+T10 | On internal I2C at 0x36 (**carried, not datasheet-cited — B-60**). ALRT reaches a test point only. |
| **Reverse polarity (main path)** | **LTC4368-1 + P2: TWO back-to-back N-FET stages in TWO SEPARATE PACKAGES** (4 FETs) + R_SENSE **10 mΩ** (D-771; 15 mΩ until then) + R_GATE 22 kΩ + **C_GATE 4.7 nF** + OV divider + RETRY→GND + SHDN pull-up to VIN + FAULT + **≈5 A backstop fuse** + secondary clamp | **Single-FET-short tolerant by isolation** (D-068 met). VIN on the **cell side**. `-1` suffix load-bearing: `-2` trips at −3 mV and blocks charging. **Forward trip 3.960 / 5.000 / 6.061 A** — ADI guarantees `ΔVSENSE,F` as 40/50/60 mV over temperature, so D-771 moved `R75` to 10 mΩ to put this LATCHING breaker entirely ABOVE the BQ25185's recoverable `IBAT_OCP` band (2.5625–3.6875 A), which at 15 mΩ it overlapped. **UV deliberately UNUSED** (510 kΩ to VIN) — using it would deepen the dead-cell lockout. Stages must not share a package: two die on one leadframe are not independent. |
...
| Switched accessory rail (3.3 V) | **`TPS22950-Q1`** (was `TPS22950C`; D-765) | Replaces TPS22918, which has no reverse blocking, no current limit and no thermal shutdown. `VIN` 1.8-5.5 V, RCB, `ILIM` 0.5-3.5 A, auto-retry, TSD 170 C, `FLT`. `R_ILIM` 1.5 k recommended (D-086). |
| **Accessory 5 V rail (NEW)** | **`TPS61023` (2nd instance) + `TPS22950-Q1` (2nd instance)** | `SYS` -> 5.0 V boost -> protected switch -> `ACC_5V_SW`. Separate from USB VBUS and from the NFC fallback. Loads `SYS`, **not** `+3V3` (D-087/D-088). |
...
| **Connector** | **BUILT 2026-08-23 (FBV2-S1-009, D-179).** **Samtec `BCS-112-S-D-HE`** - .100 in / 2.54 mm, 2x12, **FEMALE** Tiger Claw dual-beam receptacle, **horizontal (right-angle) entry**, through-hole, **30 uin gold**. **ACTIVE**, 385 pcs next-day from Samtec, MOQ 1, $7.31 @ 1. Body **30.48 x 8.13 x 5.33 mm**. **4.6 A/contact**, 450 VAC, -55...+125 C. Mates with any 0.64 mm square-post header whose **mating post is 4.34-6.35 mm** (reference: `TSW-112-07-L-D`, 5.84 mm post). **LOCKED (D-093).** `BCS-112-L-D-HE` (10 uin gold) is a plating-only cost-down alternate, no board change. **Harwin `M20-7881242` is REJECTED as obsolete.** |
...
| Footprint | **BUILT AND VERIFIED 2026-08-23 (D-179): `AQROOT_Beta:Samtec_BCS-112-S-D-HE`.** 2 x 12 PTH, **2.54 mm within a row, 7.87 +/-0.05 mm BETWEEN rows, 0.71 mm drill**, pin field **27.94 mm**, taken from the Samtec RECOMMENDED PCB LAYOUT **REVISION B, FIG 3 (`BCS-1XX-XXX-D-HE-XXX`)**, re-read in this task. Odd = row A, even = row B, verified pin by pin against the netlist; pad 1 rectangular. Body 30.48 x 8.13 x 5.33 mm, courtyard 31.48 x 9.13 mm. **Not** interchangeable with a vertical 2x12 pattern, whose rows sit 2.54 mm apart (B-29 CLOSED). **If JLC cannot place a through-hole part automatically it becomes MANUAL/SECONDARY assembly for the first five boards.** |
...
| **3.3 V accessory rail** | `+3V3` -> **`TPS22950-Q1`** -> `ACC_3V3_SW`. Default OFF, external 100 k pull-down mandatory, RCB, auto-retry, TSD, `FLT`. `R_ILIM` **1.87 k -> 0.6032 A typ, 0.4058 A GUARANTEED / 0.8049 A worst case (D-791; was 1.78 k at D-771 and 2.7 k / 0.277 A at D-753)**; the limiter IS the envelope because the board has no accessory current measurement, and D-771 set it so the rail GUARANTEES the **400 mA TOTAL** D-098 publishes for it -- 2.7 k guaranteed only 0.277 A. Silicon corrected to the `-Q1` by **D-765**: the `C` variant is only specified from 0.5 A. **D-787 ADDED THE DELIVERY HALF, AND EVERYTHING IT CONCLUDED HAS SINCE BEEN REPLACED — the 3.135 V minimum, the 28-AWG reinforcement leads, U20's 68 mOhm RON and the 3.146366 V delivered figure are ALL RETIRED.**  Guaranteeing a current is not guaranteeing a voltage, which is the part that stands.  CURRENT (D-792): the ILI9488 panel's 3.3 V ABSOLUTE MAXIMUM on VCI/IOVCC makes a 3.3 V -5 % connector minimum unreachable at any current, so the owner-approved D-788 Option A contract publishes **2.80 V** at the J5 mating interface at 400 mA in the worst permitted wiring; **NO manual conductor is fitted** (D-789 removed the last one); `U20`'s RON is the guaranteed **116 mOhm** at the nearest published row at or below its own input voltage; and the two rails TOGETHER are the **declared simultaneous 220 mA + 170 mA** pair.  `assembly/ACC_3V3_REINFORCEMENT.json` is retained as the RETIRED traveler.  The authority is `DEVICE_SPEC.md` §6.3a. |
| **5 V accessory rail** | **RE-DERIVED AND SEQUENCED 2026-08-23 (FBV2-S1-009, D-185/D-186).** `BQ25185_SYS` -> **`U21` TPS61023DRLR** at 5.0 V nominal.  **D-773 CORRECTED THIS SETPOINT.**  It read *"setpoint re-checked: `R99` 732 k / `R100` 100 k, VREF 0.6 V -> 4.99 V"*, and **0.6 V is not what TI publishes**: `SLVSF14B`'s EC table gives the FB reference as **580 / 595 / 610 mV** in PWM mode, typ **595 mV**.  Over `R99`/`R100` at their own 1 % bands the real setpoint is **4.742 / 4.950 / 5.165 V**, and the worst-case high is what the accessory envelope must be costed against.  It clears the part's own `VOVP` minimum of 5.5 V by 6.1 % -> **`U22` TPS22950CQDDCRQ1 (TPS22950-Q1, D-765)** -> `ACC_5V_SW`. **D-787 MOVED IT AGAIN.**  `R_ILIM` **2.43 k -> 0.4555 A typ, 0.3065 A guaranteed / 0.6078 A worst case (D-787; 2.37 k / 0.624 A at D-773, 2.32 k at D-771, 2.7 k / 0.277 A at D-753)**, set so the rail GUARANTEES the **300 mA TOTAL** D-098 publishes for it (+2.16 %) while the limiter's worst user-reachable state stays under the pack's own minimum trip (+1.49 %).  The window moved because `F6` stopped costing the main 3.3 V rail at a typed 3.3 V and now derives it from `R39`/`R40`; 2.43 k is the **E96 value nearest the centre of the corrected legal window, 2.367-2.479 k**. **Verified from the netlist to be electrically independent of USB `VBUS` and of the NFC 5 V fallback**; only `SYS` and the device family are shared. Peak inductor current **2.19 A** at V_SYS 3.0 V, so **`L4` I_sat >= 3 A must be confirmed at BOM lock (B-68)**; 3.7 A valley switch limit; **true input-to-output disconnection in shutdown**. **THE ENABLE IS SPLIT: `ACC_5V_BOOST_EN` on `U3` P13 and `ACC_5V_SW_EN` on `U23` P04**, each with its own 100 k pull-down (`R102`, **`R131` new**), giving two independent series disconnects and letting the boost start into a known 44 uF instead of an unknown accessory. Sequence: detect -> 3.3 V -> **>= 5 ms** -> boost -> **>= 5 ms** -> 5 V switch; reversed on power-down. **The 5 ms is 7x the 700 us TYPICAL soft start, which has no published maximum (B-69).** Historical text follows. Not USB VBUS, not the NFC fallback rail, tied to neither. `R_ILIM` **1.65 k recommended (~0.69 A typ), not locked**; **published 300 mA continuous** on build 1. Inductor 1 uH, I_sat >= 3 A (D-087). **It loads `SYS`, not `+3V3`, so it consumes none of the TPS63020's 2 A budget.** **O-3 REJECTED: no link of any kind to the NFC fallback** (D-095). |
| BOM consolidation | One `TPS22950-Q1` MPN on both rails (D-088, silicon set by D-765); `TPS61023` reused from the NFC fallback with identical passives (D-088). |
| Expanders | **NXP PCAL9535APW,118** on `U2`, `U3` **and `U23`** (D-061, D-165). **O-6 RATIFIED 2026-08-23 (D-175): `U23` and the front RGB are LOCKED architecture and B-37 IS RETIRED** - 37 of 48 expander pins used, **eleven spare** plus `RESERVED_SPARE`. **`U23` P04 = `ACC_5V_SW_EN` since FBV2-S1-009 (D-186).** **CAPTURED 2026-08-23 (FBV2-S1-008).** **`U23` = a THIRD device at `0x22`** carrying only the front RGB and the `RESERVED_SPARE`, added because the two-expander allocation genuinely fails - **35 committed signals against 32 pins** - and every escape is closed: zero free native GPIO (B-10) makes the WS2812 route impossible, D-094 mandates the spare, D-082 locks the ten XGPIO. It adds **no new MPN, no new footprint, no new driver and no new rail** and **retires B-37** with 12 spare I/O. **O-6: awaiting CTO ratification.** **`U3` final: `XGPIO0-9`, `ACC_3V3_EN` P12, `ACC_5V_EN` P13, `ACC_DETECT_N` P14, `ACC_POWER_FAULT_N` P15, `SX1262_RXEN` P16, `ACC_PWR_EN` P17 = 16/16, zero spare** - `RESERVED_SPARE` moved to `U23` P03 because `ACC_PWR_EN` is retained pending the sheet-09 O-4 ruling. **`U2` = 16/16, zero spare.** Historical note: **`U3` = 15 assigned + 1 `RESERVED_SPARE`** (D-094): `XGPIO0-9`, `ACC_3V3_EN`, `ACC_5V_EN`, `ACC_DETECT_N`, **`ACC_POWER_FAULT_N`** (wire-OR of both `FLT`), `SX1262_RXEN`. **`U2` = 16/16, still zero spare** (B-37, half closed). |
| External I2C | **REBUILT 2026-08-23 (FBV2-S1-009, O-4 APPROVED, D-176): `U16` is now TI `TCA4307DGKR` (LCSC C880333), FITTED.** The port is inherently HOT-PLUG and the external segment is 3.3 V ONLY, so the TCA9517A's level translation was never used while its hot-insertion and stuck-bus weaknesses were. SCPS270B: IN is not joined to OUT until a **STOP or bus-idle**; **1 V precharge** on all four SDA/SCL pins; **stuck-bus disconnect at `tSTUCKBUS` 25 ms MIN / 40 typ / 65 MAX** followed by **up to 16 pulses on SCLOUT**; **powered-off high-impedance I2C pins**; **400 kHz max - FAST MODE, NOT 1 MHz**. `VCC` = `ACC_3V3_SW`, `EN` = `ACC_PWR_EN` held low by `R17` 100 k. **O-4 IS CLOSED AND THE CIRCULAR DEPENDENCY IS BROKEN** - a wedged accessory no longer needs the MCU to command the expander over the bus that is wedged. **ACCESSORY RULE (D-177): never hold SDA or SCL low longer than 25 ms.** External pull-ups **`R49`/`R50` 1.5 k, FITTED** - the inherited 4.7 k gave 796 ns against a 300 ns fast-mode budget at 200 pF and could never have worked (D-181). Historical text follows. Because that rail is now default-OFF and detect-gated, a dead accessory can no longer hang the internal bus. **Address collision remains open (P-18) — and no buffer of any kind solves it; see [`I2C_ADDRESS_REGISTRY.md`](I2C_ADDRESS_REGISTRY.md) (D-142).** **`U16`, `R49`/`R50`, `U15` and `D2`/`D3` are ALL DNP**, so nothing external is fitted today and the Sheet 09 choice costs no rework. TI SCPS245E: *"the TCA9517A logic and all I/Os are powered by the `VCCB` pin"*, so a de-asserted `ACC_3V3_SW` leaves it unpowered and high-Z on both sides. **The weakness is that its disable control, `ACC_PWR_EN` = `U3` P17, sits behind the bus it protects. O-4 (open): evaluate a TCA4307-class hot-swap buffer with stuck-bus recovery at Sheet 09 migration.** **Address `0x50` is RESERVED for an optional accessory-ID EEPROM** - protocol only, no main-board hardware, no accessory obliged to fit one (D-095). |
| **Logic safety** | **3.3 V CMOS ONLY on every signal contact.** 100 Ohm series on XGPIO, both natives **and now `ACC_DETECT_N`** - a gap D-090 had left on an exposed contact running straight to a PCAL input - 22 Ohm on I2C, 330 Ohm on WAKE. **Bidirectional level translators REJECTED** (D-090). **ESD, BUILT 2026-08-23 (D-188): four `TPD4E1B06DRLR` arrays, FITTED, covering ALL SIXTEEN exposed signal contacts** - +/-12 kV contact / +/-15 kV air-gap IEC 61000-4-2, 3.0 A surge, **0.7 pF** I/O capacitance, 0.5 nA leakage, VRWM +/-5.5 V. They arrived from Beta-DM **all DNP**; D-090 had protected only the natives and the I2C pair, which under-weighted the XGPIO. **DELIBERATELY NO TVS ON EITHER POWER RAIL - VRWM 5.5 V against a 5.0 V nominal rail leaves no working margin**; the rails are protected by bulk capacitance, the TPS22950C ratings and the current limit. `TPD2E009DBZR` leaves the BOM. Silkscreen: *"COMMUNITY PORT - 3V3 LOGIC ONLY / 5V PIN IS POWER OUTPUT ONLY"*. |
| **WAKE isolation** | **BUILT 2026-08-23 (FBV2-S1-009, D-187) - it had only ever been a decision.** `Q10` **2N7002** between `WAKE_ATTN_N_HDR` and `WAKE_INT_N`, gate = `ACC_3V3_SW`, with `R63` 10 k to `ACC_3V3_SW` on the contact side and `R66` 330 R in series. **ORIENTATION IS LOAD-BEARING: SOURCE faces the connector, DRAIN faces the internal line**, so with the rail off the body diode is reverse-biased against the internal 3.3 V and a shorted accessory **cannot hold `WAKE_INT_N` low or starve the buttons**. Reverse the FET and the body diode alone defeats it. `R63` must pull to `ACC_3V3_SW`, never `+3V3`. **Bounded residual:** an accessory *driving* the contact to 5 V injects ~3 mA through the body diode and `R66` - the reason `R66` is 330 R. **Closes B-08 in copper.** Consequence: accessory wake needs the rail held up in sleep (B-36). |
...
| **Physical presentation** | **REBUILT 2026-08-24 (FBV2-EXP-002, D-237/D-238/D-240).** `J5` is a **Samtec `SSQ-124-02-G-S-RA` 1 × 24, 2.54 mm, FEMALE, right-angle socket** presenting **one pin per line** down the right wall and mating an ordinary male header or **Dupont jumper** — the community-standard interface, not a proprietary one. **`J8` `JST SM04B-SRSS-TB` Qwiic / STEMMA QT** is added on the PROTECTED external I²C node (`EXT_SDA`/`EXT_SCL`, downstream of `U16` and the 22 Ω pair, at `D2`'s clamp) and **costs ZERO components**; its power is `ACC_3V3_SW` because `U16`'s own VCC already is, and **`ACC_5V_SW` is never exposed on it**. **ORDER-B** pin order is **safe under 180° reversal by construction — power-to-signal maps: ZERO.** **ALL 24 electrical functions and every protection component are UNCHANGED**; what changed is the presentation. |
| Mechanical | ~~**Keyed, shrouded/polarized and recessed**~~ **The asymmetric key is NO LONGER REQUIRED (D-240): a closed-end 62.5 mm recess against a 60.96 mm male body leaves 1.54 mm of play on a 2.54 mm pitch, so a one-position shift is physically impossible.** Recessed, right-side exit. **Z column 19.53 mm of the 23.0 mm external budget - 3.47 mm spare**, level with the control region and **no longer the sole governing column**. Insertion force **~33 N average** (peak higher); the enclosure must carry it (M-10). |
